...
nrow(HS_df[HS_df$f_rel_bias < abs(5),])
nrow(POP_df[POP_df$f_rel_bias < abs(5),])
nrow(HS_df[HS_df$m_rel_bias < abs(5),])
nrow(POP_df[POP_df$m_rel_bias < abs(5),])

###Spot checks
#Spot check for values
mean(HS_df$f_rel_bias[which(HS_df$prop_sampled==0.17 & HS_df$prop_mature==.08)])
mean(POP_df$f_rel_bias[which(POP_df$prop_sampled==0.2 & POP_df$prop_mature==.56)])
...
f_heatmap <- ggplot(data=HS_df, mapping=aes(x=as.character(round(prop_mature,2)),
                                           y=as.character(round(prop_sampled,2)),
                                           fill=abs(f_rel_bias2))) +
  geom_tile() +
...
ggplot(data=HS_df, aes(x=POPs_found, y=f_rel_bias)) +
  geom_point() +
  labs(x="Kin pairs detected", y="Relative bias", title="Bias by kin pairs Half-Sib") +
  ylim(-200, 200)
...
#Hitogram of kin pairs
ggplot(data=POP_df, aes(x=POPs_found, y=..density..)) + 
...
ggplot(data=HS_df, aes(x=POPs_found, y=..density..)) + 
  geom_histogram(binwidth=1, color="black", fill="white") +
