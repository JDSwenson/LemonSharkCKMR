...
#Minimum age at which adults are mature
...
get_P_cownose <- function(Pars1,P_Mother,P_Father,t_start,t_end){
  Log_trend = (0.01/(1+exp(-Pars1[3]))-0.005)*(min_est_cohort:(t_end-1))  #trend on log scale limited to +/- 0.01/yr
  N_F_yr=exp(Pars1[1]+Log_trend)
  for(os_birth in min_est_cohort:(n_yrs-1)){  #This loops over the possible ages of older sibs
    for(ys_birth in (os_birth+1):n_yrs){ #This loops over possible ages of younger sibs
      if((ys_birth - os_birth) <= ((maxAge+1) - f_age_at_mat)){ #Make sure the difference in birth years allows for an animal to be mature in both years. 
        #Fill in array with kinship probability from CKMR equation
        #See Bravington 2016 equation 3.10
        P_Mother[os_birth, ys_birth] <- (surv^(ys_birth - os_birth))/N_F_yr[ys_birth-min_est_cohort]
...
##Repeat the above with males
  ...
  N_M_yr=exp(Pars1[2]+Log_trend) #number of mature males
  for(os_birth in min_est_cohort:(n_yrs-1)){  #loop over possbile ages of older sib
    for(ys_birth in (os_birth+1):n_yrs){ #loop over possible ages of younger sib
      if((ys_birth - os_birth) <= ((maxAge+1) - m_age_at_mat)){ #Make sure adult could have been mature in both birth years
        
        #Fill array with kinship probability from half-sib CKMR equation
        #See Bravington 2016 equation 3.10
        P_Father[os_birth,ys_birth] <- (surv^(ys_birth - os_birth))/N_M_yr[ys_birth-min_est_cohort]
