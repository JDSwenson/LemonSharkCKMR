...
lambda.df <- readRDS(file = "./output/Objective.3_life_history_sensitivity/lambda.df")
...
#Split the above dataframe so I can loop through over multiple different scripts
q <- nrow(lambda.df.summ_filt)/6
l.1 <- c(1:q)
l.2 <- c((max(l.1) + 1):(max(l.1) + q))
l.3 <- c((max(l.2) + 1):(max(l.2) + q))
l.4 <- c((max(l.3) + 1):(max(l.3) + q))
l.5 <- c((max(l.4) + 1):(max(l.4) + q))
l.6 <- c((max(l.5) + 1):(max(l.5) + q))

lambda.split.1 <- lambda.df.summ_filt[l.1,]
lambda.split.2 <- lambda.df.summ_filt[l.2,]
lambda.split.3 <- lambda.df.summ_filt[l.3,]
lambda.split.4 <- lambda.df.summ_filt[l.4,]
lambda.split.5 <- lambda.df.summ_filt[l.5,]
lambda.split.6 <- lambda.df.summ_filt[l.6,]

#----------------Set input file locations ------------------------------
PopSim.location <- "./output/Population.simulations/"
PopSim.lambda <- "lambda.1" # Can be lambda.1 or lambda.variable
Sampling.scheme <- "sample.all.juvenile.ages" # sample.all.juvenile.ages, target.YOY, or sample.ALL.ages
...
MCMC_location <- "./output/Objective.3_life_history_sensitivity/Model.output/"
jags.model_location <- "./output/Objective.3_life_history_sensitivity/models/"
results_location <- "./output/Objective.3_life_history_sensitivity/Model.results/"
...
script_name <- "Obj3.1_sample.all.juvenile.ages_2.R" #Copy name of script here
...
question3 <- "Part 2 of lambda trials; sample all juvenile ages"
purpose <- "Obj3.1_sample.all.juvenile.ages_2" #For naming output files
...
samples.df <- truth.df <- readRDS(file = paste0("./output/Population.simulations/sample.info_", date.of.PopSim, "_", inSeeds, "_", PopSim.lambda, "_", Sampling.scheme))

samples.df %>% group_by(age.x) %>% summarize(n()) #Check that correct age classes were sampled

pop_size.df <- readRDS(file = paste0("./output/Population.simulations/pop.size_", date.of.PopSim, "_", inSeeds, "_", PopSim.lambda, "_", Sampling.scheme))

truth.df <- readRDS(file = paste0("./output/Population.simulations/truth_", date.of.PopSim, "_", inSeeds, "_", PopSim.lambda, "_", Sampling.scheme))
...
sim.samples <- paste0(sample.sizes[1], "prop.sampled")
...
for(lam in 1:nrow(lambda.split.2)){
  
  lambda.trial <- lambda.split.2[lam, ]
  
  lambda.prior.mean <- lambda.trial$lambda.mean
  lambda.prior.sd <- lambda.trial$lambda.sd
  survival.prior.mean <- adult.survival
  survival.prior.sd <- as.numeric(as.character(lambda.trial$adult.cv)) * survival.prior.mean
...
#If we only want to use one type of relationship, we'll filter here
    ...
    #Define JAGS data and model, and run the MCMC engine
      set.seed(rseed)
    source("Objective.3_life_history_sensitivity/functions/Obj3.1_run.JAGS_HS.only.R")
    ...
    results.temp <- model.summary2 %>% left_join(truth.iter, by = c("parameter", "iteration", "seed")) %>% 
      ...
      mutate(HSPs_detected = c(mom.HSPs, mom.HSPs, dad.HSPs, mom.HSPs + dad.HSPs, mom.HSPs + dad.HSPs),
             ...
             purpose = purpose) %>% 
      mutate(juvenile.survival.cv = lambda.trial$juvenile.cv,
             adult.survival.cv = lambda.trial$adult.cv,
             adult.fecundity.cv = lambda.trial$fecund.cv,
             surv.fec.corr = lambda.trial$survival.fecundity_correlation,
             lambda_prior_mean = lambda.prior.mean,
             lambda_prior_sd = lambda.prior.sd)
  ...
  #-----------------Save output files iteratively--------------------
   ...
   saveRDS(sims.list.1, file = paste0(temp_location, MCMC_prefix, "_", date.of.simulation, "_", outSeeds, "_", sim.samples, "_", MCMC.settings, "_", purpose))
   # 
   ...
   cat(paste0("\n Finished iteration ", iter, " of lambda trial ", lam, " of 36. \n Took ", iter.time, " minutes \n\n"))
...
write.table(results2, file = paste0(results_location, results_prefix, "_", date.of.simulation, "_", outSeeds, "_", purpose, ".csv"), sep=",", dec=".", qmethod="double", row.names=FALSE)
 ...
 saveRDS(sims.list.1, file = paste0(MCMC_location, MCMC_prefix, "_", date.of.simulation, "_", outSeeds, "_", sim.samples, "_", MCMC.settings, "_", purpose))
 
 # saveRDS(sims.list.2, file = paste0(MCMC_location, MCMC_prefix, "_", date.of.simulation, "_", outSeeds, "_", sim.samples.2, "_", MCMC.settings, "_", purpose))
 # 
 # saveRDS(sims.list.3, file = paste0(MCMC_location, MCMC_prefix, "_", date.of.simulation, "_", outSeeds, "_", sim.samples.3, "_", MCMC.settings, "_", purpose))
 # 
 # saveRDS(sims.list.4, file = paste0(MCMC_location, MCMC_prefix, "_", date.of.simulation, "_", outSeeds, "_", sim.samples.4, "_", MCMC.settings, "_", purpose))
