...
date.of.PopSim <- "22Jul2022" # 11Jul2022
...
script_name <- "scenario_2.2.2_lambda.extreme_sample.all.juvenile.ages_change.est.yr.R" #Copy name of script here
...
purpose <- "scenario_2.2.2_lambda.extreme_sample.all.juvenile.ages_change.est.yr" #For naming output files
...
#If we only want to use one type of relationship, we'll filter here
    ...
    #Define JAGS data and model, and run the MCMC engine
      set.seed(rseed)
    source("Objective.2_population.growth/functions/scenario_2.2.2_run.JAGS_HS.only.R")
