...
Sampling.scheme <- "sample.all.juvenile.ages_2" # Can be sample.all.juvenile.ages, target.YOY, or sample.ALL.ages
...
script_name <- "scenario_2.1_lambda.trial_sample.all.juvenile.ages.R" #Copy name of script here
...
question3 <- ""
purpose <- "scenario_2.1_lambda.trial_sample.all.juvenile.ages" #For naming output files
...
#If we only want to use one type of relationship, we'll filter here
    ...
    #Define JAGS data and model, and run the MCMC engine
      set.seed(rseed)
    source("Objective.2_population.growth/functions/scenario_2.1_run.JAGS_HS.only.R")

      #Calculate truth of mean population size
