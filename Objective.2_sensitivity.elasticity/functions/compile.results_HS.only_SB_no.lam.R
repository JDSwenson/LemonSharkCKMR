...
#Lambda
 lambda.prior <- lam.fix
 lambda.sd <- NA

#Create dataframe of estimates and truth
estimates <- model.summary2 %>%
  mutate(all.truth = c(Mom.all_truth, psi_truth, Dad.all_truth, surv_mean),
         breed.truth = c(Mom.breed_truth, psi_truth, Dad.breed_truth, surv_mean),
         lam.truth = c(rep(lam_truth, times = length(jags_params))),
         lambda.fix = c(rep(lam.fix, times = length(jags_params))),
         prior.mean = c(Nf.prior, psi.prior, Nm.prior, survival.prior),
         prior.sd = c(Nf.sd, psi.sd, Nm.sd, survival.sd)) %>%
  as_tibble()
...
metrics <- cbind(c(rep(mom.Exp.PO, times = 2), #for Nfa, Nf
                   dad.Exp.PO, #For Nm
