...
#Dad
dad_comps.HS <- dad_comps.all %>% dplyr::filter(type == "HS")
dad_comps.PO <- dad_comps.all %>% dplyr::filter(type == "PO") %>% 
  mutate(POP.ispaternal = 1)

dad.mort.yrs_HS <- dad_comps.HS$mort.yrs
...
#PO
all_comps.PO <- mom_comps.PO %>% bind_rows(dad_comps.PO)

POP.mort.yrs <- all_comps.PO$mort.yrs
POP.popGrowth.yrs <- all_comps.PO$pop.growth.yrs
n.comps_POPs <- all_comps.PO$all
positives_POPs <- all_comps.PO$yes
POP.yrs <- nrow(all_comps.PO)
POP.isPaternal <- all_comps.PO$POP.ispaternal
...
#Calculate parameters for beta distribution from mean and variance for survival
 ...
 # surv.beta <- surv.betaParams[[2]]
  ...
  jags_data = list(
    ...
    #POPs
    POP.mort.yrs = POP.mort.yrs,
    POP.popGrowth.yrs = POP.popGrowth.yrs,
    n.comps_POPs = n.comps_POPs,
    positives_POPs = positives_POPs,
    POP.yrs = POP.yrs,
    POP.isPaternal = POP.isPaternal,
