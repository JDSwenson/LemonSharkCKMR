...
**10/21/2021**
These simulations were run with a Bayesian model fitted with JAGS
```{r}
#neutral_growth
IBS.null_Lemon_neutral_growth_lambdaModel.df <- read_csv("~/R/working_directory/LemonSharkCKMR/02_IBS/Dovi_IBS_model_validation/Lemon_sharks/results/basic_bayesian/simple_model_neutralGrowth_validation.csv") #%>% 
  mutate(sex = factor(sex, levels = c("M", "F")))

#negative_growth_lambdaModel
IBS.null_Lemon_negative_growth_lambdaModel.df <- read_csv("~/R/working_directory/LemonSharkCKMR/02_IBS/Dovi_IBS_model_validation/Lemon_sharks/results/basic_bayesian/simple_model_validation_negGrowth.csv") #%>% 
  mutate(sex = factor(sex, levels = c("M", "F")))

#positive_growth_lambdaModel
IBS.null_Lemon_positive_growth_lambdaModel.df <- read_csv("~/R/working_directory/LemonSharkCKMR/02_IBS/Dovi_IBS_model_validation/Lemon_sharks/results/basic_bayesian/simple_model_validation_posGrowth.csv") #%>% 
  mutate(sex = factor(sex, levels = c("M", "F")))
...
IBS.null_Lemon_neutral_growth_lambdaModel.df %>% group_by(parameter, total_samples) %>% summarize(median(relative_bias))

IBS.null_Lemon_negative_growth_lambdaModel.df %>% group_by(parameter, total_samples) %>% summarize(median(relative_bias))

IBS.null_Lemon_positive_growth_lambdaModel.df %>% group_by(parameter, total_samples) %>% summarize(median(relative_bias))

neutral.df <- IBS.null_Lemon_neutral_growth_lambdaModel.df %>% 
   mutate(in_interval = ifelse(`2.5` < truth & truth < `97.5`, "Y", "N"))

pos.df <- IBS.null_Lemon_positive_growth_lambdaModel.df %>% 
  mutate(in_interval = ifelse(`2.5` < truth & truth < `97.5`, "Y", "N"))

neg.df <- IBS.null_Lemon_negative_growth_lambdaModel.df %>% 
  mutate(in_interval = ifelse(`2.5` < truth & truth < `97.5`, "Y", "N"))

table(neutral.df$in_interval)
table(pos.df$in_interval)  
table(neg.df$in_interval)
...
neutral_growth_lambdaModel_Scatter <- ggplot(data=IBS.null_Lemon_neutral_growth_lambdaModel.df, aes(x=percent_sampled)) +
  geom_jitter(aes(y=relative_bias, color=parameter)) +
...
>>>>>>> 6a22807facf5712de31fddf4874ca2ad492165fd
**06/23/2021**
