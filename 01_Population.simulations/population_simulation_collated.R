        ...
        for(i in sample.years){
          ...
          if(sampling.scheme == "target.YOY"){ #If targeting YOY for juvenile samples
            ...
            sample.size <- pop.size.yr %>%
              mutate(sample.size = round(Num.YOY*(sample.prop/100)), 0) %>%
              pull(sample.size)
            ...
            #Calculate the number of juveniles (minus YOY) in the population this year
              n.juvs <- pop.size.yr$population_size - pop.size.yr$Total.adult.pop - pop.size.yr$Num.YOY
              
              #Set number of samples to a specific proportion of the population
              sample.size <- round(n.juvs*(sample.prop/100), 0)
