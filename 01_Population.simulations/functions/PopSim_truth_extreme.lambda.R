...
surv_min <- min(sVec[ref.year:n_yrs]) #Minimum survival over estimation period
surv_max <- max(sVec[ref.year:n_yrs]) #Maximum survival over estimation period
...
(mean.adult.lambda <- mean(pop.size.tibble$adult.lambda[(n_yrs-10):n_yrs], na.rm=T)) # mean population growth for whole population
...
estimates <- tibble(parameter = c("Nf", "psi", "Nm", "survival", "lambda"),
                    ...
                    mean.adult.lambda.last10 = mean.adult.lambda,
                    adult.survival.adj = Sa,
...
if(exists("ff.shift") == TRUE){
  estimates$female.fecundity = ff.shift
}
