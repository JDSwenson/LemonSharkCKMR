...
HS.PO_model = function(){
  ...
  for(i in 1:mom.yrs_HS.even){ # Loop over maternal cohort comparisons
    mom.positives_HS.even[i] ~ dbin((surv^mom.mort.yrs_HS.even[i])/(Nf*(lam^mom.popGrowth.yrs_HS.even[i])), mom.n.comps_HS.even[i]) # Sex-specific CKMR model equation
  }

  #Moms
  #HS - odd years
  for(j in 1:mom.yrs_HS.odd){ # Loop over maternal cohort comparisons
    mom.positives_HS.odd[j] ~ dbin(((surv^mom.mort.yrs_HS.odd[j])*(1-psi))/(Nf*(lam^mom.popGrowth.yrs_HS.odd[j])), mom.n.comps_HS.odd[j]) # Sex-specific CKMR model equation
  }

  #Moms
  #PO
  for(k in 1:mom.yrs_PO){ # Loop over maternal cohort comparisons
    mom.positives_PO[k] ~ dbin((surv^mom.mort.yrs_PO[k])/(Nf*(lam^mom.popGrowth.yrs_PO[k])), mom.n.comps_PO[k]) # Sex-specific CKMR model equation
  }
...
#      psi = runif(1, min=0, max=1)
  ...
  HS.PO_model = function(){
    ...
    pb ~ dunif(0, 1) #Correction factor for breeding population vs total population
    psi ~ dunif(0, 1) #Percent of animals breeding bi-ennially
    ...
    #PO
    for(k in 1:mom.yrs_PO){ # Loop over maternal cohort comparisons
      mom.positives_PO[k] ~ dbin((surv^mom.mort.yrs_PO[k])/((Nfb/pb)*(lam^mom.popGrowth.yrs_PO[k])), mom.n.comps_PO[k]) # Sex-specific CKMR model equation
    }
...
if(abundance.only == "yes"){ #Should we only monitor abundance nodes?
  jags_params = c("Nfa", "Nm") #If fixing other parameters
