...
breeding.schedule <- "biennial.breeding_psi0.5"
sampling.scheme <- "sample.ALL.ages"

#-------------------Set and save population simulation settings----------------------------
script_name <- "population_simulation_lambda.1_sample.all.ages.SB_psi0.5.R" #Copy name of script here
...
non.conformists <- 0.5 #proportion of off-year breeders to randomly include off their breeding cycle - want to change this to non.conformists
...
pop.sim.df <- tibble(script_name = script_name,
                     ...
                     female.fecundity = as.character(ff),
                     n.sim.yrs = n_yrs
)
 ...
 for(iter in 1:iterations) {
  ...
  for(samps in 1:length(sample.vec.prop)){
    ...
    sample.df_all.info <- sample.df_all.info %>% 
      mutate(sample.size = sample.size,
             sampling.scheme = sampling.scheme,
             iteration = iter,
             seed = rseed,
