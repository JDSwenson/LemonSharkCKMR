...
NEXT TO DO 6/4/2020:
  Re-run simulations but this time save information about sampled ages to see if there is a correspondence between the heavily biased estimates and the ages sampled
```{r, echo=FALSE}
library(data.table)
library(tidyverse)
library(ggpubr)
...
files <- list.files(path = "~/R/R_working_dir/CKMR/Results/HS_constant_abundance/", pattern = "*.csv", full.names = T)
...
samps_30 <- HS %>%
  filter(Total_samples == 30)
View(samps_30)

######################## Plot #########################

#Plot relative bias by samples and sex
ggplot(data=HS, aes(x=factor(Total_samples))) +
...
G <- ggplot(data=HS, aes(x=Parents_detected)) +
