...
purpose.combo <- "Examine model performance under different sampling schemes" #What am I comparing right now? This will be used to name output files
...
#Initialize values
purpose1.lab = purpose2.lab = purpose3.lab<- NULL

#------------- Simulation parameters and labels  ----------------#
date.of.simulation1 <- "30Jun2022"
purpose1 <- "Obj2.1_target.YOY"
purpose1.lab <- "Target YOY"
model.type1 <- "HS.only"

date.of.simulation2 <- "30Jun2022"
purpose2 <- "Obj2.1_sample.all.juveniles"
purpose2.lab <- "Sample all juvenile ages"
model.type2 <- "HS.only"

date.of.simulation3 <- "30Jun2022"
purpose3 <- "Obj2.1_sample.ALL.ages"
purpose3.lab <- "Sample all ages"
model.type3 <- "HS.PO"
...
results_plots_location <- "G://My Drive/Personal_Drive/R/CKMR/Objective.2_sampling_schemes/Model.results/figures/"
...
results.1 = results.2 = results.3 <- NULL
...
no.convergence <- results.all %>% dplyr::filter(Rhat > 1.01) %>% 
  mutate(purp = ifelse(purpose.lab == purpose1.lab, 1,
                       ifelse(purpose.lab == purpose2.lab, 2, 3))) %>% 
  mutate(samps = ifelse(proportion.sampled == sim.samples.all[1], 1,
                        ifelse(proportion.sampled == sim.samples.all[2], 2,
                               ifelse(proportion.sampled == sim.samples.all[3], 3, 4)))) %>% 
...
results.all %>% group_by(proportion.sampled, parameter, purpose.lab) %>% 
...
#------------------------------- Within HPDI? -----------------------------------#
results.all %>% group_by(purpose.lab, proportion.sampled, parameter) %>% 
...
#------------------------------- CV -----------------------------------#
results.all %>% group_by(model_type, purpose.lab, proportion.sampled, parameter) %>% 
...
results.all %>% dplyr::filter(parameter == "Nfb" | parameter == "Nm") %>% 
  group_by(purpose.lab, parameter, proportion.sampled) %>% 
...
HPD.samples1.1.4viz <- results.1 %>% filter(proportion.sampled == sim.samples.all[1]) %>% 
  right_join(HPD.samples1.1, by = c("parameter", "iteration")) %>% 
  anti_join(no.convergence, by = c("purpose.lab", "iteration", "proportion.sampled")) %>%  #Remove iterations that failed to converge
  mutate(samples.lab = samples1.lab)
  
HPD.samples2.1.4viz <- results.1 %>% filter(proportion.sampled == sim.samples.all[2]) %>% 
  right_join(HPD.samples2.1, by = c("parameter", "iteration")) %>% 
  anti_join(no.convergence, by = c("purpose.lab", "iteration", "proportion.sampled")) %>%  #Remove iterations that failed to converge
  mutate(samples.lab = samples2.lab)

HPD.samples3.1.4viz <- results.1 %>% dplyr::filter(proportion.sampled == sim.samples.all[3]) %>% 
  right_join(HPD.samples3.1, by = c("parameter", "iteration")) %>% 
  anti_join(no.convergence, by = c("purpose.lab", "iteration", "proportion.sampled")) %>%  #Remove iterations that failed to converge
  mutate(samples.lab = samples3.lab)

HPD.samples4.1.4viz <- results.1 %>% filter(proportion.sampled == sim.samples.all[4]) %>% 
...
#---------------------------Purpose 2 --------------------------------#    
#Separate out by sample size
HPD.samples1.2.4viz <- results.2 %>% filter(proportion.sampled == sim.samples.all[1]) %>% 
  right_join(HPD.samples1.2, by = c("parameter", "iteration")) %>% 
  anti_join(no.convergence, by = c("purpose.lab", "iteration", "proportion.sampled")) %>%  #Remove iterations that failed to converge
  mutate(samples.lab = samples1.lab)

HPD.samples2.2.4viz <- results.2 %>% filter(proportion.sampled == sim.samples.all[2]) %>% 
  right_join(HPD.samples2.2, by = c("parameter", "iteration")) %>% 
    anti_join(no.convergence, by = c("purpose.lab", "iteration", "proportion.sampled")) %>%  #Remove iterations that failed to converge
  mutate(samples.lab = samples2.lab)

HPD.samples3.2.4viz <- results.2 %>% filter(proportion.sampled == sim.samples.all[3]) %>% 
  right_join(HPD.samples3.2, by = c("parameter", "iteration")) %>% 
  anti_join(no.convergence, by = c("purpose.lab", "iteration", "proportion.sampled")) %>%  #Remove iterations that failed to converge
  mutate(samples.lab = samples3.lab)


HPD.samples4.2.4viz <- results.2 %>% filter(proportion.sampled == sim.samples.all[4]) %>% 
...
#---------------------------Purpose 3 --------------------------------# 
#Separate out by sample size
HPD.samples1.3.4viz <- results.3 %>% filter(proportion.sampled == sim.samples.all[1]) %>% 
  right_join(HPD.samples1.3, by = c("parameter", "iteration")) %>% 
  anti_join(no.convergence, by = c("purpose.lab", "iteration", "proportion.sampled")) %>%  #Remove iterations that failed to converge
  mutate(samples.lab = samples1.lab)


HPD.samples2.3.4viz <- results.3 %>% filter(proportion.sampled == sim.samples.all[2]) %>% 
  right_join(HPD.samples2.3, by = c("parameter", "iteration")) %>% 
  anti_join(no.convergence, by = c("purpose.lab", "iteration", "proportion.sampled")) %>%  #Remove iterations that failed to converge
  mutate(samples.lab = samples2.lab)


HPD.samples3.3.4viz <- results.3 %>% filter(proportion.sampled == sim.samples.all[3]) %>% 
  right_join(HPD.samples3.3, by = c("parameter", "iteration")) %>% 
  anti_join(no.convergence, by = c("purpose.lab", "iteration", "proportion.sampled")) %>%  #Remove iterations that failed to converge
  mutate(samples.lab = samples3.lab)


HPD.samples4.3.4viz <- results.3 %>% filter(proportion.sampled == sim.samples.all[4]) %>% 
...
### The above is the main part that needs to be run with this script to produce the output files for data viz.


```{r Continue with analysis}
...
#Format PO dataframe
results.PO <- results.all %>% dplyr::select(parameter, total.sample.size, kin.detected = POPs_detected,  unique_parents_in_sample, iteration, cv, purpose = purpose2) %>% 
  mutate(type = "PO")

#Format HS dataframe
results.HS <- results.all %>% dplyr::select(parameter, total.sample.size, kin.detected = HSPs_detected,  unique_parents_in_sample, iteration, cv, purpose = purpose2) %>% 
  mutate(type = "HS")

#Combine PO and HS dataframe for plotting
results.kin <- rbind(results.PO, results.HS) %>% 
  mutate(total.sample.size = paste0(total.sample.size, " samples"))

results.kin$total.sample.size <- factor(results.kin$total.sample.size, levels = c("200 samples", "600 samples", "1000 samples"))
...
k1 <- results.kin %>% dplyr::filter(parameter == "Nf", type == "PO") %>%
  ...
  xlim(0, 60) + 
  facet_wrap(~total.sample.size, ncol = 1) + 
  theme(legend.title = element_blank())
...
results.all$sample.label <- factor(paste0(results.all$total.sample.size, " samples"), levels = c("200 samples", "600 samples", "1000 samples"))
