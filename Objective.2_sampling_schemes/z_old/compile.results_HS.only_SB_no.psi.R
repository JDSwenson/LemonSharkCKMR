...
estimates <- model.summary2 %>%
  mutate(all.truth = c(Mom.all_truth, Dad.all_truth, surv_mean, lam_truth),
         ...
         psi.truth = c(rep(psi_truth, times = length(jags_params))),
         psi.fix = c(rep(1, times = length(jags_params))),
         prior.mean = c(Nf.prior, Nm.prior, survival.prior, lambda.prior),
         prior.sd = c(Nf.sd, Nm.sd, survival.sd, lambda.sd)) %>%
  as_tibble()
...
metrics <- cbind(c(mom.Exp.PO, #for Nfa, Nf
                   dad.Exp.PO, #For Nm
