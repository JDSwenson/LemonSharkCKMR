...
PopSim.lambda <- "lambda.1" #choices are lambda.variable or lambda.1
Sampling.scheme <- "sample.all.juvenile.ages" #choices are target.YOY or sample.all.juvenile.ages OR sample.ALL.ages (to include parents in sampling)
date.of.PopSim <- "21Jun2022" #21Jun2022 UNLESS running lambda.1, sample.ALL.ages (i.e. sampling adults too), in which case 28Jun2022
...
script_name <- "Obj1.1_model.validation_uniform.lambda_PopSimLambda.1_sample.all.juvenile.ages.R" #Copy name of script here
primary_goal <- "Model validation in data-poor scenario" #Why am I running this simulation? Provide details

question1 <- "Does the model perform reasonably well when we have diffuse priors on everything when lambda is stable?"
...
purpose <- "Obj1.1_model.validation_uniform.lambda_PopSimLambda.1_sample.all.juvenile.ages" #For naming output files
