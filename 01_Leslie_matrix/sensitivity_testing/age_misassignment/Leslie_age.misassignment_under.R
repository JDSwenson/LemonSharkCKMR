...
#parameters for zero population growth
...
Samples <- c() 
  ...
  Samples <- sample(c(1:maxAge), n_samples, replace=TRUE, prob=stable_age) #birth year - probability of sampling based on Prop_age
  mis_vec <- c(seq(-3, 0, by=1)) #misassignment of ages
  Samples_mis <- Samples + sample(mis_vec, n_samples, replace = TRUE)
  Samples_mis_corr <- ifelse(Samples_mis > maxAge, maxAge, ifelse(
    Samples_mis < 1, 1, Samples_mis))
  
  Samples2 <- t_end-Samples_mis_corr
...
#  results$Relative_bias <- round(((results$N_est - results$Truth)/results$Truth)*100,1)
  ...
  write.table(results, file = paste0("/home/js16a/R/working_directory/CKMR_simulations/results/Leslie.null_", n_samples, ".samples_02.10.2021_ages.misassigned.under.csv"), sep=",", dec=".", qmethod="double", row.names=FALSE)
}