...
script_name <- "scenario_4.2.1_ageMiss_sample.ALL.ages.R" #Copy name of script here
...
purpose <- "scenario_4.2.1_ageMiss_sample.ALL.ages" #For naming output files
...
age.cv <- 0.10 #age misassignment cv
...
#If we only want to use one type of relationship, we'll filter here
    ...
    #Define JAGS data and model, and run the MCMC engine
      set.seed(rseed)
      source("Objective.4_aging_error/functions/Obj4_runJAGS_HSPO_wLam.R")
