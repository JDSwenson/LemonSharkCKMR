...
num.samps_down.block3.tibble <- NULL
num.samps_down.block5.tibble <- NULL
num.samps_down.all.tibble <- NULL
...
#### End Three year block ####
  ...
  #Three year
  num.samps_down.block3 <- tibble(samps = num.samps_down.block3) %>% mutate(seed = rseed,
             iteration = iteration,
             estimation.year = estimation.year,
             time_window = "three year block")
  
  num.samps_down.block3.tibble <- bind_rows(num.samps_down.block3.tibble, num.samps_down.block3)
  
  #Five year
  num.samps_down.block5 <- tibble(samps = num.samps_down.block5) %>% mutate(seed = rseed,
                                                                            estimation.year = estimation.year,
                                                                            iteration = iteration,
                                                                            time_window = "five year block")
  
  num.samps_down.block5.tibble <- bind_rows(num.samps_down.block5.tibble, num.samps_down.block5)

  #all
  num.samps_down.all <- tibble(samps = num.samps_down.all) %>% mutate(seed = rseed,
                                                                            iteration = iteration,
                                                                      estimation.year = estimation.year,
                                                                            time_window = "all available samples")
  
  num.samps_down.all.tibble <- bind_rows(num.samps_down.all.tibble, num.samps_down.all)
  
  num.samps_down.all.tibble %>% dplyr::filter(time_window == "all available samples") %>% 
    group_by(estimation.year, time_window) %>% 
    summarize(round(mean(samps), 0))
