...
source("./time_series/models/get_P_lemon_HS_time_series_4yrs_TotalA.R")
...
sim_results <- matrix(0, nrow = iterations, ncol = 23)

for(samps in 1:3){
  n_samples <- c(90, 120, 150)[samps]
...
####LOOP####
  ...
  #########################Fit model!##############################
    ...
    #Store estimates in vector -- need to change based on number of parameters
      estimates <- c(exp(CK_fit$p1[1]),exp(CK_fit$p2[1]), exp(CK_fit$p3[1]), exp(CK_fit$p4[1]))
...
#cat(paste("Estimated adult abundance: ",c(exp(CK_fit[1,1:4])),", SE = ",c(SE[1:4]),"\n"))
 #cat(paste("Estimated mature male abundance: ",c(exp(CK_fit[1,5:8])),", SE = ",c(SE[5:8]),"\n"))
    ...
    sim_results[iter, 4] <- sum(Data_matches[which(Data_matches$Young_sib_birth==4),3]) #Number of parents
    sim_results[iter, 5] <- round(exp(CK_fit$p2[1]),0) #N Adults
    sim_results[iter, 6] <- round(SE[2],0) #N Adults_SE
    sim_results[iter, 7] <- exp(Pars[2]) #Truth
    sim_results[iter, 8] <- sum(Data_matches[which(Data_matches$Young_sib_birth==5),3]) #Number of parents
    sim_results[iter, 9] <- round(exp(CK_fit$p3[1]),0) #N Adults
    sim_results[iter, 10] <- round(SE[3],0) #N AdultsSE
    sim_results[iter, 11] <- exp(Pars[3]) #Truth
    sim_results[iter, 12] <- sum(Data_matches[which(Data_matches$Young_sib_birth==6),3]) #Number of parents
    sim_results[iter, 13] <- round(exp(CK_fit$p4[1]),0) #N Adults
    sim_results[iter, 14] <- round(SE[4],0) #N AdultsSE
    sim_results[iter, 15] <- exp(Pars[4]) #Truth
    sim_results[iter, 16] <- sum(Data_matches[which(Data_matches$Young_sib_birth==7),3]) #Number of parents
    sim_results[iter, 17] <- sum(Samples==1)
    sim_results[iter, 18] <- sum(Samples==2)
    sim_results[iter, 19] <- sum(Samples==3)
    sim_results[iter, 20] <- sum(Samples==4)
    sim_results[iter, 21] <- sum(Samples==5)
    sim_results[iter, 22] <- sum(Samples==6)
    sim_results[iter, 23] <- sum(Samples==7)
    
    save(CK_fit, file=paste0("../Results/model_objects/Lemon_CKModel_HS_time_series_smallPop_TotalA_null_iteration_", iter))
    #  } else {
...
colnames(sim_results) <- c("N_adults_yr4", "N_adults_SE_yr4", "N_adultsTruth_yr4", "Parents_detected_yr4",
                           "N_adults_yr5", "N_adults_SE_yr5", "N_adultsTruth_yr5", "Parents_detected_yr5",
                           "N_adults_yr6", "N_adults_SE_yr6", "N_adultsTruth_yr6", "Parents_detected_yr6",
                           "N_adults_yr7", "N_adults_SE_yr7", "N_adultsTruth_yr7", "Parents_detected_yr7", "Yr1_samples", "Yr2_samples", "Yr3_samples", "Yr4_samples", "Yr5_samples", "Yr6_samples", "Yr7_samples")
#head(sim_results, 30)

write.table(sim_results, file = paste0("../Results/HS_time_series_smallpop_TotalA_", n_samples,"samps_6.11.2020.csv"), sep=",", dec=".", qmethod="double", row.names=FALSE)
}
