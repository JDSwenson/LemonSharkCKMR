...
objective <- 3 #Can be any number for the objectives (1-5)
scenario <- "scenario_3.1.3" #See Excel sheet with simulation scenarios: Simulation_log_key_UPDATED.xlsx on Google Drive
 ...
 for(iter in 1:iterations) {
   ...
   for(s in 1:length(sample.prop_vec)){
    ...
    #Save output as different dataframes; includes both HS and PO relationships (but can filter below)
      #Remove off-cycle HS comparisons
...
#If we only want to use one type of relationship, we'll filter here
    ...
    ####------------------------ Fit CKMR model ----------------####
      ...
      #Remove off-cycle comparisons
      mom_comps.all <- mom_comps.all %>% 
        dplyr::filter(case_when(type == "HS" ~ BI == "on",
                                TRUE ~ type == "PO"))
