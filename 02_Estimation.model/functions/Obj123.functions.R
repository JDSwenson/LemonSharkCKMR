...
find_aunt.uncle_nephew.niece_pairs <- function(sample.info = sample.info2, 
  ...
  sample.info_w_grandparents <- sample.info %>% 
    ...
    dplyr::select(indv.name, 
                  ...
                  seed, 
                  sample.prop, 
                  ref.yr) %>% #Keep most columns but remove the x from mother and father
  ...
  aunt.unc_niece.nephew_pw.comps.all <- sampled_aunts_or_uncs.df %>% 
    ...
    mutate(aunt.unc_relation = ifelse(aunt.unc_sex == "F", "aunt", 
                                      ifelse(aunt.unc_sex == "M", "uncle", NA)),
           niece.nephew_relation = ifelse(niece.nephew_sex == "F", "niece",
