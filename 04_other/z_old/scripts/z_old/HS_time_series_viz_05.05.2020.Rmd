...
HS time-series null model simulations with small population size
...
files <- list.files(path = "~/R/LemonSharkCKMR/", pattern = "*.csv", full.names = T)
...
#UGLY ... make dataframe that merges data from males and females into one column and add column names
HS2 <- data.frame(cbind(c(HS$Nf, HS$Nm), c(HS$NfSE, HS$NmSE), c(HS$F_truth, HS$M_truth), c(HS$Mothers_detected, HS$Fathers_detected), c(rep(HS$Total_samples, times=2))))
HS2$Sex <- c(rep(c("F", "M"), each=nrow(HS)))
colnames(HS2) <- c("N_est", "N_SE", "Truth", "Parents_detected", "Total_samples", "Sex")
...
#Get mean relative bias by samples taken
HS2 %>% group_by(Total_samples) %>% 
  summarize(mean(Relative_bias))

#Get mean parents detected by samples taken
HS2 %>% group_by(Total_samples, Sex) %>% 
  summarize(mean(Parents_detected))

HS3 <- HS2 %>%
  filter(Parents_detected > 5)

######################## Plot #########################

#Plot relative bias by samples and sex
ggplot(data=HS3, aes(x=factor(Total_samples))) +
...
G <- ggplot(data=HS2, aes(x=Parents_detected)) +
