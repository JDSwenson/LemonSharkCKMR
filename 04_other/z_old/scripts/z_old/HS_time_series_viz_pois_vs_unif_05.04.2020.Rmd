...
---
Overall result 5/4/2020: Assigning kinship based on the poisson and uniform distributions appear to give the same results. So, I'll use the Poisson ... 
...
getwd()
HS_ts_unif <- read.csv(file="HS_time_series_null_4.30.2020.csv", header=TRUE)
HS_ts_pois <- read.csv(file="HS_time_series_null_poisson_05.01.2020.csv", header=TRUE)
...
#Create columns for relative bias for each abundance estimate - uniform
for(i in 1:length(col_f_yrs)){ #years analyzed
  colname_f <- paste0(col_f_yrs[i],"_rel_bias")
  colname_m <- paste0(col_m_yrs[i],"_rel_bias")
  for(j in 1:nrow(HS_ts_unif)){
    HS_ts_unif[j, colname_f] <- ((HS_ts_unif[j, col_f_yrs[i]] - HS_ts_unif[j,col_f_truth[i]])/HS_ts_unif[j,col_f_truth[i]])*100
        HS_ts_unif[j, colname_m] <- ((HS_ts_unif[j, col_m_yrs[i]] - HS_ts_unif[j,col_m_truth[i]])/HS_ts_unif[j,col_m_truth[i]])*100
  }
}
...
#Create dataframe where females and males are combined into the same column - females first - pois
HS_ts_unif2 <- NULL
HS_ts_unif2 <- data.frame(cbind(Yr4_rel_bias=c(HS_ts_unif[,40], HS_ts_unif[,41]), parents_detected_yr4=c(HS_ts_unif[,4],HS_ts_unif[,20]), Yr5_rel_bias=c(HS_ts_unif[,42], HS_ts_unif[,43]), parents_detected_yr5=c(HS_ts_unif[,8],HS_ts_unif[,24]), Yr6_rel_bias=c(HS_ts_unif[,44], HS_ts_unif[,45]), parents_detected_yr6=c(HS_ts_unif[,12],HS_ts_unif[,28]), Yr7_rel_bias=c(HS_ts_unif[,46], HS_ts_unif[,47]), parents_detected_yr7=c(HS_ts_unif[,16],HS_ts_unif[,32]), sex_truth=rep(c(N_f[1],N_m[1]), each=500), total_truth=total_abundance[1]))

HS_ts_unif2$sex <- rep(c("F","M"), each=500) #Add column for sex for visualizing
head(HS_ts)
head(HS_ts_unif2)
tail(HS_ts)
tail(HS_ts_unif2)
...
HS_ts_unif2 <- as.data.table(HS_ts_unif2) #to melt multiple columns, need to convert dataframe to data.table
head(HS_ts_unif2)
colnames(HS_ts_unif2)

#Melt columns together so different years are factors
var1_unif <- colnames(HS_ts_unif2)[c(1,3,5,7)] #store column names for relative bias
var2_unif <- colnames(HS_ts_unif2)[c(2, 4, 6, 8)] #store column names for parents detected

HS.m1_unif <- reshape2::melt(HS_ts_unif2, id.vars='sex', measure.vars=var1_unif, value.name="relative_bias")
HS.m2_unif <- reshape2::melt(HS_ts_unif2, id.vars='sex', measure.vars=var2_unif, value.name="parents_detected")

HS.m_unif <- NULL
HS.m_unif <- cbind(HS.m1_unif, parents_detected=HS.m2_unif$parents_detected, sex_truth=HS_ts_unif2$sex_truth, total_truth=HS_ts_unif2$total_truth)

head(HS.m_unif) #check that values match
head(HS_ts_unif2)
tail(HS.m_unif)
tail(HS_ts_unif2)
nrow(HS.m_unif) #should be iterations x 2(M+F) x number of years estimated (e.g. 500 iterations estimating 4 years should be 4,000 rows long)

#### Poisson ####
#Create columns for relative bias for each abundance estimate - poisson
for(i in 1:length(col_f_yrs)){ #years analyzed
  colname_f <- paste0(col_f_yrs[i],"_rel_bias")
  colname_m <- paste0(col_m_yrs[i],"_rel_bias")
  for(j in 1:nrow(HS_ts_pois)){
    HS_ts_pois[j, colname_f] <- ((HS_ts_pois[j, col_f_yrs[i]] - HS_ts_pois[j,col_f_truth[i]])/HS_ts_pois[j,col_f_truth[i]])*100
        HS_ts_pois[j, colname_m] <- ((HS_ts_pois[j, col_m_yrs[i]] - HS_ts_pois[j,col_m_truth[i]])/HS_ts_pois[j,col_m_truth[i]])*100
  }
}
...
#Create dataframe where females and males are combined into the same column - females first - pois
HS_ts_pois2 <- NULL
HS_ts_pois2 <- data.frame(cbind(Yr4_rel_bias=c(HS_ts_pois[,40], HS_ts_pois[,41]), parents_detected_yr4=c(HS_ts_pois[,4],HS_ts_pois[,20]), Yr5_rel_bias=c(HS_ts_pois[,42], HS_ts_pois[,43]), parents_detected_yr5=c(HS_ts_pois[,8],HS_ts_pois[,24]), Yr6_rel_bias=c(HS_ts_pois[,44], HS_ts_pois[,45]), parents_detected_yr6=c(HS_ts_pois[,12],HS_ts_pois[,28]), Yr7_rel_bias=c(HS_ts_pois[,46], HS_ts_pois[,47]), parents_detected_yr7=c(HS_ts_pois[,16],HS_ts_pois[,32]), sex_truth=rep(c(N_f[1],N_m[1]), each=200), total_truth=total_abundance[1]))

HS_ts_pois2$sex <- rep(c("F","M"), each=200) #Add column for sex for visualizing
head(HS_ts_pois)
head(HS_ts_pois2)
tail(HS_ts_pois)
tail(HS_ts_pois2)
...
HS_ts_pois2 <- as.data.table(HS_ts_pois2) #to melt multiple columns, need to convert dataframe to data.table
head(HS_ts_pois2)
colnames(HS_ts_pois2)

#Melt columns together so different years are factors
var1_pois <- colnames(HS_ts_pois2)[c(1,3,5,7)] #store column names for relative bias
var2_pois <- colnames(HS_ts_pois2)[c(2, 4, 6, 8)] #store column names for parents detected

HS.m1_pois <- reshape2::melt(HS_ts_pois2, id.vars='sex', measure.vars=var1_pois, value.name="relative_bias")
HS.m2_pois <- reshape2::melt(HS_ts_pois2, id.vars='sex', measure.vars=var2_pois, value.name="parents_detected")

HS.m_pois <- NULL
HS.m_pois <- cbind(HS.m1_pois, parents_detected=HS.m2_pois$parents_detected, sex_truth=HS_ts_pois2$sex_truth, total_truth=HS_ts_pois2$total_truth)

head(HS.m_pois) #check that values match
head(HS_ts_pois2)
tail(HS.m_pois)
tail(HS_ts_pois2)
nrow(HS.m_pois) #should be iterations x 2(M+F) x number of years estimated (e.g. 500 iterations estimating 4 years should be 4,000 rows long)
```

Do some quick calculations and comparisons
```{r, echo=FALSE}
#Need to understand how the bias relates to number of half-siblings detected
#Use dataframe HS.m; HS_ts is the original
apply(HS_ts_unif[, grep("^Dads", colnames(HS_ts))], 2, mean) #Calculate mean detected dads
apply(HS_ts_unif[, grep("^Moms", colnames(HS_ts))], 2, mean) #Calculate mean detected moms

apply(HS_ts_pois[, grep("^Dads", colnames(HS_ts))], 2, mean) #Calculate mean detected dads
apply(HS_ts_pois[, grep("^Moms", colnames(HS_ts))], 2, mean) #Calculate mean detected moms

apply(HS_ts_pois[,40:47], 2, mean) #Calculate mean detected moms
apply(HS_ts_unif[,40:47], 2, mean) #Calculate mean detected moms
```

Visualize data
```{r, echo=FALSE}
#Plot relative bias by year - uniform
g_unif <- ggplot(data=HS.m_unif, aes(x=variable)) +
...
#Plot relative bias by year - Poisson
g_pois <- ggplot(data=HS.m_pois, aes(x=variable)) +
  geom_boxplot(aes(y=relative_bias, fill=sex)) +
  ylim(-100, 100) +
  geom_hline(yintercept=0, col="black", size=1.25) +
  annotate("rect", xmin=0, xmax=Inf, ymin=-20, ymax=20, alpha=.5, col="red") +
...
#Plot relative bias by kin pairs detected - scatter plot - uniform
gk_unif <- ggplot(data=HS.m_unif, aes(x=parents_detected)) +
...
#Plot relative bias by kin pairs detected - scatter plot - poisson
gk_pois <- ggplot(data=HS.m_pois, aes(x=parents_detected)) +
  geom_point(aes(y=relative_bias, color=sex)) +
  ylim(-100, 100) +
  labs(x="Parents detected", y="Relative bias", title="Relative Bias by Kin Pairs") +
  scale_colour_brewer(palette="Set2")
