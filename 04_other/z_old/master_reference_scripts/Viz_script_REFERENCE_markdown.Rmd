...
files_null <- list.files(path = "~/R/R_working_dir/CKMR/LemonSharkCKMR_GitHub/null/results", pattern = "*.csv", full.names = T)
null_hs <- sapply(files, read_csv, simplify=FALSE) %>% 
bind_rows() #%>% 
  #drop_na()

#Sex-specific
files_fishSim <- Sys.glob("~/R/R_working_dir/CKMR/LemonSharkCKMR_GitHub/fishSim/results/2021.02.03/*.N.csv")
fs_hs <- sapply(files, read_csv, simplify=FALSE) %>% 
bind_rows() #%>% 
  #drop_na()

files_age <- Sys.glob("~/R/R_working_dir/CKMR/LemonSharkCKMR_GitHub/fishSim/results/2021.02.03/*age.dist.csv")
