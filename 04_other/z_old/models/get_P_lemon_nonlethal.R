...
####Troubleshoot####
#Add another dimension for young capture year to array? Maybe just need juvenile birth year ...
...
get_P_lemon <- function(Pars,P_Mother,P_Father,n_yrs,t_start,t_end){
     ...
     for(acapt in max(abirth, t_start):t_end){
       for(jbirth in 1:n_yrs){
         if(jbirth <= acapt & jbirth > abirth & ((acapt - abirth) <= n_ages) & ((jbirth - abirth) <= n_ages)){
          P_Mother[abirth,acapt,jbirth] <- f_mat[jbirth -abirth]/N_F #If juvenile was born before the adult was captured AND after the adult was born AND the adult wasn't too old to have been captured OR to have given birth to the juvenile, then probability equals f_mat at the age the adult was when the juvenile was born over N_f
          } else if(jbirth > acapt & jbirth - abirth >= min(f_adult_age) & ((acapt - abirth) <= n_ages) & ((jbirth - abirth) <= n_ages)) {
  ...
  for(abirth in 1:n_yrs){  #dad
    for(acapt in max(abirth, t_start):t_end){ 
      for(jbirth in 1:n_yrs){
        if(jbirth <= acapt & jbirth > abirth & ((acapt - abirth) <= n_ages) & ((jbirth - abirth) <= n_ages)){
          P_Father[abirth,acapt,jbirth] <- m_mat[jbirth -abirth]/N_M #If juvenile was born before the adult was captured AND after the adult was born AND the adult wasn't too old, then probability equals f_mat at the age the adult was when the juvenile was born over N_f
        } else if(jbirth > acapt & jbirth - abirth >= min(f_adult_age) & ((acapt - abirth) <= n_ages) & ((jbirth - abirth) <= n_ages)) {
