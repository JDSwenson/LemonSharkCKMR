...
maxClutch <- 1 #Maximum number of offspring one individual can possibly produce
...
batchSize <- .65 #Average size of clutch (8) divided by two for skipped breeding
fb <- batchSize/2 #Litter size for Leslie Matrix - females only

Leslie_input <- data.frame(
  x = c(0:maxAge), #age
  sx = c(rep(0.91, times = 30), 0), #survival
...
for(samps in 1:4){
  ...
  for(iter in 1:iterations) {
    ...
    for (y in 1:length(sim_yrs)) {
      ...
      indiv <- altMate(indiv, batchSize = batchSize, fecundityDist = "binomial", year = y, type = mat_type, maxClutch = maxClutch, singlePaternity = FALSE, maleCurve = maleCurve, femaleCurve = femaleCurve)
