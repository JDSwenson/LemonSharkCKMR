...
t_end = 49 #Last year we take samples
...
pop_size_mean <- mean(pop_size[min_cohort:n_yrs])

estimates <- data.frame(cbind(round(exp(c(CK_fit$p1[1], CK_fit$p2[1])),0)), c(SE), c("F", "M"))
estimates <- cbind(estimates, c(Mom_truth_mean, Dad_truth_mean))
...
results <- results %>% 
  ...
  write.table(results, file = paste0("~/R/R_working_dir/LemonSharkCKMR_GitHub/02_IBS/fishSim_model_validation/results/fishSim_AvgN_", n_samples_total, "_samples_sex-specific_loop_10_yrs_05.12.2021.csv"), sep=",", dec=".", qmethod="double", row.names=FALSE)
}
