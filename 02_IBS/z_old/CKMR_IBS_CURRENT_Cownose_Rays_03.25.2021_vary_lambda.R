...
Num.years <- 59 # The number of years to run in the simulation beyond the burn in
...
Pars1 <- c(log(f.init), log(m.init), 0)
...
#lam <- lambda1(A1_pre)
lam <- mean(pop.size$Lambda[min_est_cohort:60], na.rm=T) #Mean Lambda
#lam <- 1
...
#For PC
source("~/R/R_working_dir/CKMR/LemonSharkCKMR_GitHub/functions/get_P_cownose_HS_sex-specific_vary_lambda.R")
...
D1=diag(length(Pars1))*c(exp(CK_fit1$p1[1]),exp(CK_fit1$p2[1]), 0.01*exp(CK_fit1$p3)/(1+exp(CK_fit1$p3))^2) #derivatives of transformations
...
estimates <- data.frame(cbind(round(exp(c(CK_fit1$p1[1], CK_fit1$p2[1], CK_fit2$p1[1])),0)), c(SE1[1:2], SE2), c("F", "M", "All"))
...
#Save demographic output
pop.size <- pop.size %>% mutate(iter = iter)
pop.size_all <- rbind(pop.size_all, pop.size)
...
write.table(pop.size_all, file = paste0("~/R/R_working_dir/CKMR/LemonSharkCKMR_GitHub/IBS_simulations/Dovi_IBS_results/demographic_output/Dovi.IBS_", total_samples, ".samples_Cownose.Ray_03.26.2021_demographic.out.csv"), sep=",", dec=".", qmethod="double", row.names=FALSE)
 
write.table(results, file = paste0("~/R/R_working_dir/CKMR/LemonSharkCKMR_GitHub/IBS_simulations/Dovi_IBS_results/Dovi.IBS_", total_samples, ".samples_Cownose.Ray_03.26.2021_N.csv"), sep=",", dec=".", qmethod="double", row.names=FALSE)
