...
t_start = 51 #First year we take samples
t_end = 60 #Last year we take samples
...
results <- results %>% 
  ...
  write.table(results, file = paste0("fishSim_AvgN_", n_samples, "_samples_capt_b4_mort_05.11.2021.csv"), sep=",", dec=".", qmethod="double", row.names=FALSE)
}

###Currently (5/11/2021 9pm):
# Running on a loop taking small number of samples per year over a long period of time.

# Next, 

#try running the exact same script, but put capture after altmate and BEFORE mort. Maybe the sampling of year 0 animals is screwing things up; post- vs pre-breeding census

# Compare these results to taking the same number of samples over a shorter period of time.

# The bias could arise from not sampling enough cohorts combined with YOY mortality.

# CURRENTLY: Running the slightly older sex-specific script only. It seems for some reason this might produce unbiased results?
...
IBS.null_results_CNR <- sapply(IBS.null_CNR, read_csv, simplify=FALSE) %>% 
  bind_rows() %>% mutate(Total_samples = ifelse(is.na(Total_samples), Samples, Total_samples))

ggplot(data=IBS.null_results_CNR, aes(x = factor(Total_samples))) + 
