...
Leslie_input <- data.frame(
  x = c(0:maxAge), #age
  sx = c(.6, .72, rep(0.85, times = 28), 0), #survival
...
#PoNG(mateType = "ageSex", mortType = "age", batchSize = 3, firstBreed = 12, ageMort = ageMort, maxClutch = maxClutch, maxAge = maxAge, femaleCurve = femaleCurve)
...
for(samps in 1:4){
  ...
  for(iter in 1:iterations) {
    ...
    age_p <- indiv %>% filter(is.na(DeathY) == TRUE) %>% 
      group_by(AgeLast) %>% 
      dplyr::summarize(yr1 = n()/total_abundance)
    ...
    #  group_by(AgeLast) %>% 
    #  summarize(age_prop = n()/total_abundance) %>% 
    #  select(age_prop) %>% 
    #  t()
    ...
    for (y in 1:length(sim_yrs)) {
      ...
      ###Proportion-at-age###
      #Save age distribution at the beginning of each year. Column corresponds to age; iteration is added later
      ap <- indiv %>% filter(is.na(DeathY) == TRUE) %>% #filter for living individuals
        group_by(AgeLast) %>% #group by age
...
#Dad_truth
    ...
    mean_pop_growth <- mean(pop_growth_all)
    ...
    get_P_lemon <- function(Pars,P_Mother,P_Father,n_yrs,t_start,t_end){
      ...
      for(os_birth in 1:(n_yrs-1)){  #> = after, < = before
        for(ys_birth in (os_birth+1):n_yrs){
            P_Mother[os_birth, ys_birth] <- (mean_adult_surv^(ys_birth - os_birth))/(N_F*mean_pop_growth^(ys_birth-min_est_cohort_F))
          }
        }
      N_M=exp(Pars[2]) #number of mature males (time constant) - (total reproductive output from males)
      for(os_birth in 1:(n_yrs-1)){  #> = after, < = before
        for(ys_birth in (os_birth+1):n_yrs){
            P_Father[os_birth,ys_birth] <- (mean_adult_surv^(ys_birth - os_birth))/(N_M*mean_pop_growth^(ys_birth-min_est_cohort_M))
          }
        }
...
#Fit model
    ...
    #-----------------Loop end-----------------------------
results[index,] <- c(round(exp(CK_fit[1]),0), round(SE[1],0), round(Mom_truth[min_est_cohort_F],0), "F", sum(mom_positives[,3]), mean_pop_growth, n_samples)
results[index+1,] <- c(round(exp(CK_fit[2]),0), round(SE[2],0), round(Dad_truth[min_est_cohort_M],0), "M", sum(dad_positives[,3]), mean_pop_growth, n_samples)
