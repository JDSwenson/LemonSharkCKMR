...
batchSize = 3 #average size of brood. Fecundity divided by 2 (for skipped breeding). Value of 3.5 gives almost 0 population growth; value of 4 gives positive population growth, value of 3 gives negative population growth
...
#Fit model
    ...
    #-----------------Loop end-----------------------------
results[index,] <- c(round(exp(CK_fit[1]),0), round(SE[1],0), round(Mom_truth[min_est_cohort_F],0), "F", sum(mom_positives[,3]), n_samples)
results[index+1,] <- c(round(exp(CK_fit[2]),0), round(SE[2],0), round(Dad_truth[min_est_cohort_M],0), "M", sum(dad_positives[,3]), n_samples)
colnames(results) <- c("N_est", "SE", "Truth", "Sex", "Parents_detected", "Samples")
 ...
 results <- results %>% 
  ...
  write.table(results, file = paste0("results/sex.specific_single.est_no.popgrowth_", n_samples, "_samples.12.01.2020.csv"), sep=",", dec=".", qmethod="double", row.names=FALSE)
}
