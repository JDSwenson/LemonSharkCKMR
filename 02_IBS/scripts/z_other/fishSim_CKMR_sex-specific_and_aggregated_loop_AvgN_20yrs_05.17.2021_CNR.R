...
t_end = 59 #Last year we take samples
...
batchSize = 1 #average size of brood -- assumes Poisson distribution for fecundity (which is default for altMate). Average fecundity (6) divided by 2 for skipped-breeding
firstBreed <- 7 #If using same knife-edge maturity for males and females
...
maxAge <- 19 #max age of lemon sharks
Surv_age <- rep(adult.surv, maxAge) #Set survival for each age to 85%
Surv_age[1] <- YOY.surv
Surv_age[2:5] <- Juv.surv
...
maxClutch <- 2 #Maximum number of offspring one individual can possibly produce
mat_type <- "ageSex"
...
####Sex-specific####
Surv <- adult.surv
max_age = maxAge #max age of cownose rays
m_adult_age <- c(firstBreed:max_age) #Set ages at which males and females are mature.
f_adult_age <- c(firstBreed:max_age)
...
#check_growthrate(mateType = "ageSex", femaleCurve = femaleCurve, mortType = "age", ageMort = ageMort, batchSize = batchSize, firstBreed = firstBreed, maxClutch = maxClutch)
...
for (y in 1:length(sim_yrs)) {
  indiv <- altMate(indiv, batchSize = batchSize, fecundityDist = "binomial", year = y, type = mat_type, maxClutch = maxClutch, singlePaternity = FALSE, maleCurve = maleCurve, femaleCurve = femaleCurve, firstBreed = firstBreed)
...
results <- results %>% 
  ...
  write.table(results, file = paste0("~/R/R_working_dir/LemonSharkCKMR_GitHub/02_IBS/fishSim_model_validation/Cownose_Rays/results/length_of_sampling/fishSim_AvgN_", n_samples_total, "_samples_sex-specific_and_aggregated_loop_20_yrs_CNR_05.17.2021.csv"), sep=",", dec=".", qmethod="double", row.names=FALSE)
}
