...
sample.vec <- c(500, 1000, 1500) #vector to sample over per year
...
for(yr in 2:length(loopy.list)){
  ...
  #try({  
    ...
    jags_file = paste0("~/R/working_directory/LemonSharkCKMR/02_IBS/Dovi_IBS_model_validation/Lemon_sharks/results/basic_bayesian/models/HS_neutralGrowth_estSurvLam_iteration_", iter, ".txt")
...
##### Compile and report results ####
    ...
    lam_min <- min(adult.lambda[min_cohort:n_yrs])
    lam_max <- max(adult.lambda[min_cohort:n_yrs])
    
    #Create dataframe of estimates and truth
    estimates <- model_summary %>% rownames_to_column(var = "parameter") %>% 
      mutate(min = c(Mom_min, Dad_min, surv_min, lam_min), max = c(Mom_max, Dad_max, surv_max, lam_max), truth = c(Mom_truth, Dad_truth, surv_truth, mean.adult.lambda)) %>%
      as_tibble() %>% 
    ...
    metrics <- cbind(c(sum(mom_comps[,3]), sum(dad_comps[,3]), rep(sum(mom_comps[,3]) + sum(dad_comps[3]), times = 2)), 
...
#  }) # end try clause    
  ...
  write.table(results, file = paste0("~/R/working_directory/temp_results/neutralGrowth_estSurvLam_iteration", iter, ".csv"), sep=",", dec=".", qmethod="double", row.names=FALSE)
