...
YOY.survival <- .68 # young of year survival
...
s <- sample(c(1:100000000), size = iterations)
...
mean(pop.size$Lambda[30:90], na.rm=T) #Mean Lambda

hist(lambda[30:90])

####---------Checking population parameters-------####
nrow(YOY.df)/length(mothers) #Average fecundity for last year; remember they're skipped breeding
...
f.init <- pop.size$Female.adult.pop[min_cohort]
m.init <- pop.size$Male.adult.pop[min_cohort]
...
write.table(results, file = paste0("~/R/R_working_dir/LemonSharkCKMR_GitHub/02_IBS/Dovi_IBS_model_validation/Lemon_sharks/results/length_of_sampling/six_year_sampling/Dovi_AvgN_", total_samples, "_samples_05.26.2021_noskip.csv"), sep=",", dec=".", qmethod="double", row.names=FALSE)
