...
#  theme_ridges() + 
  ...
  scale_y_discrete(expand = expansion(mult = c(0.01, .05))) +
    ...
    theme(axis.text = element_text(size = 12),
      ...
      ggsave("Figure_3a_2022.11.29.tiff", plot = last_plot(), device = "tiff", dpi = 300, path = "G://My Drive/Personal_Drive/R/CKMR/manuscript.figs/raw/")
...
(figS2 <- obj2_lam.results_4viz %>% 
  ...
  facet_grid(rows = vars(degree), cols = vars(sampling.scheme)) +
...
ggsave("Figure_S2_2022.11.29.tiff", plot = last_plot(), device = "tiff", dpi = 300, path = "G://My Drive/Personal_Drive/R/CKMR/manuscript.figs/raw/")
...
obj3_results_4plot <- obj3_results %>% mutate(breeding.schedule = ifelse(mating.periodicity == 2, "biennial", "triennial")) %>% 
  mutate(popsim.psi = factor(popsim.psi, levels = c(1, 0.9, 0.75, 0.5))) %>% 
  mutate(sampling.scheme = ifelse(sampling.scheme == "sample ALL ages", "sample all ages", sampling.scheme))

(fig4a <- obj3_results_4plot %>% dplyr::filter(parameter == "Nf" | parameter == "Nm", breeding.schedule == "biennial", est.yr == 85) %>% 
  ...
  ggtitle("Intermittent breeding")
      ...
      ggsave("Figure_4a_2022.11.29.tiff", plot = last_plot(), device = "tiff", dpi = 300, path = "G://My Drive/Personal_Drive/R/CKMR/manuscript.figs/raw/")
...
obj4_results_4plot <- obj4_results %>% mutate(sampling.scheme = ifelse(sampling.scheme == "sample ALL ages", "sample all ages", sampling.scheme)) %>% 
  ...
  ggtitle("Aging error")
      ...
      ggsave("Figure_4b_2022.11.29.tiff", plot = last_plot(), device = "tiff", dpi = 300, path = "G://My Drive/Personal_Drive/R/CKMR/manuscript.figs/raw/")
...
(figS3a <- obj3_results_4plot %>% dplyr::filter(parameter %in% c("survival", "lambda"), breeding.schedule == "biennial", est.yr == 85) %>% 
  ...
  guides(fill=guide_legend(title="Model"))
      ...
      ggsave("Figure_S3a_2022.11.29.tiff", plot = last_plot(), device = "tiff", dpi = 300, path = "G://My Drive/Personal_Drive/R/CKMR/manuscript.figs/raw/")
  ...
  theme(axis.text = element_text(size = 12),
      ...
      ggsave("Figure_S3b_2022.11.29.tiff", plot = last_plot(), device = "tiff", dpi = 300, path = "G://My Drive/Personal_Drive/R/CKMR/manuscript.figs/raw/")
...
```{r objective 5 viz}
ls_data <- read_csv(file = "G://My Drive/Personal_Drive/R/CKMR/Objective.5_lemon_shark_data/Model.results/CKMR_results_2022.12.15_Allsamples.csv")

ls_data %>% dplyr::filter(years_sampled == 3, parameter == "Nf") %>% 
  dplyr::slice_max(Q50)

fig5a <- ls_data %>% dplyr::filter(max.year < 2016) %>% 
  dplyr::filter(years_sampled %in% c(3, 4, 5, 6, 7, 8), parameter == "Nf") %>%
...
ggsave("Figure_5a_2022.12.01.tiff", plot = last_plot(), device = "tiff", dpi = 300, path = "G://My Drive/Personal_Drive/R/CKMR/manuscript.figs/raw/")

#-----------------------Down sampled-----------------------------------
ls_data.down <- read_csv(file = "G://My Drive/Personal_Drive/R/CKMR/Objective.5_lemon_shark_data/Model.results/CKMR_results_2022.12.01_downsample.csv")

ls_data.down %>% dplyr::filter(years_sampled == 2, parameter == "Nf") %>% 
  dplyr::slice_max(Q50)

fig5b <- ls_data.down %>% dplyr::filter(years_sampled %in% c(2, 3, 4, 5, 6, 7), parameter == "Nf") %>% dplyr::mutate(years_sampled_actual = years_sampled + 1) %>% 
  mutate(years.lab = paste0(years_sampled_actual, " years sampled"),
         year.est = ceiling(min.year + years_sampled/2)) %>% 
  ggplot(aes(x = year.est, y = Q50)) + 
  geom_point() + 
  geom_smooth(method = "gam") +
...
#----------------------Figure S7----------------------------
figS7a <- ls_data %>% dplyr::filter(years_sampled %in% c(2, 3, 4, 5, 6, 7), parameter == "survival") %>% dplyr::mutate(years_sampled_actual = years_sampled + 1) %>%
  mutate(years.lab = paste0(years_sampled_actual, " years sampled"),
         year.est = ceiling(min.year + years_sampled/2)) %>% 
  ggplot(aes(x = year.est, y = Q50)) + 
  geom_point() + 
  ...
  facet_wrap(~years.lab) +
  theme_bw() +
  labs(x = "Estimation year", y = "Adult survival") +
  ggtitle("Full dataset") +
  theme(axis.text = element_text(size = 15),
        axis.title = element_text(size = 20),
        strip.text.x = element_text(size = 15),
...
ggsave("Figure_S7a_2022.12.01.tiff", plot = last_plot(), device = "tiff", dpi = 300, path = "G://My Drive/Personal_Drive/R/CKMR/manuscript.figs/raw/")

figS7b <- ls_data.down %>% dplyr::filter(years_sampled %in% c(2, 3, 4, 5, 6, 7), parameter == "survival") %>% dplyr::mutate(years_sampled_actual = years_sampled + 1) %>%
  mutate(years.lab = paste0(years_sampled_actual, " years sampled"),
         year.est = ceiling(min.year + years_sampled/2)) %>% 
  ggplot(aes(x = year.est, y = Q50)) + 
  geom_point() + 
...
ggsave("Figure_S7b_2022.12.01.tiff", plot = last_plot(), device = "tiff", dpi = 300, path = "G://My Drive/Personal_Drive/R/CKMR/manuscript.figs/raw/")


#Make table of full dataset and downsampled dataset results
table4.full <- ls_data %>% dplyr::filter(min.year == 1993 & max.year == 2015) %>% 
  mutate(samples = table4.down$num.samps_all,
         dataset = "Full")

table4.full <- table4.full %>% dplyr::select(parameter,
                              median = Q50,
                              HPD2.5,
                              HPD97.5,
                              sd = sd,
...
table4.down <- ls_data.down %>% dplyr::filter(min.year == 1993 & max.year == 2015) %>% 
  mutate(dataset = "Downsampled")

table4.down <- table4.down %>% dplyr::select(parameter,
                              median = Q50,
                              HPD2.5,
                              HPD97.5,
                              sd = sd,
                              samples = num.samps_down,
                              mom_HSPs = mom_HSPs.down,
                              dad_HSPs = dad_HSPs,
                              dataset)

(table4.all <- table4.full %>% bind_rows(table4.down))
```
