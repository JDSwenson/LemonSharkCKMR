...
(fig4a <- obj3_results_4plot %>% dplyr::filter(parameter == "Nf" | parameter == "Nm", breeding.schedule == "biennial", est.yr == 85) %>% 
  ...
  ggtitle("a) Intermittent breeding")
      ...
      ggsave("Figure_4a_2022.12.16.tiff", plot = last_plot(), device = "tiff", dpi = 300, path = "G://My Drive/Personal_Drive/R/CKMR/manuscript.figs/raw/", width = 14, height = 6, units = "in")
...
(figS3a <- obj3_results_4plot %>% dplyr::filter(parameter %in% c("survival", "lambda"), breeding.schedule == "biennial", est.yr == 85) %>% 
  ...
  guides(fill=guide_legend(title="Model"))
      ...
      ggsave("Figure_S3a_2022.12.16.tiff", plot = last_plot(), device = "tiff", dpi = 300, path = "G://My Drive/Personal_Drive/R/CKMR/manuscript.figs/raw/", width = 14, height = 7.35, units = "in")
...
```{r objective 5 viz}
ls_data <- read_csv(file = "G://My Drive/Personal_Drive/R/CKMR/Objective.5_lemon_shark_data/Model.results/CKMR_results_2022.12.16_FullLitter_wLambda.csv") %>% 
  mutate(num.samps = num.samps_all) %>% 
  dplyr::select(-c(num.samps_all))

#One individual per litter
# ls_data <- read_csv(file = "G://My Drive/Personal_Drive/R/CKMR/Objective.5_lemon_shark_data/Model.results/CKMR_results_2022.12.15_OneIndvPerLitter_wLambda.csv") %>% 
#   mutate(num.samps = num.samps_all) %>% 
#   dplyr::select(-c(num.samps_all))


ls_data.down <- read_csv(file = "G://My Drive/Personal_Drive/R/CKMR/Objective.5_lemon_shark_data/Model.results/CKMR_results_2022.12.16_FullLitter_downsample_wLambda.csv") %>% 
...
ls_data.full4viz <- ls_data %>% dplyr::filter(estimation_year > 1996) %>% 
  mutate(Dataset = "Full")
ls_data.down4viz <- ls_data.down %>% dplyr::filter(estimation_year > 1996) %>% 
  mutate(Dataset = "Downsampled")

ls_data4viz <- bind_rows(ls_data.full4viz, ls_data.down4viz) %>% 
  mutate(Dataset = factor(Dataset, levels = c("Full", "Downsampled")))
...
fig5a <- ls_data4viz %>% dplyr::filter(max.year < 2016, min.year == 1993) %>% 
  dplyr::filter(parameter == "Nf") %>%
...
#-----------------------Fig5b: Survival------------------------------------#
fig5b <- ls_data4viz %>% dplyr::filter(max.year < 2016, min.year == 1993) %>% 
...
ggsave("Figure_5_2022.12.16.tiff", plot = last_plot(), device = "tiff", dpi = 300, path = "G://My Drive/Personal_Drive/R/CKMR/manuscript.figs/raw/", width = 12, height = 6, units = "in")
...
table4 <- bind_rows(ls_data.full4viz, ls_data.down4viz) %>%
  dplyr::filter(parameter == "Nf") %>% 
...
write_csv(table4, file = "G://My Drive/Personal_Drive/R/CKMR/manuscript.figs/tables/table4.csv")
```
