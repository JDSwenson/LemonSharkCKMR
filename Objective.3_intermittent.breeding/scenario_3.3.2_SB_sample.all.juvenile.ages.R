...
script_name <- "scenario_3.3.2_SB_sample.all.juvenile.ages.R" #Copy name of script here
...
purpose <- "scenario_3.3.2_SB_sample.all.juvenile.ages" #For naming output files
...
#If we only want to use one type of relationship, we'll filter here
    ...
    #Define JAGS data and model, and run the MCMC engine
      set.seed(rseed)
    source("Objective.3_intermittent.breeding/functions/scenario_3.3.2_run.JAGS_HS.only.R")
