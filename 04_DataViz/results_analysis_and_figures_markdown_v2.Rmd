...
#What percent of convergence failures come from scenario2.3?
results.raw %>% group_by(scenario) %>% 
  summarize(failed = sum(Rhat > 1.01), 
            passed = sum(Rhat <= 1.01)) %>% 
  mutate(percent_failed = (failed/(failed+passed))*100) %>% 
...
ggsave("Figure_2ALL_2023.09.12.tiff", plot = last_plot(), device = "tiff", dpi = 500, path = "G://My Drive/Personal_Drive/R/CKMR/manuscript.figs/raw/", width = 5000, height = 3000, units = "px")
...
p_aunt.niece.count <- obj1_results %>% dplyr::filter(aunt.label != "No aunt/niece",
  ...
  facet_grid(rows = vars(sample.prop.lab), cols = vars(sampling.scheme)) + 
  theme_bw() +
#  xlim(-100, 100) +
  scale_fill_manual(values = cp) +
  theme_bw() +
...
ggsave("SuppFigure1_2023.09.12.tiff", plot = last_plot(), device = "tiff", dpi = 300, path = "G://My Drive/Personal_Drive/R/CKMR/manuscript.figs/raw/", width = 3694, height = 2200, units = "px")
...
#Read in truth values -- need to recalculate for naive scenarios
...
#Update truth values for naive scenarios for changing population simulations
...
obj2_results <- obj2_results %>% dplyr::filter(!scenario %in% c("scenario_2.1.1", "scenario_2.1.2", "scenario_2.1.3")) %>% 
  ...
  mutate(model = ifelse(scenario %in% naive_scenarios, "naive", "adapted")) %>% 
...
#Calculate percent of lambda estimates that were correct in direction
...
obj2_results %>% dplyr::count(model)


#-------------------Figure 3-------------------------#
#DID NOT SAVE 09/12/2023
(fig3.1 <- obj2_results %>% dplyr::filter(parameter %in% c("Nf"),
                                          population.growth != "stable",
                                          scenario %in% c(no_lambda_scenarios, "scenario_2.2.1", "scenario_2.2.2", "scenario_2.3.3")) %>% #Added scenario filter so we don't have one density plot per lambda prior
  ggplot(aes(x = relative_bias,
...
#ggsave("Figure_3.1_2023.09.12.tiff", plot = last_plot(), device = "tiff", dpi = 300, width = 15, height = 12, path = "G://My Drive/Personal_Drive/R/CKMR/manuscript.figs/raw/")
...
#LAMBDA AND SURVIVAL PLOT
obj2_results %>% dplyr::filter(parameter %in% c("lambda", "survival"),
                                          !scenario %in% c(no_lambda_scenarios, "scenario_2.2.3", "scenario_2.3.1", "scenario_2.3.2", "scenario_2.3.4")) %>% #Added scenario filter so we don't have one density plot per lambda prior
  ggplot(aes(x = relative_bias,
             y = estimation.sim.label, 
             fill = parameter)) + #Changed TO model from model.label
...
#  theme_ridges() + 
  ...
  scale_y_discrete(expand = expansion(mult = c(0.01, .05))) +
    ...
    theme(axis.text = element_text(size = 12),
        ...
        legend.text = element_text(size = 15),
        legend.title = element_text(size = 20)) +
  ggtitle(expression(paste("Estimates of ", lambda, " and ", phi)))
