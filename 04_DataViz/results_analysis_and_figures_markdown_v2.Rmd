...
results.raw %>% group_by(scenario) %>% 
  ...
  summarize(percent.failed = sum(failed)/((sum(failed) + sum(passed))))
...
p.full.sibs <- obj1_results %>% dplyr::filter(scenario == "scenario_1.2.1", parameter == "Nf") %>% 
  ...
  theme_bw() +
#  xlim(-100, 100) +
  scale_fill_manual(values = cp, labels = c("different cohort", "all")) +
  theme_bw() +
  theme(legend.title = element_blank(),
        legend.position = "bottom")
...
ggsave("SuppFig_FullSibs_2023.10.02.tiff", plot = last_plot(), device = "tiff", dpi = 300, path = "G://MyDrive/Personal_Drive/R/CKMR/manuscript.figs/raw/", width = 3694, height = 2200, units = "px")
```
