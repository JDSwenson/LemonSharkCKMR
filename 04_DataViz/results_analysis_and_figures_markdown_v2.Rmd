...
#-------------------Objective 4 Density plots of relative bias-----------------------#
      (fig4b <- obj4_results %>% dplyr::filter(parameter == "Nf" | parameter == "survival", estimation.sim.label == "present") %>% 
...
ls_dataTrend_raw <- readRDS(file = paste0(results_location, "/lemon_shark_sims/lemon_shark_time_series_sims_popSize_Conn3_temp"))
...
ls_dataTrend.df <- ls_dataTrend_raw %>% dplyr::group_by(year) %>% 
  ...
  mutate(Nf.trend.trans = Nf.trend * 4,
         Nm.trend.trans = Nm.trend * 4,
         Nfb.trend.trans = Nfb.trend * 4) %>% 
...
ls_Nfb_boxplot <- ls_dataSim %>% dplyr::filter(parameter %in% c("Nfbt")) %>% 
  ggplot() +
  geom_boxplot(aes(x=factor(estimation.year), fill = time_window, y=relative_bias), notch = TRUE, outlier.alpha = 0.1) +
  geom_hline(yintercept=0, col="blue", size=1.25, linetype = "dashed") +
  labs(y="Relative bias", x = "Year") +
    geom_path(data = ls_dataTrend.df, aes(x = factor(year), y = Nfb.trend.trans, color = type, group = 1), linewidth = 1.25, linetype = "solid") +
...
ls_survival_boxplot <- ls_dataSim %>% dplyr::filter(parameter %in% c("survival")) %>% 
  ...
  geom_hline(yintercept=0, col="blue", size=1.25, linetype = "dashed") +
  labs(y="Relative bias", x = "Year") +
    geom_path(data = ls_dataTrend.df, aes(x = factor(year), y = Nfb.trend, group = 1), linewidth = 1.25, linetype = "solid", colour = "darkslateblue") +
