...
results <- map_dfr(results.files, read_csv, guess_max = 18000) #Have to specify guess_max or else the function will guess column type using the first 1,000 columns and assume the POP columns are logical (bc the first several thousand are NA)

naive_scenarios <- c("scenario_2.1.1", "scenario_2.1.2", "scenario_2.1.3", "scenario_3.1.1", "scenario_3.2.1", "scenario_3.3.1", "scenario_3.4.1", "scenario_3.5.1", "scenario_3.6.1")

objective1 <- "scenario_1_model.validation"
objective2 <- c("scenario_2.1.1", "scenario_2.1.2", "scenario_2.1.3", "scenario_2.2.1", "scenario_2.2.2", "scenario_2.2.3", "scenario_2.3.1", "scenario_2.3.2", "scenario_2.3.3")
objective3 <- c("scenario_3.1.1", "scenario_3.1.2", "scenario_3.2.1", "scenario_3.2.2", "scenario_3.3.1", "scenario_3.3.2", "scenario_3.4.1", "scenario_3.4.2", "scenario_3.5.1", "scenario_3.5.2", "scenario_3.6.1", "scenario_3.6.2")
objective4 <- c("scenario_4.1", "scenario_4.2", "scenario_4.3", "scenario_4.4", "scenario_4.5", "scenario_4.6")

results <- results %>% mutate(model = ifelse(scenario %in% naive_scenarios, "naive", "adapted")) %>% 
...
results %>% dplyr::count(sampling.scheme)
results %>% dplyr::count(estimation.year)
results %>% dplyr::count(sample.prop)
results %>% dplyr::count(sampling.scheme)
results %>% dplyr::count(estimation.sim)
results %>% dplyr::count(is.na(relative_bias))
results %>% dplyr::count(in_interval)
results %>% dplyr::count(scenario) %>% View()
results %>% dplyr::filter(scenario == "scenario_1_model.validation") %>% 
  count(estimation.sim)
results %>% dplyr::count(scenario)
results %>% dplyr::count(objective)
results %>% dplyr::count(iteration) %>% View()
results %>% dplyr::count(seed) %>% View()
...
results %>% group_by(model, estimation.sim) %>% 
  summarize(median(relative_bias))
```
