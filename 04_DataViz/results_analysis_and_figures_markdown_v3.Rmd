...
objective2 <- c("scenario_2.1.1", "scenario_2.1.2", "scenario_2.1.3", "scenario_2.1.4", "scenario_2.4.1", "scenario_2.4.2", "scenario_2.4.3", "scenario_2.4.4")
...
ggsave("SuppFig_FullSibs_2023.10.05.tiff", plot = last_plot(), device = "tiff", dpi = 300, path = "G://My Drive/Personal_Drive/R/CKMR/manuscript.figs/raw", width = 3694, height = 2200, units = "px")
```
...
obj2_results <- obj2_results %>% mutate(parameter = ifelse(parameter == "Nft", "Nf",
                                           ifelse(parameter == "Nmt", "Nm",
                                                  ifelse(parameter == "Nf0" & scenario == "scenario_2.4.3", "Nf", parameter))))

#Check that Paul Conn's model registered appropriately
obj2_results %>% dplyr::filter(parameter == "survival", population.growth != "stable") %>% dplyr::count(model)
```
...
#ggsave("Figure_3a_2023.10.03.tiff", plot = last_plot(), device = "tiff", dpi = 300, width = 15, height = 12, path = "G://My Drive/Personal_Drive/R/CKMR/manuscript.figs/raw/")
...
obj2_results %>% dplyr::filter(parameter %in% c("lambda"),
                                          scenario %in% conn_scenarios) %>% #Added scenario filter so we only have one density plot for each situation. Want the situations to match up with Figure 3.
...
ggsave("Fig3d_lambda_Obj2_2023.10.06.tiff", plot = last_plot(), device = "tiff", dpi = 300, width = 12, height = 8, path = "G://My Drive/Personal_Drive/R/CKMR/manuscript.figs/raw/")
...
ggsave("SuppFig2b_survival_Obj2_2023.10.06.tiff", plot = last_plot(), device = "tiff", dpi = 300, width = 12, height = 8, path = "G://My Drive/Personal_Drive/R/CKMR/manuscript.figs/raw/")
...
cc.table <- crossCorr.all %>% as_tibble() %>% 
  ...
  dplyr::select(parameter, survival, lambda, T0, sampling.scheme, population.growth) 
...
ggsave("SuppFig_variablepsi_Obj3_2023.10.06.tiff", plot = last_plot(), device = "tiff", dpi = 300, width = 14, height = 6, units = "in", path = "G://My Drive/Personal_Drive/R/CKMR/manuscript.figs/raw/")
...
ggsave("Figure_5_aging.error_2023.10.06.tiff", plot = last_plot(), device = "tiff", dpi = 300, width = 12, height = 8, path = "G://My Drive/Personal_Drive/R/CKMR/manuscript.figs/raw/")
...
(lemonSim.files <- list.files(path = paste0(results_location, "/lemon_shark_sims/"),
                            recursive = FALSE,
                            pattern = "lemon_shark.*\\.csv$",
                            full.names = TRUE))

ls_dataSim <- map_dfr(lemonSim.files, read_csv) 
#Don't need the below anymore -- that was for the first run of this
# %>% 
...
ls_dataTrend_raw <- readRDS(file = paste0(results_location, "/lemon_shark_sims/lemon_shark_time_series_sims_popSize_Conn3"))
...
ls_estimatesSim <- ls_dataSim %>% pivot_longer(cols = c(Q50, all.truth), names_to = "type", values_to = "estimate") %>% 
  dplyr::filter(type == "Q50",
                iteration == iter) %>% 
  mutate(time_window = ifelse(time_window == "all samples" ,"all available samples", time_window))
...
ls_truthSim <- ls_dataSim %>% pivot_longer(cols = c(Q50, all.truth), names_to = "type", values_to = "estimate") %>% 
  ...
  mutate(type = ifelse(parameter %in% c("Nft", "Nf0", "Nmt", "Nm0", "Nfbt", "Nfb0", "psi"), "truth",
                       ifelse(time_window == "all samples", "truth: all samples",
                       ifelse(time_window == "five year block", "truth: five year block",
                              ifelse(time_window == "three year block", "truth: three year block", NA)))))
...
ls_data <- read_csv(file = "G://My Drive/Personal_Drive/R/CKMR/output_peer_review/Model.results/lemon_shark_sims/LS_data_ConnModel_2023.10.05.csv") %>% 
  #mutate(num.samps = num.samps_all) %>% 
  #dplyr::select(-c(num.samps_all)) %>% 
  dplyr::filter(estimation.year >= 1997)

ls_data.Down <- read_csv(file = "G://My Drive/Personal_Drive/R/CKMR/output_peer_review/Model.results/lemon_shark_sims/LS_data_ConnModel_Down_2023.10.05.csv") %>% 
...
ls_Nfb_boxplot <- ls_dataSim %>% dplyr::filter(parameter %in% c("Nfbt")) %>% 
  ...
  scale_y_continuous(limits = c(-50, 200), 
                     breaks = c(-50, -25, 0, 25, 50, 75, 100, 125, 150, 175, 200)) +
...
#ggsave("LS_sim_Nfb_boxplot_2023.10.04.tiff", plot = last_plot(), device = "tiff", dpi = 300, width = 15, height = 8, path = "G://My Drive/Personal_Drive/R/CKMR/manuscript.figs/raw/")
...
geom_path(data = ls_dataTrend.df, #Include if we want a trend line of the average realized abundance
          ...
          size = 1.25,
          linetype = "solid") +
    scale_color_manual(values = "darkslateblue", name = element_blank()) +
 ...
 #            scales = "free") +
  ...
  geom_vline(xintercept=7, col="darkslategrey", size=1.25, linetype = "twodash") +
    geom_vline(xintercept=12, col="lightseagreen", size=1.25, linetype = "twodash") +
  guides(fill=guide_legend(title="Sample window")) +
  ggtitle(expression(paste("b) ", lambda)))

LS.sim_boxplots <- ggarrange(ls_Nfb_boxplot, ls_lambda_boxplot, common.legend = FALSE, nrow = 2, legend = "right")
...
ggsave("LS_sim_boxplots_2023.10.04.tiff", plot = last_plot(), device = "tiff", dpi = 300, width = 15, height = 8, path = "G://My Drive/Personal_Drive/R/CKMR/manuscript.figs/raw/")
...
ggsave("LS_sim_Nfb_singleTrend_2023.10.04.tiff", plot = last_plot(), device = "tiff", dpi = 300, width = 15, height = 8, path = "G://My Drive/Personal_Drive/R/CKMR/manuscript.figs/raw/")
...
ggsave("SuppFig_survival_boxplot_2023.10.04.tiff", plot = last_plot(), device = "tiff", dpi = 300, width = 15, height = 8, path = "G://My Drive/Personal_Drive/R/CKMR/manuscript.figs/raw/")
...
ggsave("LS_data_Nfb_trend_2023.10.05.tiff", plot = last_plot(), device = "tiff", dpi = 300, path = "G://My Drive/Personal_Drive/R/CKMR/manuscript.figs/raw/")
...
ggsave("LS_data_survival_2023.10.05.tiff", plot = last_plot(), device = "tiff", dpi = 300, path = "G://My Drive/Personal_Drive/R/CKMR/manuscript.figs/raw/")
...
ggsave("SuppFig_LS.Down.tiff", plot = last_plot(), device = "tiff", dpi = 300, path = "G://My Drive/Personal_Drive/R/CKMR/manuscript.figs/raw/", width = 12, height = 6, units = "in")
