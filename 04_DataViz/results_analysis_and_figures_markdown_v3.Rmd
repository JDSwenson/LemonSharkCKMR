...
naive.filtered_scenarios <- c("scenario_3.1.3", "scenario_3.5.3", "scenario_3.6.3")
...
objective3 <- c("scenario_3.1.1", "scenario_3.1.2", "scenario_3.1.3", "scenario_3.2.1", "scenario_3.2.2", "scenario_3.3.1", "scenario_3.3.2", "scenario_3.4.1", "scenario_3.4.2", "scenario_3.5.1", "scenario_3.5.2", "scenario_3.5.3", "scenario_3.6.1", "scenario_3.6.2", "scenario_3.6.3", "scenario_3.7.1", "scenario_3.7.2")
...
##################### Objective 3 ############################
#---------------------Prep results----------------------------
#Group scenarios by tests
psi1_scenarios <- c("scenario_3.1.1", "scenario_3.1.2", "scenario_3.1.3", "scenario_3.5.1", "scenario_3.5.2", "scenario_3.5.3", "scenario_3.7.1", "scenario_3.7.2")
psi1stoch_scenarios <- c("scenario_3.6.1", "scenario_3.6.2", "scenario_3.6.3")
...
biennial_scenarios <- c("scenario_3.1.1", "scenario_3.1.2", "scenario_3.1.3", "scenario_3.2.1", "scenario_3.2.2", "scenario_3.3.1", "scenario_3.3.2", "scenario_3.4.1", "scenario_3.4.2")
biennial_stoch_scenarios <- c("scenario_3.6.1", "scenario_3.6.2", "scenario_3.6.3")
triennial_scenarios <- c("scenario_3.5.1", "scenario_3.5.2", "scenario_3.5.3")
no.offcycle_scenarios <- c("scenario_3.1.3", "scenario_3.5.3", "scenario_3.6.3")

#Subset all results for Obj3 results
obj3_results <- results %>% dplyr::filter(objective == "objective3") 

#Re-calculate estimates as Q50 * 2 and truth as all.truth
scenario_3.1.3_trueNf <- obj3_results %>% dplyr::filter(scenario == "scenario_3.1.1", parameter == "Nf") %>% 
  dplyr::select(parameter, iteration, all.truth) %>% 
  distinct(.keep_all = TRUE) %>% 
  mutate(scenario = "scenario_3.1.3")
  
scenario_3.5.3_trueNf <- obj3_results %>% dplyr::filter(scenario == "scenario_3.5.1", parameter == "Nf") %>% 
  dplyr::select(parameter, iteration, all.truth) %>% 
  distinct(.keep_all = TRUE) %>% 
  mutate(scenario = "scenario_3.5.3")

scenario_3.6.3_trueNf <- obj3_results %>% dplyr::filter(scenario == "scenario_3.6.1", parameter == "Nf") %>% 
  dplyr::select(parameter, iteration, all.truth) %>% 
  distinct(.keep_all = TRUE) %>% 
  mutate(scenario = "scenario_3.6.3")

offcycle_totalNf_truths <- bind_rows(scenario_3.1.3_trueNf, scenario_3.5.3_trueNf, scenario_3.6.3_trueNf) %>% 
  mutate(totalNf = all.truth) %>% 
  dplyr::select(-all.truth)

obj3_results <- obj3_results %>% left_join(offcycle_totalNf_truths, by = c("parameter", "iteration", "scenario")) %>% 
  mutate(all.truth = ifelse(scenario %in% no.offcycle_scenarios & parameter == "Nf", totalNf, all.truth)) %>% 
  ...
  mutate(Q50 = ifelse(scenario %in% no.offcycle_scenarios & parameter == "Nf" & breeding.schedule != "triennial", Q50*2,
                      ifelse(scenario %in% no.offcycle_scenarios & parameter == "Nf" & breeding.schedule == "triennial", Q50*3, Q50))) %>% 
  mutate(relative_bias = round(((Q50 - all.truth)/all.truth)*100, 1)) %>% 
  ...
  mutate(psi.label = factor(psi.label, levels = c("1", "1*", "0.90", "0.75", "0.50", "0"))) %>% 
    mutate(model.label = ifelse(scenario %in% naive_scenarios, "naive", 
                              ifelse(scenario %in% naive.filtered_scenarios, "naive - filtered", "adapted"))) %>% 
  mutate(model.label = factor(model.label, levels = c("naive", "naive - filtered", "adapted"))) %>%
  mutate(sampling.scheme.label = ifelse(sampling.scheme == "sample.ALL.ages", "sample all ages", 
                                      ifelse(sampling.scheme == "sample.all.juvenile.ages", "sample juveniles", 
                                             ifelse(sampling.scheme == "target.YOY", "target YOY", NA))))# %>% 
    #mutate(estimation.sim.label = factor(estimation.sim, levels = c("T0-10", "T0", "present-5", "present")))
...
#Will throw an error if run more than once
...
#Calculate naive "truth" for psi as the proportion of positive comparisons that come from on-cycle mothers -- view for specific scenarios
#All comparisons
total.HSPs <- mom.comps.obj3 %>% dplyr::filter(scenario == "scenario_3.6.2") %>% 
  ...
  summarize(mean.yes = mean(all.yes),
            mean.all = mean(all.all))

#All comparisons by BI
on.HSPs <- mom.comps.obj3 %>% dplyr::filter(scenario == "scenario_3.6.2") %>% 
  ...
  summarize(mean.yes.BI = mean(all.yes.BI),
            mean.all.BI = mean(all.all.BI))

(psi.truth <- total.HSPs %>% left_join(on.HSPs, by = "sampling.scheme") %>% 
  mutate(psi_truth = mean.yes.BI/mean.yes,
         percent_total = mean.all.BI/mean.all))

#How many on vs off-cycle comparisons were there?
mom.comps.obj3 %>% dplyr::filter(scenario == "scenario_3.6.2") %>% 
  group_by(sampling.scheme, BI) %>% 
  summarize(all.yes = sum(yes),
            all.all = sum(all))

#Fix dataframe for temporary results (tests without off-cycle breeders)
# plot.3 <- obj3_results %>% mutate(relative_bias = round(((Q50 - all.truth)/all.truth)*100, 1)) %>% 
#   dplyr::filter(scenario %in% c(no.offcycle_scenarios, "scenario_3.1.1", "scenario_3.5.1", "scenario_3.6.1")) %>% 
#   mutate(plot.label = ifelse(scenario %in% no.offcycle_scenarios, "No off-cycle comps", "Include off-cycle comps"))
...
#   dplyr::select(-all.truth)
# 
# plot.4 <- plot.3 %>% left_join(offcycle_totalNf_truths, by = c("parameter", "iteration", "scenario")) %>% 
#   mutate(all.truth = ifelse(scenario %in% no.offcycle_scenarios & parameter == "Nf", totalNf, all.truth)) %>% 
#   mutate(Q50 = ifelse(scenario %in% no.offcycle_scenarios & parameter == "Nf" & breeding.schedule != "triennial", Q50*2,
#                       ifelse(scenario %in% no.offcycle_scenarios & parameter == "Nf" & breeding.schedule == "triennial", Q50*3, Q50))) %>% 
#   mutate(relative_bias = round(((Q50 - all.truth)/all.truth)*100, 1)) %>% 
...
```{r Quick plot for checking}
plot.4 %>% dplyr::filter(parameter %in% c("Nf", "Nm", "survival", "lambda")) %>% 
  ggplot(aes(x=factor(breeding.schedule, levels = c("triennial", "biennial", "biennial*")), fill = plot.label)) +
...
(fig4a <- obj3_results %>% dplyr::filter(parameter %in% c("Nf", "Nm", "survival"),
  ...
  facet_grid(cols = vars(sampling.scheme.label), rows = vars(factor(parameter, levels = c("Nf", "Nm", "survival"))), scales = "free") +
  theme_bw() +
  theme(axis.text = element_text(size = 12),
        axis.title = element_text(size = 20),
        strip.text.x = element_text(size=15),
...
ggsave("Figure_4_Obj3_2023.12.19.tiff", plot = last_plot(), device = "tiff", dpi = 300, width = 14, height = 6, units = "in", path = "G://My Drive/Personal_Drive/R/CKMR/manuscript.figs/raw/")
