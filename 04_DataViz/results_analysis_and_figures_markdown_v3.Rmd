...
#Add labels for naive and adapted; CHANGE Nft and Nmt to Nf and Nm
...
results.raw %>% dplyr::filter(parameter == "Nf" | parameter == "Nf0") %>% dplyr::count(sampling.scheme, estimation.sim, sample.prop, scenario) %>% 
  summarize(sum(n))
...
ggsave("SuppFig2_FullSibs_2023.10.05.tiff", plot = last_plot(), device = "tiff", dpi = 300, path = "G://My Drive/Personal_Drive/R/CKMR/manuscript.figs/raw", width = 3694, height = 2200, units = "px")
```
...
obj2_results <- obj2_results %>% dplyr::filter(!parameter %in% c("Nf0", "Nm0")) %>% 
  ...
  #Check on things
obj2_results %>% dplyr::count(estimation.sim, population.growth, parameter) %>% 
  print(n = 100)
...
ggsave("Figure_3a_2023.10.06.tiff", plot = last_plot(), device = "tiff", dpi = 300, width = 15, height = 12, path = "G://My Drive/Personal_Drive/R/CKMR/manuscript.figs/raw/")
...
#Save each plot for Obj2 individually and combine in Affinity Designer
#ggsave("SuppFig_age.dist_Obj2_2023.10.03.tiff", plot = last_plot(), device = "tiff", dpi = 300, width = 15, height = 6, path = "G://My Drive/Personal_Drive/R/CKMR/manuscript.figs/raw/")
cp <- brewer.pal(3, "Set2") #Choose palette + number of colors
...
ggsave("Fig3e_lambda_Obj2_2023.10.06.tiff", plot = last_plot(), device = "tiff", dpi = 300, width = 15, height = 6, path = "G://My Drive/Personal_Drive/R/CKMR/manuscript.figs/raw/")
...
#  facet_grid(cols = vars(sampling.scheme.label)) +
  ...
  scale_y_discrete(expand = expansion(mult = c(0.01, .05))) +
    theme_bw() +
  ggtitle("Stable population growth") +
...
#Save each plot for Obj2 individually and combine in Affinity Designer
#ggsave("SuppFig_StablePop_Nf_Obj2.tiff", plot = last_plot(), device = "tiff", dpi = 300, width = 12, height = 8, path = "G://My Drive/Personal_Drive/R/CKMR/manuscript.figs/raw/")
...
ggsave("SuppFig3b_survival_Obj2_2023.10.06.tiff", plot = last_plot(), device = "tiff", dpi = 300, width = 12, height = 5, path = "G://My Drive/Personal_Drive/R/CKMR/manuscript.figs/raw/")
...
ggsave("SuppFig_NmSurv_Obj2_2023.10.03.tiff", plot = last_plot(), device = "tiff", dpi = 500, path = "G://My Drive/Personal_Drive/R/CKMR/manuscript.figs/raw/", width = 14, height = 8)
...
#Save each plot for Obj2 individually and combine in Affinity Designer
ggsave("SuppFig8_t0vNtComp_2023.10.08.tiff", plot = last_plot(), device = "tiff", dpi = 300, width = 15, height = 12, path = "G://My Drive/Personal_Drive/R/CKMR/manuscript.figs/raw/")
```
...
(fig4a <- obj3_results %>% dplyr::filter(parameter %in% c("Nf", "Nm", "survival"),
  ...
  facet_grid(cols = vars(sampling.scheme.label), rows = vars(factor(parameter, levels = c("Nf", "Nm", "survival", "lambda"))), scales = "free") +
  theme_bw() +
  theme(axis.text = element_text(size = 12),
        axis.title = element_text(size = 20),
        strip.text.x = element_text(size=15),
...
ggsave("Figure_4_Obj3_2023.10.06.tiff", plot = last_plot(), device = "tiff", dpi = 300, width = 14, height = 6, units = "in", path = "G://My Drive/Personal_Drive/R/CKMR/manuscript.figs/raw/")
...
#-------------------Mixed breeding --------------------#  
(fig4_others <- obj3_results %>% dplyr::filter(!breeding.schedule %in% c("triennial"),
                                         estimation.year == 90,
...
ggsave("SuppFig4_variablepsi_Obj3_2023.10.06.tiff", plot = last_plot(), device = "tiff", dpi = 300, width = 14, height = 6, units = "in", path = "G://My Drive/Personal_Drive/R/CKMR/manuscript.figs/raw/")
...
# #-------------------Plot of psi bias-----------------------#
      (figS3_psi <- obj3_results %>% dplyr::filter(parameter %in% c("psi"),
                                         breeding.schedule != "triennial",
                                         estimation.year %in% c(90)) %>%  # May want to add a filter for year estimate: estimation.sim.label == present-5
  ggplot(aes(x=factor(psi.label), fill = model.label)) +
  geom_boxplot(aes(y=relative_bias), draw_quantiles = 0.5, notch = TRUE) +
  geom_hline(yintercept=0, col="blue", size=1.25, linetype = "dashed") +
  labs(x="Proportion biennial breeders", y="Relative bias") +
...
# 
# 
ggsave("SuppFig5_Obj3_psi_2023.10.07.tiff", plot = last_plot(), device = "tiff", dpi = 300, width = 12, height = 8, path = "G://My Drive/Personal_Drive/R/CKMR/manuscript.figs/raw/")
...
##################### Objective 4 ############################
#---------------------Prep results----------------------------
accurate.annual.scenario <- results.raw %>% dplyr::filter(scenario == "scenario_2.4.4") %>% #stable population growth with narrow prior
...
ggsave("Figure_5b_aging.error_2023.10.06.tiff", plot = last_plot(), device = "tiff", dpi = 300, width = 12, height = 8, path = "G://My Drive/Personal_Drive/R/CKMR/manuscript.figs/raw/")
...
#-------------------Lambda, Nm, and survival--------------------------
  ...
  ggsave("SuppFig_other.parameters_Obj4.tiff", plot = last_plot(), device = "tiff", dpi = 300, width = 12, height = 8, path = "G://My Drive/Personal_Drive/R/CKMR/manuscript.figs/raw/")
```
...
ls_dataSim_allSibs <- read_csv(file = paste0(results_location, "/lemon_shark_sims/lemon_shark_time_series_sims_results_Conn5_allSibs_temp.csv")) %>%
...
ls_dataSim <- ls_dataSim_allSibs

#----------------------Load population size for trajectory
ls_dataTrend_raw <- readRDS(file = paste0(results_location, "/lemon_shark_sims/lemon_shark_time_series_sims_popsize_Conn5_allSibs_temp"))

# #------------------Removing full siblings---------------------
# ls_dataSim_filterSibs <- read_csv(file = paste0(results_location, "/lemon_shark_sims/lemon_shark_time_series_sims_results_Conn5_filterSibs.csv")) %>%
#   mutate(time_window = ifelse(time_window == "all available samples" ,"all available samples",
#                               ifelse(time_window == "three year block", "three year window",
#                                      ifelse(time_window == "five year block", "five year window", NA))))

#ls_dataSim <- ls_dataSim_filterSibs

#----------------------Load population size for trajectory
ls_dataTrend_raw <- readRDS(file = paste0(results_location, "/lemon_shark_sims/lemon_shark_time_series_sims_popsize_Conn5_filterSibs"))
...
min.yr <- ls_dataSim %>% dplyr::filter(parameter == "Nfbt") %>% summarize(min.yr = min(estimation.year)) %>% 
  pull(min.yr)

ls_dataTrend.df <- ls_dataTrend_raw %>% dplyr::group_by(year) %>% 
...
ls_truthSim.abund <- ls_truthSim %>% dplyr::filter(parameter %in% c("Nft", "Nf0", "Nmt", "Nm0", "Nfbt"))
...
#--------------------All sibs -------------------------------
ls_data.allSibs <- read_csv(file = "G://My Drive/Personal_Drive/R/CKMR/output_peer_review/Model.results/lemon_shark_sims/LS_data_ConnModel_AllSibs_2023.10.08.csv") %>%
...
ls_data.Down.allSibs <- read_csv(file = "G://My Drive/Personal_Drive/R/CKMR/output_peer_review/Model.results/lemon_shark_sims/LS_data_ConnModel_AllSibsDown_2023.10.06.csv") %>%
...
ls_data <- ls_data.allSibs
ls_data.Down <- ls_data.Down.allSibs

#--------------------Filter sibs -------------------------------
 # ls_data.filterSibs <- read_csv(file = "G://My Drive/Personal_Drive/R/CKMR/output_peer_review/Model.results/lemon_shark_sims/LS_data_ConnModel_filterSibs_2023.10.05.csv") %>%
 #  #mutate(num.samps = num.samps_all) %>%
 #  #dplyr::select(-c(num.samps_all)) %>%
 #  dplyr::filter(estimation.year >= 1997) %>%
 #  mutate(time_window = ifelse(time_window == "all available samples" ,"all available samples",
 ...
 #                                     ifelse(time_window == "five year block", "five year window", NA))))
# 
# ls_data.Down.filterSibs <- read_csv(file = "G://My Drive/Personal_Drive/R/CKMR/output_peer_review/Model.results/lemon_shark_sims/LS_data_ConnModel_filterSibsDown_2023.10.05.csv") %>%
#   mutate(mom_HSPs = mom_HSPs.down,
#          num.samps = num.samps_down) %>%
#   dplyr::select(-c(mom_HSPs.down, num.samps_down, num.samps_all, mom_HSPS.all)) %>%
#   dplyr::filter(estimation_year >= 1997) %>%
...
#                                      ifelse(time_window == "five year block", "five year window", NA))))
# 
 #ls_data <- ls_data.filterSibs
# ls_data.Down <- ls_data.Down.filterSibs
# 
 ls_data.Down4viz <- ls_data.Down %>% group_by(time_window, estimation.year, parameter) %>% 
  summarize(mean_Q50 = mean(Q50),
            mean_HPD2.5 = mean(HPD2.5),
            mean_HPD97.5 = mean(HPD97.5))
# 
...
ls_Nfb_boxplot <- ls_dataSim %>% dplyr::filter(parameter %in% c("Nfbt")) %>% 
  ggplot() +
  ...
  geom_vline(xintercept=7, col="darkslategrey", size=1.25, linetype = "longdash") +
    geom_vline(xintercept=12, col="lightseagreen", size=1.25, linetype = "longdash") +
...
# ggsave("LS_sim_allSibs_Nfb_boxplot_2023.10.08.tiff", plot = last_plot(), device = "tiff", dpi = 300, width = 15, height = 8, path = "G://My Drive/Personal_Drive/R/CKMR/manuscript.figs/raw/")

#ggsave("LS_sim_filterSibs_Nfb_boxplot_2023.10.04.tiff", plot = last_plot(), device = "tiff", dpi = 300, width = 15, height = 8, path = "G://My Drive/Personal_Drive/R/CKMR/manuscript.figs/raw/")

#------- Box plot of relative bias for lambda ----------------#
ls_lambda_boxplot <- ls_dataSim %>% dplyr::filter(parameter %in% c("lambda")) %>% 
  ggplot() +
...
#Survival
ls_survival_boxplot <- ls_dataSim %>% dplyr::filter(parameter %in% c("survival")) %>% 
  ggplot() +
  geom_boxplot(aes(x=factor(estimation.year), fill = time_window, y=relative_bias),
               notch = TRUE, 
               outlier.alpha = 0.1) +
...
(LS.sim_boxplots <- ggarrange(ls_Nfb_boxplot, ls_lambda_boxplot, ls_survival_boxplot, common.legend = FALSE, nrow = 3, legend = "right"))

(LS.sim_boxplots.annotated <- annotate_figure(LS.sim_boxplots, left = text_grob("Relative Bias", rot = 90, size = 18, face = "bold", family = "sans")))
...
ggsave("Fig6_LS_sim_filterSibs_boxplots_2023.10.08.tiff", plot = last_plot(), device = "tiff", dpi = 300, width = 15, height = 10, path = "G://My Drive/Personal_Drive/R/CKMR/manuscript.figs/raw/")
...
ls_truthSim.abund.Plot <- ls_truthSim.abund %>% dplyr::filter(parameter %in% params,
                                                              iteration == iter)
...
fig5sim <- ls_estimatesSim %>% dplyr::filter(iteration == iter,
  ...
  geom_line(data = ls_truthSim.abund.Plot,
            aes(y = truth, 
                color = type),
            size = 1) +
...
#ggsave("Fig6_LS_data_allSibs_Nfb_trend_2023.10.08.tiff", plot = last_plot(), device = "tiff", dpi = 300, width = 15, height = 4, path = "G://My Drive/Personal_Drive/R/CKMR/manuscript.figs/raw/")

ggsave("Fig6_LS_data_filterSibs_Nfb_trend_2023.10.08.tiff", plot = last_plot(), device = "tiff", dpi = 300, width = 15, height = 4, path = "G://My Drive/Personal_Drive/R/CKMR/manuscript.figs/raw/")

#-----------------------Fig5b: Survival------------------------------------#
fig5.survival <- ls_data %>% 
...
#ggsave("SuppFig7_LS_data_allSibs_survival_2023.10.08.tiff", plot = last_plot(), device = "tiff", dpi = 300, path = "G://My Drive/Personal_Drive/R/CKMR/manuscript.figs/raw/")

ggsave("SuppFig7_LS_data_filterSibs_survival_2023.10.08.tiff", plot = last_plot(), device = "tiff", dpi = 300, path = "G://My Drive/Personal_Drive/R/CKMR/manuscript.figs/raw/")



#-----------------------Supplementary: Downsampled data (100 iterations)------------------------------------#
#Nfbt
p.LS.down_Nfbt <- ls_data.Down4viz %>% 
...
#Survival
p.LS.down_survival <- ls_data.Down4viz %>% 
...
(p.LS.Down <- ggarrange(p.LS.down_Nfbt, p.LS.down_survival, common.legend = TRUE, nrow = 2, legend = "right"))


#ggarrange(fig5a, fig5b, common.legend = TRUE, legend = "bottom")

ggsave("SuppFig7_LSData_allSibs_Down.tiff", plot = last_plot(), device = "tiff", dpi = 300, path = "G://My Drive/Personal_Drive/R/CKMR/manuscript.figs/raw/", width = 12, height = 6, units = "in")
