...
#Initialize values
purpose1.lab = purpose2.lab <- NULL

#------------- Simulation parameters and labels  ----------------#
date.of.simulation3 <- "29Jun2022"
purpose3 <- "Obj1.2_PopSimLambda.1_fixed.lambda.prior"
purpose3.lab <- "Target YOY | PopSimLambda 1| Fixed lambda"
model.type3 <- "HS.only"

date.of.simulation4 <- "29Jun2022"
purpose4 <- "Obj1.2_PopSimLambda.variable_fixed.lambda.prior"
purpose4.lab <- "Target YOY | PopSimLambda variable | Fixed lambda"
model.type4 <- "HS.only"
...
#Use this if results have already been combined
# results4.2 <- read_csv(paste0(results_location, results_prefix, "_", date.of.simulation4, "_", seeds, "_", purpose4, "allCombined.csv")) %>%
#     mutate(model_type = model.type4,
#            purpose.lab = purpose4.lab,
#            lambda.prior = "fixed")
...
for(i in 1:length(s3.1)){ #Loop over all the posterior samples from each iteration; length should be same as number of iterations
  ...
  rownames_to_column(var = "parameter")
   ...
   # #---------------------------Purpose 3--------------------------------#
    ...
    post.HPD.samples1.3 <- post.HPD.samples1.3[row.names(post.HPD.samples1.3) %in% jags_params3.4,] %>% #Remove deviance
      rownames_to_column(var = "parameter")
    ...
    post.HPD.samples1.4 <- post.HPD.samples1.4[row.names(post.HPD.samples1.4) %in% jags_params3.4,] %>% #Remove deviance
      rownames_to_column(var = "parameter")
...
results.all <- results3.2 %>% 
  bind_rows(results4.2) %>% 
     mutate(relative_bias = round(((Q50 - all.truth)/all.truth)*100, 1)) %>% #Can change truth to breed.truth if looking for number of active breeders
     mutate(in_interval = ifelse(HPD2.5 < all.truth & all.truth < HPD97.5, "Y", "N")) %>% 
...
results.all$purpose.lab <- factor(results.all$purpose.lab, levels = c(purpose3.lab, purpose4.lab))
...
-------------------------------------------------------------------------------------------
Part above is the part that matters before the data viz script (specifically, writing the HPD summary files)

```{r other analysis code}
