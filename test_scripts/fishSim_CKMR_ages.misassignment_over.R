...
#Dad_truth
    ...
    indiv_mis <- indiv %>% rename(trueAgeLast = AgeLast) %>% 
      mutate(AgeLast = trueAgeLast + sample(mis_vec, nrow(indiv), replace = TRUE)) %>% select(Me, Sex, Dad, Mum, BirthY, DeathY, Stock, AgeLast, SampY, trueAgeLast) 
...
print(paste0("finished iteration", iter, " at: ", Sys.time()))
 ...
 results %>% group_by(Sex) %>% 
  ...
  write.table(results, file = paste0("/home/js16a/R/working_directory/CKMR_simulations/results/fishSim_null", total_samples, ".samples_02.03.2021_ages.over_fishSim.N.csv"), sep=",", dec=".", qmethod="double", row.names=FALSE)

    write.table(age_dist, file = paste0("/home/js16a/R/working_directory/CKMR_simulations/results/fishSim_age.distributions_", total_samples, ".samples_02.03.2021_ages.over_age.dist.csv"), sep=",", dec=".", qmethod="double", row.names=FALSE)

    write.table(survival_at_age, file = paste0("/home/js16a/R/working_directory/CKMR_simulations/results/fishSim_survival.at.age_", total_samples, ".samples_02.03.2021_ages.over_surv.csv"), sep=",", dec=".", qmethod="double", row.names=FALSE)
}