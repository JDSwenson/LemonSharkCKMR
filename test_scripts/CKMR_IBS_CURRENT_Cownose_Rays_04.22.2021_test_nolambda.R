...
Mom_truth <- mean(pop.size$Female.adult.pop[year_of_est:n_yrs], na.rm = TRUE)
Dad_truth <- mean(pop.size$Male.adult.pop[year_of_est:n_yrs], na.rm = TRUE)
...
colnames(metrics) <- c("Parents_detected","Obs_lambda", "Obs_survival", "Total_samples")
...
write.table(results, file = paste0("~/R/R_working_dir/LemonSharkCKMR_GitHub/02_IBS/Dovi_IBS_model_validation/abundance_estimates/Dovi.IBS_", total_samples, ".samples_Cownose.Ray_04.22.2021_N_nolambda.csv"), sep=",", dec=".", qmethod="double", row.names=FALSE)

write.table(demo, file = paste0("~/R/R_working_dir/LemonSharkCKMR_GitHub/02_IBS/Dovi_IBS_model_validation/demographic_output/Dovi.IBS_", total_samples, ".samples_Cownose.Ray_04.22.2021_demographic.out_nolambda.csv"), sep=",", dec=".", qmethod="double", row.names=FALSE)
