...
YOY.survival <- .65 # young of year survival
...
for(samps in 1:4){
  ...
  sample.size <- c(200, 400, 600, 850)[samps]
...
nrow(loopy.pop[loopy.pop$Survival=='S' & loopy.pop$age.x>=12, ])/nrow(loopy.pop[loopy.pop$age.x>=12,]) #survival of adults
nrow(loopy.pop[loopy.pop$Survival=='S' & loopy.pop$age.x>0 & loopy.pop$age.x<12, ])/nrow(loopy.pop[loopy.pop$age.x>0 & loopy.pop$age.x<12,]) #survival of juveniles
...
sample.df_all.info <- sample.df_all.info %>% dplyr::arrange(birth.year, desc()) #sort by birth year so when we make the pairwise comparison matrix, Ind_1 is always older than Ind_2

#Remove re-sampled individuals (might be able to account for these in the model ...)
...
#Make sure there are not recaptures that are being compared
pairwise.df_all.info %>% filter(Ind_1 == Ind_2)

#Extract positive HS comparisons
positives <- pairwise.df_all.info %>% filter(Ind_1_mom == Ind_2_mom | Ind_1_dad == Ind_2_dad)

#ID same-cohort positive comparisons and format for anti-join later
same_cohort <- positives %>% filter(Ind_1_birth == Ind_2_birth) %>% 
  select(Ind_1, Ind_2, birth = Ind_2_birth, Ind_1_mom, Ind_2_mom, Ind_1_dad, Ind_2_dad) %>% 
  pivot_longer(cols = c(Ind_1, Ind_2), names_to = "ind", values_to = "indv.name") %>% 
  distinct(indv.name, .keep_all = TRUE) %>% 
  select(indv.name)

#Identify full sibs and organize for filtering
full_sibs <- positives %>% filter(Ind_1_mom == Ind_2_mom & Ind_1_dad == Ind_2_dad) %>% 
  select(Ind_1, Ind_1_birth, Ind_2, Ind_1_mom, Ind_1_dad) %>% 
  mutate(cohort = paste0(Ind_1_birth, "_mom_", Ind_1_mom, "_dad_", Ind_1_dad)) %>% 
  select(Ind_1, Ind_2, cohort) %>% 
  pivot_longer(!cohort, names_to = "ind", values_to = "indv.name") %>% 
  distinct(indv.name, .keep_all = TRUE) %>% 
  select(indv.name, cohort)

full_sibs_to_include <- full_sibs %>% 
  group_by(cohort) %>% 
  sample_n(size = 1)

#Split into positive comparisons for moms and dads
mom_positives_same_cohort <- positives %>% filter(Ind_1_mom == Ind_2_mom & Ind_1_birth == Ind_2_birth & Ind_1_dad != Ind_2_dad) %>% 
  select(Ind_1, Ind_1_birth, Ind_2, Ind_1_mom, Ind_2_mom) %>% 
  mutate(cohort = paste0(Ind_1_birth, "_", Ind_1_mom)) %>% 
  select(Ind_1, Ind_2, cohort) %>% 
  pivot_longer(!cohort, names_to = "ind", values_to = "indv.name") %>% 
  distinct(indv.name, .keep_all = TRUE) %>% 
  select(indv.name, cohort)

dad_positives_same_cohort <- positives %>% filter(Ind_1_dad == Ind_2_dad & Ind_1_birth == Ind_2_birth & Ind_1_mom != Ind_2_mom) %>% 
  select(Ind_1, Ind_1_birth, Ind_2, Ind_1_dad, Ind_2_dad) %>% 
  mutate(cohort = paste0(Ind_1_birth, "_", Ind_1_dad)) %>% 
  select(Ind_1, Ind_2, cohort) %>% 
  pivot_longer(!cohort, names_to = "ind", values_to = "indv.name") %>% 
  distinct(indv.name, .keep_all = TRUE) %>% 
  select(indv.name, cohort)

#Sample one individual from each cohort
mom_positives_to_include <- mom_positives_same_cohort %>% 
  group_by(cohort) %>% 
  sample_n(size = 1)

dad_positives_to_include <- dad_positives_same_cohort %>% 
  group_by(cohort) %>% 
  sample_n(size = 1)

all_samples_to_include <- data.frame(rbind(mom_positives_to_include, dad_positives_to_include, full_sibs_to_include))

samples_to_exclude <- same_cohort %>% anti_join(all_samples_to_include, by = "indv.name")
...
mom_positives <- positives2 %>% filter(Ind_1_mom == Ind_2_mom) %>% 
select(Ind_1_birth, Ind_2_birth) %>% 
  plyr::count()

dad_positives <- positives2 %>% filter(Ind_1_dad == Ind_2_dad) %>% 
  select(Ind_1_birth, Ind_2_birth) %>% 
  plyr::count()

parent_positives <- positives2 %>% filter(Ind_1_dad == Ind_2_dad | Ind_1_mom == Ind_2_mom) %>% 
  select(Ind_1_birth, Ind_2_birth) %>% 
  plyr::count()


#Make dataframe for negative comparisons
mom_negatives <- pairwise.df_all.info2 %>% filter(Ind_1_mom != Ind_2_mom & Ind_1_birth != Ind_2_birth) %>% 
  select(Ind_1_birth, Ind_2_birth) %>% 
  plyr::count()

dad_negatives <- pairwise.df_all.info2 %>% filter(Ind_1_dad != Ind_2_dad & Ind_1_birth != Ind_2_birth) %>% 
  select(Ind_1_birth, Ind_2_birth) %>% 
  plyr::count()

parent_negatives <- pairwise.df_all.info2 %>% filter(Ind_1_dad != Ind_2_dad & Ind_1_mom != Ind_2_mom & Ind_1_birth != Ind_2_birth) %>% 
  select(Ind_1_birth, Ind_2_birth) %>% 
  plyr::count()

#-------------Kinship probabilities - Half-sib-------------------
#Set the first cohort to the first year for which we have data
min_est_cohort <- min(sample.df_all.info2$birth.year)
...
pop_growth_all_mean <- mean(pop.size$Lambda[min_est_cohort:length(pop.size)], na.rm=T)

#Calculate mean survival-at-age
mean_adult_surv <-nrow(loopy.pop[loopy.pop$Survival=='S' & loopy.pop$age.x>=12, ])/nrow(loopy.pop[loopy.pop$age.x>=12,]) #survival of adults

###Used in model###
#surv <- 0.85
surv <- mean_adult_surv
#lam <- lambda1(A1_pre)
lam <- mean(pop.size$Lambda, na.rm=T) #Mean Lambda
...
write.table(results, file = paste0("~/R/R_working_dir/CKMR/LemonSharkCKMR_GitHub/IBS_simulations/Dovi_IBS_results/Dovi.IBS_", total_samples, ".samples_one.ind.per.cohort_03.11.2021_N.csv"), sep=",", dec=".", qmethod="double", row.names=FALSE)
