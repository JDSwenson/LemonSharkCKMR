...
iterations <- 50
sample.size <- 150
lam_setting <- c(seq(-0.01, 0.01, by = .002))
surv_setting <- c(seq(-0.02, .02, by = 0.01))

for(lm in 1:length(lam_setting)){
  #set.seed(47)
  #sample.size <- c(50, 100, 150, 200)[samps]
  for(sv in 1:length(surv_setting)){
    results <- NULL
...
nrow(loopy.pop[loopy.pop$Survival=='S' & loopy.pop$age.x>=repro.age & loopy.pop$age.x < max.age, ])/nrow(loopy.pop[loopy.pop$age.x>=repro.age & loopy.pop$age.x < max.age,]) #survival of adults -- added term to discount adults that reach the max age
...
survival_vec <- NULL
for(i in 1:length(loopy.list)){ #Again, added line to account for just killing animals at age 19
  survival_vec[i] <- nrow(loopy.list[[i]][loopy.list[[i]]$Survival=='S' & loopy.list[[i]]$age.x>=repro.age & loopy.list[[i]]$age.x < max.age, ])/nrow(loopy.list[[i]][loopy.list[[i]]$age.x>=repro.age & loopy.list[[i]]$age.x < max.age, ])
}
...
#Compare to above to see how many recaptures
nrow(sample.df_all.info)
...
#Identify self-recaptures - should have been filtered above
...
mom_negatives <- pairwise.df_all.info %>% filter(Ind_1_mom != Ind_2_mom & Ind_1_birth != Ind_2_birth) %>% 
  select(Ind_1_birth, Ind_2_birth) %>% 
  plyr::count()
...
min_cohort <- min(sample.df_all.info$birth.year)

#set estimated cohort to first year of sampling
year_of_est <- min(sample.years)

m_adult_age = f_adult_age = adult_mat_age <- c(repro.age:max.age) #Set ages at which males and females are mature.

#Mean population growth for all years
pop_growth_all_mean <- mean(pop.size$Lambda[1:nrow(pop.size)], na.rm=T)

#Calculate mean survival and lambda for adults from the first year we have data to the present
mean_adult_surv <- mean(survival_vec[min_cohort:n_yrs])
mean_lam <- mean(pop.size$Lambda[min_cohort:n_yrs], na.rm=T) #Mean Lambda
...
Mom_truth <- pop.size$Female.adult.pop[year_of_est]
Dad_truth <- pop.size$Male.adult.pop[year_of_est]
...
metrics <- cbind(c(sum(mom_positives[,3]), sum(dad_positives[,3]), sum(parent_positives[,3])), #number of positive comparisons found
                 c(rep(lam, times = 3)), #used lambda
                 c(rep(mean_lam, times = 3)), #observed lambda
                 c(rep(surv, times = 3)), #used survival
                 c(rep(mean_adult_surv, times = 3)), #observed survival
                 c(rep(total_samples, times=3)) #samples
                 )

colnames(metrics) <- c("Parents_detected", "Used_lambda", "Obs_lambda", "Used_survival", "Obs_survival", "Total_samples")
...
write.table(results, file = paste0("~/R/R_working_dir/LemonSharkCKMR_GitHub/02_IBS/Dovi_IBS_model_validation/abundance_estimates/Dovi.IBS_", surv, ".surv_", lam, ".lam_Cownose.Ray_04.22.2021_N.csv"), sep=",", dec=".", qmethod="double", row.names=FALSE)

write.table(demo, file = paste0("~/R/R_working_dir/LemonSharkCKMR_GitHub/02_IBS/Dovi_IBS_model_validation/demographic_output/Dovi.IBS_", surv, ".surv_", lam, ".lam_Cownose.Ray_04.22.2021_demographic.out.csv"), sep=",", dec=".", qmethod="double", row.names=FALSE)
