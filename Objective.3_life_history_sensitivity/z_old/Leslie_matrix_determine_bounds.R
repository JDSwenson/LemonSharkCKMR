...
YOY.survival.vec <- c(seq(from = 0.4, to = 0.93, by = 0.01))
juv.survival.vec <- c(seq(from = min(YOY.survival.vec)+0.01, to = max(YOY.survival.vec)+0.01, by = 0.01))
adult.survival.vec <- c(seq(from = min(juv.survival.vec)+0.01, to = max(juv.survival.vec)+0.01, by = 0.01))

for(y in 1:length(YOY.survival.vec)){
  yoy <- YOY.survival.vec[y]
  for(j in y:length(juv.survival.vec)){
    juv <- juv.survival.vec[j]
    for(a in j:length(adult.survival.vec)){
      adult <- adult.survival.vec[a]
      
      #Input to Leslie matrix
      survival.vec <- c(yoy, rep(juv, times = juv.ages), rep(adult, times = adult.ages - 1), 0)
      fecund.vec <- c(rep(0, times = repro.age), rep(fb, times = maxAge - juv.ages))
      ...
      lambda.temp.df <- lambda1 %>% 
        mutate(yoy.survival = yoy, juvenile.survival = juv, adult.survival = adult)
      
      #Store lambda dataframe
      lambda.df <- rbind(lambda.df, lambda.temp.df)
    }
  }
  print(paste0("Finished iteration ", y, " of ", length(YOY.survival.vec)))
}
  
lambda.df %>% mutate(fb = fb)

#saveRDS(lambda.df, file = "G://My Drive/Personal_Drive/R/CKMR/Objective.2_Sensitivity.elasticity/lambda.df_varied.survival")

lambda.df %>% dplyr::filter(lambda >=0.95 & lambda <= 1.05)


lb <- readRDS(file = "G://My Drive/Personal_Drive/R/CKMR/Objective.2_Sensitivity.elasticity/lambda.df_varied.survival")

js <- juvenile.survival*.9
as <- Adult.survival*.9
lb %>% mutate(across(where(is.numeric), round, 2)) %>% dplyr::filter(juvenile.survival == Juvenile.survival, adult.survival == 0.82)



#, lambda > 0.9 & lambda < 1.1
# , juvenile.survival == js, adult.survival == as
