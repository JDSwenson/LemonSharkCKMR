...
purpose.combo <- "Model validation; determine best uninformative prior for lambda" #What am I comparing right now? This will be used to name output files
...
date.of.simulation1 <- "24Jun2022"
purpose1 <- "Obj1.1_model.validation_uniform.lambda_target.YOY"
purpose1.lab <- "Target YOY | Uniform lambda"
model.type1 <- "HS.only"

date.of.simulation2 <- "24Jun2022"
purpose2 <- "Obj1.1_model.validation_uniform.lambda_sample.all.ages"
purpose2.lab <- "All ages sampled | Uniform lambda"
model.type2 <- "HS.only"

date.of.simulation3 <- "25Jun2022"
purpose3 <- "Obj1.2_test.fixed.lambda_target.YOY"
purpose3.lab <- "Target YOY | Fixed lambda"
model.type3 <- "HS.only"

date.of.simulation4 <- "25Jun2022"
purpose4 <- "Obj1.2_test.fixed.lambda_sample.all.ages"
purpose4.lab <- "All ages sampled | Fixed lambda"
model.type4 <- "HS.only"

date.of.simulation5 <- "25Jun2022"
purpose5 <- "Obj1.Supp_model.validation_uniform.lambda_sample.all.ages_downsample"
purpose5.lab <- "All ages sampled + downsample | Fixed lambda"
model.type5 <- "HS.only"

seeds <- "Seeds2022.04.15"
sim.samples <- "1prop.sampled" #Label on file output for fixed lambda trials
samples.lab <- "1 percent sampled" #Label for dataframe and figures for fixed lambda trials
jags_params = c("Nf", "psi", "Nm", "survival", "lambda") #For trials with uniform prior
jags_params3.4 = c("Nf", "psi", "Nm", "survival") #for trials with fixed prior
...
results.1 = results.2 = results.3 = results.4 = results.5 <- NULL
...
#Use this if about to combine results
...
#Use this if results have already been combined
results3.2 <- read_csv(paste0(results_location, results_prefix, "_", date.of.simulation3, "_", seeds, "_", purpose3, "allCombined.csv")) %>% 
  mutate(model_type = model.type3,
         purpose.lab = purpose3.lab,
         lambda.prior = "fixed")
...
#Use this if results have already been combined
results.4.2 <- read_csv(paste0(results_location, results_prefix, "_", date.of.simulation4, "_", seeds, "_", purpose4, "allCombined.csv")) %>% 
    mutate(model_type = model.type4,
           purpose.lab = purpose4.lab,
           lambda.prior = "fixed")

results.5 <- read_csv(paste0(results_location, results_prefix, "_", date.of.simulation5, "_", seeds, "_", purpose5, ".csv")) %>% 
  mutate(model_type = model.type5, 
         purpose.lab = purpose5.lab,
         lambda.prior = "uniform")

#------------------------------- MCMC output -----------------------------------#
#date.of.simulation <- "07Apr2022"
#Trial 1: Uniform lambda; target YOY; 4 sample sizes
...
#Trial 2: Uniform lambda; sample all age classes; 4 sample sizes
...
#Trial 3: Lambda fixed priors (0.95, 1.0, 1.05); target YOY; 1 sample size (1%)
#date.of.simulation <- "05Apr2022"
s3.1 <- readRDS(paste0(MCMC_location, MCMC_prefix, "_", date.of.simulation3, "_", seeds, "_", sim.samples, "_", MCMC.settings, "_", purpose3, "_", lam.1))

s3.2 <- readRDS(paste0(MCMC_location, MCMC_prefix, "_", date.of.simulation3, "_", seeds, "_", sim.samples, "_", MCMC.settings, "_", purpose3, "_", lam.2))

s3.3 <- readRDS(paste0(MCMC_location, MCMC_prefix, "_", date.of.simulation3, "_", seeds, "_", sim.samples, "_", MCMC.settings, "_", purpose3, "_", lam.3))

#s3.4 <- readRDS(paste0(MCMC_location, MCMC_prefix, "_", date.of.simulation3, "_", seeds, "_", sim.samples.4, "_", MCMC.settings, "_", purpose3))

#Trial 4: Lambda fixed priors (0.95, 1.0, 1.05); sample all ages; 1 sample size (1%)
 s4.1 <- readRDS(paste0(MCMC_location, MCMC_prefix, "_", date.of.simulation4, "_", seeds, "_", sim.samples, "_", MCMC.settings, "_", purpose4, "_", lam.1))

 s4.2 <- readRDS(paste0(MCMC_location, MCMC_prefix, "_", date.of.simulation4, "_", seeds, "_", sim.samples, "_", MCMC.settings, "_", purpose4, "_", lam.2))

 s4.3 <- readRDS(paste0(MCMC_location, MCMC_prefix, "_", date.of.simulation4, "_", seeds, "_", sim.samples, "_", MCMC.settings, "_", purpose4, "_", lam.3))

 
 #Trial 5: Uniform prior; sample all ages + downsample; 4 sample sizes
s5.1 <- readRDS(paste0(MCMC_location, MCMC_prefix, "_", date.of.simulation5, "_", seeds, "_", sim.samples.1, "_", MCMC.settings, "_", purpose5))

s5.2 <- readRDS(paste0(MCMC_location, MCMC_prefix, "_", date.of.simulation5, "_", seeds, "_", sim.samples.2, "_", MCMC.settings, "_", purpose5))

s5.3 <- readRDS(paste0(MCMC_location, MCMC_prefix, "_", date.of.simulation5, "_", seeds, "_", sim.samples.3, "_", MCMC.settings, "_", purpose5))

s5.4 <- readRDS(paste0(MCMC_location, MCMC_prefix, "_", date.of.simulation5, "_", seeds, "_", sim.samples.4, "_", MCMC.settings, "_", purpose5))

#------------------------------- Population size details -----------------------------------#
#date.of.simulation <- "07Apr2022"
pop.size.tibble <- readRDS(paste0(results_location, pop.size.prefix, "_", date.of.simulation3, "_", seeds, "_", purpose3)) %>% mutate(Total.adult.pop = Male.adult.pop + Female.adult.pop)
...
#Initialize for lambda uniform prior: target YOY and sample all ages; 4 sample sizes
HPD.samples1.1 <- NULL
HPD.samples2.1 <- NULL
HPD.samples3.1 <- NULL
HPD.samples4.1 <- NULL
...
#Initialize for lambda fixed priors (0.95, 1.0, 1.05): target YOY and sample all ages; 1 sample size (1%)
HPD.samples1.3 <- NULL
HPD.samples2.3 <- NULL
HPD.samples3.3 <- NULL
HPD.samples4.3 <- NULL
...
#Initialize for lambda uniform prior: sample all ages + downsample; 4 sample sizes
HPD.samples1.5 <- NULL
HPD.samples2.5 <- NULL
HPD.samples3.5 <- NULL
HPD.samples4.5 <- NULL
...
for(i in 1:length(s1.1)){ #Loop over all the posterior samples from each iteration; length should be same as number of iterations
  for(j in 1:length(intervals)){ #Loop over the different intervals for calculating HPDI
    ...
    HPD.samples2.1 <- rbind(HPD.samples2.1, post.HPD.samples2.1)
     ...
     #samples3 samples
    #Calcualte HPD interval for iteration i and interval j
    post.HPD.samples3.1 <- combine.mcmc(s1.3[[i]]) %>% 
      HPDinterval(prob = intervals[j]) %>%
      data.frame() %>% 
      mutate(interval = intervals[j], iteration = i)
    ...
    #samples4 samples
    #Calcualte HPD interval for iteration i and interval j
    post.HPD.samples4.1 <- combine.mcmc(s1.4[[i]]) %>% 
      HPDinterval(prob = intervals[j]) %>%
      data.frame() %>% 
      mutate(interval = intervals[j], iteration = i)
    ...
    HPD.samples2.2 <- rbind(HPD.samples2.2, post.HPD.samples2.2)
        ...
        #samples3 samples
    #Calcualte HPD interval for iteration i and interval j
    post.HPD.samples3.2 <- combine.mcmc(s2.3[[i]]) %>% 
      HPDinterval(prob = intervals[j]) %>%
      data.frame() %>% 
      mutate(interval = intervals[j], iteration = i)
    ...
    #samples4 samples
    #Calcualte HPD interval for iteration i and interval j
    post.HPD.samples4.2 <- combine.mcmc(s2.4[[i]]) %>% 
      HPDinterval(prob = intervals[j]) %>%
      data.frame() %>% 
      mutate(interval = intervals[j], iteration = i)
...
#Fixed lambdas
model.sum.3.all <- model.sum.4.all <- NULL
for(i in 1:length(s3.1)){ #Loop over all the posterior samples from each iteration; length should be same as number of iterations
   # #---------------------------Purpose 3--------------------------------#
    #Combine chains for different fixed lambdas
    HPD.1.3.temp <- combine.mcmc(list(s3.1[[i]], s3.2[[i]], s3.3[[i]]))
    
    #Put together new results file from combined mcmc runs
    post.95.3 <- combine.mcmc(HPD.1.3.temp) %>%
      HPDinterval() %>%
      data.frame() %>%
      rownames_to_column(var = "parameter")
    post.95.3 <- post.95.3 %>% filter(parameter %in% jags_params3.4) #Remove deviance
    
    
    model.sum.3 <- data.frame(t(post_summ(HPD.1.3.temp, jags_params3.4, Rhat = T, neff = T))) %>% 
  rownames_to_column(var = "parameter")
    
    #Combine into data.frame
    model.sum.3.2 <- model.sum.3 %>% left_join(post.95.3, by = "parameter") %>%
    ...
    model.sum.3.all <- rbind(model.sum.3.all, model.sum.3.2)

        #---------------------------Purpose 4--------------------------------#
    #Combine chains for different fixed lambdas
    HPD.1.4.temp <- combine.mcmc(list(s4.1[[i]], s4.2[[i]], s4.3[[i]]))
    
    #Put together new results file from combined mcmc runs
    post.95.4 <- combine.mcmc(HPD.1.4.temp) %>%
      HPDinterval() %>%
      data.frame() %>%
      rownames_to_column(var = "parameter")
    post.95.4 <- post.95.4 %>% filter(parameter %in% jags_params3.4) #Remove deviance
    
    
    model.sum.4 <- data.frame(t(post_summ(HPD.1.4.temp, jags_params3.4, Rhat = T, neff = T))) %>% 
  rownames_to_column(var = "parameter")
    
    #Combine into data.frame
    model.sum.4.2 <- model.sum.4 %>% left_join(post.95.4, by = "parameter") %>%
      rename(HPD2.5 = lower, HPD97.5 = upper) %>%
      dplyr::select(parameter, Q2.5 = X2.5., Q97.5 = X97.5., Q50 = X50., mean = mean, sd = sd, HPD2.5, HPD97.5, Rhat, neff) %>% 
      mutate(iteration = i)
    
    model.sum.4.all <- rbind(model.sum.4.all, model.sum.4.2)

    
    for(j in 1:length(intervals)){ #Loop over the different intervals for calculating HPDI
   # #---------------------------Purpose 3--------------------------------#
    post.HPD.samples1.3 <- combine.mcmc(HPD.1.3.temp) %>%
      HPDinterval(prob = intervals[j]) %>%
      data.frame() %>%
      mutate(interval = intervals[j], iteration = i)
    ...
    post.HPD.samples1.4 <- combine.mcmc(HPD.1.4.temp) %>%
      HPDinterval(prob = intervals[j]) %>%
      data.frame() %>%
      mutate(interval = intervals[j], iteration = i)
...
#Uniform lambda; sample all ages + downsample; 4 sample sizes
for(i in 1:length(s5.1)){ #Loop over all the posterior samples from each iteration; length should be same as number of iterations
  for(j in 1:length(intervals)){ #Loop over the different intervals for calculating HPDI
    
    #---------------------------Purpose 1--------------------------------#
    #samples1 samples
    #Calcualte HPD interval for iteration i and interval j
    post.HPD.samples1.5 <- combine.mcmc(s5.1[[i]]) %>% 
      HPDinterval(prob = intervals[j]) %>%
      data.frame() %>% 
      mutate(interval = intervals[j], iteration = i)
    
    post.HPD.samples1.5 <- post.HPD.samples1.5[row.names(post.HPD.samples1.5) %in% jags_params,] %>% #Remove deviance
      rownames_to_column(var = "parameter")

    HPD.samples1.5 <- rbind(HPD.samples1.5, post.HPD.samples1.5)
    
    #samples2 samples
    #Calcualte HPD interval for iteration i and interval j
    post.HPD.samples2.5 <- combine.mcmc(s5.2[[i]]) %>% 
      HPDinterval(prob = intervals[j]) %>%
      data.frame() %>% 
      mutate(interval = intervals[j], iteration = i)
    
    post.HPD.samples2.5 <- post.HPD.samples2.5[row.names(post.HPD.samples2.5) %in% jags_params,] %>% #Remove deviance
      rownames_to_column(var = "parameter")
    
    HPD.samples2.5 <- rbind(HPD.samples2.5, post.HPD.samples2.5)
    
     #samples3 samples
    #Calcualte HPD interval for iteration i and interval j
    post.HPD.samples3.5 <- combine.mcmc(s5.3[[i]]) %>% 
      HPDinterval(prob = intervals[j]) %>%
      data.frame() %>% 
      mutate(interval = intervals[j], iteration = i)
    
    post.HPD.samples3.5 <- post.HPD.samples3.5[row.names(post.HPD.samples3.5) %in% jags_params,] %>% #Remove deviance
      rownames_to_column(var = "parameter")
    
    HPD.samples3.5 <- rbind(HPD.samples3.5, post.HPD.samples3.5)

    #samples4 samples
    #Calcualte HPD interval for iteration i and interval j
    post.HPD.samples4.5 <- combine.mcmc(s5.4[[i]]) %>% 
      HPDinterval(prob = intervals[j]) %>%
      data.frame() %>% 
      mutate(interval = intervals[j], iteration = i)
    
    post.HPD.samples4.5 <- post.HPD.samples4.5[row.names(post.HPD.samples4.5) %in% jags_params,] %>% #Remove deviance
      rownames_to_column(var = "parameter")
    
    HPD.samples4.5 <- rbind(HPD.samples4.5, post.HPD.samples4.5)
  }
  print(paste0("Finished with iteration ", i))
}
...
```{r Create results file for combined mcmc chains for lambda}
#Extract truth values from runs with same sample scheme
results.3.1comb <- results.3 %>% dplyr::select(!c(Q2.5, Q97.5, Q50, mean, sd, HPD2.5, HPD97.5, Rhat, neff, purpose, purpose.lab, relative_bias, in_interval, lambda.fix))

results.3.temp <- model.sum.3.all %>% left_join(results.3.1comb, by = c("parameter", "iteration")) %>% 
  mutate(purpose = purpose3, purpose.lab = purpose3.lab) %>% 
  as_tibble() %>% 
  dplyr::distinct(parameter, iteration, .keep_all = TRUE)

results3.2 <- results.3.temp %>%
  ...
  mutate(lambda.prior = "fixed") %>%
  as_tibble()

write.table(results3.2, file = paste0(results_location, results_prefix, "_", date.of.simulation3, "_", seeds, "_", purpose3, "allCombined.csv"), sep=",", dec=".", qmethod="double", row.names=FALSE)
   

#Same for second round (sampling all)
results.4.1comb <- results.4 %>% dplyr::select(!c(Q2.5, Q97.5, Q50, mean, sd, HPD2.5, HPD97.5, Rhat, neff, purpose, purpose.lab, relative_bias, in_interval, lambda.fix))

results.4.temp <- model.sum.4.all %>% left_join(results.4.1comb, by = c("parameter", "iteration")) %>% 
    mutate(purpose = purpose4, purpose.lab = purpose4.lab) %>% 
    dplyr::distinct(parameter, iteration, .keep_all = TRUE)

results4.2 <- results.4.temp %>%
     mutate(relative_bias = round(((Q50 - all.truth)/all.truth)*100, 1)) %>% #Can change truth to breed.truth if looking for number of active breeders
     mutate(in_interval = ifelse(HPD2.5 < all.truth & all.truth < HPD97.5, "Y", "N")) %>%
  mutate(lambda.prior = "fixed") %>% 
     as_tibble()

write.table(results4.2, file = paste0(results_location, results_prefix, "_", date.of.simulation4, "_", seeds, "_", purpose4, "allCombined.csv"), sep=",", dec=".", qmethod="double", row.names=FALSE)
```
...
results.all <- results.1 %>% 
  bind_rows(results.2, results3.2, results4.2, results.5) %>% 
     mutate(relative_bias = round(((Q50 - all.truth)/all.truth)*100, 1)) %>% #Can change truth to breed.truth if looking for number of active breeders
     mutate(in_interval = ifelse(HPD2.5 < all.truth & all.truth < HPD97.5, "Y", "N")) %>% 
...
results.all$samples.lab <- factor(results.all$samples.lab, levels = c(sim.samples.labs.all))
results.all %>% dplyr::filter(is.na(samples.lab == TRUE)) #Should not be any

results.all$purpose.lab <- factor(results.all$purpose.lab, levels = c(purpose1.lab, purpose2.lab, purpose3.lab, purpose4.lab, purpose5.lab))
...
no.convergence <- results.all %>% dplyr::filter(Rhat > 1.01) %>% 
  mutate(purp = ifelse(purpose.lab == purpose1.lab, 1,
                       ifelse(purpose.lab == purpose2.lab, 2,
                              ifelse(purpose.lab == purpose3.lab, 3, 
                                     ifelse(purpose.lab == purpose4.lab, 4, 5))))) %>% 
  distinct(purpose.lab, purp, sample.size.juvs, iteration)
...
results.all %>% group_by(sample.prop.juvs, parameter, purpose.lab) %>% 
...
#------------------------------- Within HPDI? -----------------------------------#
results.all %>% group_by(purpose.lab, sample.prop.juvs, parameter) %>% 
...
#------------------------------- CV -----------------------------------#
results.all %>% dplyr::group_by(purpose.lab, parameter) %>% 
...
results.all %>% dplyr::filter(parameter == "Nf" | parameter == "Nm") %>% 
  group_by(purpose.lab, parameter, sample.prop.juvs) %>% 
...
HPD.samples1.1.4viz <- results.1 %>% filter(sample.prop.juvs == sim.samples.all[1]) %>% 
  right_join(HPD.samples1.1, by = c("parameter", "iteration")) %>% 
  anti_join(no.convergence, by = c("purpose.lab", "iteration", "sample.size.juvs")) %>%  #Remove iterations that failed to converge
  mutate(samples.lab = samples1.lab)
  
HPD.samples2.1.4viz <- results.1 %>% filter(sample.prop.juvs == sim.samples.all[2]) %>% 
  right_join(HPD.samples2.1, by = c("parameter", "iteration")) %>% 
  anti_join(no.convergence, by = c("purpose.lab", "iteration", "sample.size.juvs")) %>%  #Remove iterations that failed to converge
  mutate(samples.lab = samples2.lab)

HPD.samples3.1.4viz <- results.1 %>% dplyr::filter(sample.prop.juvs == sim.samples.all[3]) %>% 
  right_join(HPD.samples3.1, by = c("parameter", "iteration")) %>% 
  anti_join(no.convergence, by = c("purpose.lab", "iteration", "sample.size.juvs")) %>%  #Remove iterations that failed to converge
  mutate(samples.lab = samples3.lab)

HPD.samples4.1.4viz <- results.1 %>% filter(sample.prop.juvs == sim.samples.all[4]) %>% 
...
#---------------------------Purpose 2 --------------------------------#    
#Separate out by sample size
HPD.samples1.2.4viz <- results.2 %>% filter(sample.prop.juvs == sim.samples.all[1]) %>% 
  right_join(HPD.samples1.2, by = c("parameter", "iteration")) %>% 
  anti_join(no.convergence, by = c("purpose.lab", "iteration", "sample.size.juvs")) %>%  #Remove iterations that failed to converge
  mutate(samples.lab = samples1.lab)

HPD.samples2.2.4viz <- results.2 %>% filter(sample.prop.juvs == sim.samples.all[2]) %>% 
  right_join(HPD.samples2.2, by = c("parameter", "iteration")) %>% 
    anti_join(no.convergence, by = c("purpose.lab", "iteration", "sample.size.juvs")) %>%  #Remove iterations that failed to converge
  mutate(samples.lab = samples2.lab)

HPD.samples3.2.4viz <- results.2 %>% filter(sample.prop.juvs == sim.samples.all[3]) %>% 
  right_join(HPD.samples3.2, by = c("parameter", "iteration")) %>% 
  anti_join(no.convergence, by = c("purpose.lab", "iteration", "sample.size.juvs")) %>%  #Remove iterations that failed to converge
  mutate(samples.lab = samples3.lab)


HPD.samples4.2.4viz <- results.2 %>% filter(sample.prop.juvs == sim.samples.all[4]) %>% 
...
HPD.samples4.2.summary <- HPD.samples4.2.4viz %>% group_by(parameter, interval) %>%
  #mutate(in_HPD_interval = ifelse(parameter == "Nfb", ifelse(lower < breed.truth & breed.truth < upper, "Y", "N"), ifelse(lower < all.truth & all.truth < upper, "Y", "N"))) %>% #If using breed truth for females
  mutate(in_HPD_interval = ifelse(lower < all.truth & all.truth < upper, "Y", "N")) %>% #If using all truth
  dplyr::summarize(percent_in_interval.2_percent_sampled = sum(in_HPD_interval == "Y")/n() * 100)
...
#---------------------------Purpose 3 --------------------------------# 
#Separate out by sample size
HPD.samples1.3.4viz <- results3.2 %>%
  right_join(HPD.samples1.3, by = c("parameter", "iteration")) %>% 
  anti_join(no.convergence, by = c("purpose.lab", "iteration", "sample.size.juvs")) %>%  #Remove iterations that failed to converge
  mutate(samples.lab = samples.lab)
...
#  mutate(in_HPD_interval = ifelse(parameter == "Nfb", ifelse(lower < breed.truth & breed.truth < upper, "Y", "N"), ifelse(lower < all.truth & all.truth < upper, "Y", "N"))) %>% #If using breed truth for females
  mutate(in_HPD_interval = ifelse(lower < all.truth & all.truth < upper, "Y", "N")) %>% #If using all truth
  dplyr::summarize(percent_in_interval = sum(in_HPD_interval == "Y")/n() * 100)

#Combine above dataframes
HPD.3.summary_all <- HPD.samples1.3.summary %>% 
  mutate(interval.scale = interval*100)
...
#Combine above dataframes
HPD.4.summary_all <- HPD.samples1.4.summary %>%
  mutate(interval.scale = interval*100)
...
#---------------------------Purpose 5 --------------------------------#    
#Separate out by sample size
HPD.samples1.5.4viz <- results.5 %>% filter(sample.prop.juvs == sim.samples.all[1]) %>% 
  right_join(HPD.samples1.5, by = c("parameter", "iteration")) %>% 
  anti_join(no.convergence, by = c("purpose.lab", "iteration", "sample.size.juvs")) %>%  #Remove iterations that failed to converge
  mutate(samples.lab = samples1.lab)
  
HPD.samples2.5.4viz <- results.5 %>% filter(sample.prop.juvs == sim.samples.all[2]) %>% 
  right_join(HPD.samples2.5, by = c("parameter", "iteration")) %>% 
  anti_join(no.convergence, by = c("purpose.lab", "iteration", "sample.size.juvs")) %>%  #Remove iterations that failed to converge
  mutate(samples.lab = samples2.lab)

HPD.samples3.5.4viz <- results.5 %>% dplyr::filter(sample.prop.juvs == sim.samples.all[3]) %>% 
  right_join(HPD.samples3.5, by = c("parameter", "iteration")) %>% 
  anti_join(no.convergence, by = c("purpose.lab", "iteration", "sample.size.juvs")) %>%  #Remove iterations that failed to converge
  mutate(samples.lab = samples3.lab)

HPD.samples4.5.4viz <- results.5 %>% filter(sample.prop.juvs == sim.samples.all[4]) %>% 
  right_join(HPD.samples4.5, by = c("parameter", "iteration")) %>% 
  anti_join(no.convergence, by = c("purpose.lab", "iteration", "sample.size.juvs")) %>%  #Remove iterations that failed to converge
  mutate(samples.lab = samples4.lab)


#Calculate percent estimates in each interval
HPD.samples1.5.summary <- HPD.samples1.5.4viz %>% group_by(parameter, interval) %>% 
#  mutate(in_HPD_interval = ifelse(parameter == "Nfb", ifelse(lower < breed.truth & breed.truth < upper, "Y", "N"), ifelse(lower < all.truth & all.truth < upper, "Y", "N"))) %>% #If using breed truth for females
  mutate(in_HPD_interval = ifelse(lower < all.truth & all.truth < upper, "Y", "N")) %>% #If using all truth
  dplyr::summarize(percent_in_interval.0.5_percent_sampled = sum(in_HPD_interval == "Y")/n() * 100)

HPD.samples2.5.summary <- HPD.samples2.5.4viz %>% group_by(parameter, interval) %>% 
#  mutate(in_HPD_interval = ifelse(parameter == "Nfb", ifelse(lower < breed.truth & breed.truth < upper, "Y", "N"), ifelse(lower < all.truth & all.truth < upper, "Y", "N"))) %>%#If using breed truth for females
  mutate(in_HPD_interval = ifelse(lower < all.truth & all.truth < upper, "Y", "N")) %>% #If using all truth
    dplyr::summarize(percent_in_interval.1_percent_sampled = sum(in_HPD_interval == "Y")/n() * 100)

HPD.samples3.5.summary <- HPD.samples3.5.4viz %>% group_by(parameter, interval) %>% 
#  mutate(in_HPD_interval = ifelse(parameter == "Nfb", ifelse(lower < breed.truth & breed.truth < upper, "Y", "N"), ifelse(lower < all.truth & all.truth < upper, "Y", "N"))) %>% #If using breed truth for females
  mutate(in_HPD_interval = ifelse(lower < all.truth & all.truth < upper, "Y", "N")) %>% #If using all truth
  dplyr::summarize(percent_in_interval.1.5_percent_sampled = sum(in_HPD_interval == "Y")/n() * 100)

HPDsamples4.5.summary <- HPD.samples4.5.4viz %>% group_by(parameter, interval) %>%
  #mutate(in_HPD_interval = ifelse(parameter == "Nfb", ifelse(lower < breed.truth & breed.truth < upper, "Y", "N"), ifelse(lower < all.truth & all.truth < upper, "Y", "N"))) %>% #If using breed truth for females
  mutate(in_HPD_interval = ifelse(lower < all.truth & all.truth < upper, "Y", "N")) %>% #If using all truth
  dplyr::summarize(percent_in_interval.2_percent_sampled = sum(in_HPD_interval == "Y")/n() * 100)

#Combine above dataframes    
HPD.5.summary_all <- HPD.samples1.5.summary %>% inner_join(HPD.samples2.5.summary, by = c("parameter", "interval")) %>% 
  inner_join(HPD.samples3.5.summary, by = c("parameter", "interval")) %>% 
  inner_join(HPDsamples4.5.summary, by = c("parameter", "interval")) %>% 
  mutate(interval.scale = interval*100)

#Read in previously saved file
#HPD.1.summary_all <- read_csv(file = paste0(results_location, "HPD.summaries/HPD.summary_", date.of.simulation1, "_", purpose1, ".csv"))

HPD.5.summary.tidy <- HPD.5.summary_all %>% 
  pivot_longer(
    cols = starts_with("percent"),
    names_to = "proportion_sampled",
    names_prefix = "percent_in_interval.",
...
#Save
#write_csv(HPD.5.summary_all, file = paste0(results_location, "HPD.summaries/HPD.summary_", date.of.simulation5, "_", purpose5, ".csv"))
