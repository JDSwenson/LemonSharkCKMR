...
Sampling.scheme <- "target.YOY" # sample.all.juvenile.ages or target.YOY or sample.ALL.ages
...
script_name <- "Obj1.1_model.validation.R" #Copy name of script here
primary_goal <- "Validate model using informed priors" #Why am I running this simulation? Provide details

question1 <- "Does the model perform as expected when we set an informed prior on lambda and survival?"
question2 <- ""
question3 <- ""
purpose <- "Obj1.1_model.validation" #For naming output files
...
lambda.prior.mean <- 1
lambda.prior.cv <- .01
lambda.prior.sd <- lambda.prior.cv * lambda.prior.mean
lambda.prior.info <- "Normal, w/ mean as 1 and 1% CV"
...
#If we only want to use one type of relationship, we'll filter here
    ...
    #Define JAGS data and model, and run the MCMC engine
      set.seed(rseed)
    source("Objective.1_model.construction/functions/Obj1.1_run.JAGS_HS.only_informed.priors.R")
