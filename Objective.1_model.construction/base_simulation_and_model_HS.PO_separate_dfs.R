...
#Outputs a list, where the first element is a list of various population parameters for each year, and the second is the population size for each year
  ...
  for(samps in 1:length(sample.vec)){
    ...
    filter1.out <- filter.samples(sample.df_all.info) #Filter for full sibs
    ...
    mom_comps.PO <- pairwise.out[[1]] %>% 
      bind_rows()
    dad_comps.PO <- pairwise.out[[2]] %>% 
      bind_rows()
    mom_comps.HS <- pairwise.out[[3]]
    dad_comps.HS <- pairwise.out[[4]]
    
    #NEXT (03/24/2022): 
    #filter above dataframes for instances with positive comparisons
    #define input data for CKMR
    #adapt CKMR model code (in separate script)
    #run CKMR model
    #refer to manuscript outline
