...
script_name <- "Obj1.2_PopSimLambda.variable_uniform.lambda.prior.R" #Copy name of script here
...
question3 <- ""
purpose <- "Obj1.2_PopSimLambda.variable_uniform.lambda.prior" #For naming output files
...
#If we only want to use one type of relationship, we'll filter here
    ...
    #Define JAGS data and model, and run the MCMC engine
      set.seed(rseed)
    source("Objective.1_model.construction/functions/Obj1.2_run.JAGS_HS.only_diffuse.priors.R")
...
write.table(results2, file = paste0(results_location, results_prefix, "_", date.of.simulation, "_", outSeeds, "_", purpose, ".csv"), sep=",", dec=".", qmethod="double", row.names=FALSE)
 ...
 saveRDS(sims.list.1, file = paste0(MCMC_location, MCMC_prefix, "_", date.of.simulation, "_", outSeeds, "_", sim.samples, "_", MCMC.settings, "_", purpose)) #Sample size 1
