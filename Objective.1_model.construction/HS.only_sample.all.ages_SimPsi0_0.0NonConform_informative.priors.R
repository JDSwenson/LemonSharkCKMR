...
#-------------------Set up simulation----------------------------
script_name <- "HS.only_sample.all.ages_SimPsi0_0.0NonConform_informative.priors.R" #Copy name of script here
primary_goal <- "Figure out how the model performs with prior information regarding psi and survival and sampling all juvenile age classes." #Why am I running this simulation? Provide details
...
purpose <- "HS.only_sample.all.ages_SimPsi0_0.0NonConform_informative.priors" #For naming output files
...
non.conformists <- 0.0 #proportion of off-year breeders to randomly include off their breeding cycle - want to change this to non.conformists
...
#set.seed(rseed.pop)
 ...
 for(iter in 1:iterations) {
  ...
  #Save parents tibble
  saveRDS(parents.tibble, file = paste0(results_location, parents_prefix, "_", date.of.simulation, "_", seeds, "_", purpose, "_iter_", iter))
  
  # Detailed info on population size
  saveRDS(pop.size.tibble, file = paste0(results_location, pop.size.prefix, "_", date.of.simulation, "_", seeds, "_", purpose, "_iter_", iter))
