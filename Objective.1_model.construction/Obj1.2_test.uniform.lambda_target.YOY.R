...
script_name <- "test.uniform.lambda_target.YOY.R" #Copy name of script here
...
question2 <- ""
question3 <- ""
purpose <- "test.uniform.lambda_target.YOY" #For naming output files
...
jags_params = c("Nf", "psi", "Nm", "surv", "lam")
...
iterations <- 300 #Number of iterations to loop over
...
#If we only want to use one type of relationship, we'll filter here
    ...
    set.seed(rseed)
    source("Objective.2_sensitivity.elasticity/functions/run.JAGS_HS.only_uniform.lambda.R")
    ...
    source("Objective.2_sensitivity.elasticity/functions/compile.results_HS.only_SB.R")
