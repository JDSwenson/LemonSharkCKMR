...
#These are the names of the input files, either variable population growth (lambda.variable) or stable (lambda.1); etiher target.YOY or sample.all.ages
PopSim.location <- "G://My Drive/Personal_Drive/R/CKMR/Population.simulations/"
PopSim.lambda <- "lambda.1"
Sampling.scheme <- "sample.all.ages" #either target.YOY or sample.all.ages
...
script_name <- "Obj1.1_sample.all.ages_yes.downsample.R" #Copy name of script here
...
question3 <- "Is bias improved with downsampling when sampling all ages?"
purpose <- "Obj1.1_sample.all.ages_yes.downsample" #For naming output files
