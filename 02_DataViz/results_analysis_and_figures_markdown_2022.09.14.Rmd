...
obj4_results.all <- read_rds(file = paste0(objective_4_results_location, "obj4.all_results_w_agesCorr")) %>% 
  mutate(prop.sampled = factor(sample.prop.juvs),
           sampling.scheme = factor(sampling.scheme),
           lambda.prior = factor(lambda.prior, levels = c("none", "diffuse", "tight")),
...
obj4_samps <- read_csv(file = paste0(objective_4_results_location, "sceanrio_4.samps.csv")) %>% 
...
o3.p_naive <- obj3_results.liz %>% dplyr::filter(model_used == "annual", lambda.prior == "tight") %>% 
...
o4.p_miss <- obj4_results %>% dplyr::filter(lambda.prior == "tight") %>% 
  mutate(model_type = ifelse(miss.cv == 0.05, "adapted - 5% aging error CV", "adapted - 10% aging error CV"))

o3.p_all <- bind_rows(o3.p_naive, o3.p_adapted, o4.p_miss) %>% 
  mutate(model_type = factor(model_type, levels = c("naive", "adapted - ages correct", "adapted - 5% aging error CV", "adapted - 10% aging error CV")))
...
ggsave("Figure_3a_2022.09.17", plot = last_plot(), device = "tiff", dpi = 300, path = "G://My Drive/Personal_Drive/R/CKMR/Objective.3_intermittent.breeding/Model.results/figures/")
...
fig3b <- obj3_results.liz %>% dplyr::filter(sim.psi == 1, 
                                   parameter != "psi") %>% 
...
#  facet_wrap(~model_used + lambda.prior)
  facet_grid(rows = vars(lambda.prior), cols = vars(model_used)) +
  theme_bw() +
  theme(legend.position = "bottom")

ggsave("Figure_3b_2022.09.17", plot = last_plot(), device = "tiff", dpi = 300, path = "G://My Drive/Personal_Drive/R/CKMR/Objective.3_intermittent.breeding/Model.results/figures/")


#-----------------Supplementary figure -------------------
#Abundance
figs2 <- obj3_results.liz %>% dplyr::filter(model_used == "Liz_biennial",
...
ggplot(aes(x=factor(sampling.scheme), fill = factor(sim.psi))) +
  ...
  ylim(-100, 100) +
  facet_grid(rows = vars(parameter), cols = vars(lambda.prior)) +
#  facet_wrap(~parameter, nrow = 2) +
  theme(legend.title = element_blank())

figs2

ggsave("Figure_s2_2022.09.17", plot = last_plot(), device = "tiff", dpi = 300, path = "G://My Drive/Personal_Drive/R/CKMR/Objective.3_intermittent.breeding/Model.results/figures/")

#other parameters
figs3 <- obj3_results.liz %>% dplyr::filter(model_used == "Liz_biennial",
                                   psi.prior != "fixed",
...
ggplot(aes(x=factor(sampling.scheme), fill = factor(sim.psi))) +
  ...
  #ylim(-100, 100) +
  facet_grid(rows = vars(parameter), cols = vars(lambda.prior), scales = "free") +
#  facet_wrap(~parameter, nrow = 2, scales = "free") +
  theme(legend.title = element_blank())

figs3

ggsave("Figure_s3_2022.09.17", plot = last_plot(), device = "tiff", dpi = 300, path = "G://My Drive/Personal_Drive/R/CKMR/Objective.3_intermittent.breeding/Model.results/figures/")
...
figs3 <- obj4_samps %>% dplyr::filter(lambda.prior == "none") %>% #There aren't many individuals older than 25
  ggplot(aes(x = yrs.off,
             y = age.x,
             fill = sampling.scheme)) + 
...
#Abundance
# obj4_results.all %>% dplyr::filter(parameter %in% c("Nf", "Nm"),
#                                    lambda.prior == "none") %>% 
# ggplot(aes(x=factor(parameter), fill = miss.cv)) +
#   geom_violin(aes(y=relative_bias), draw_quantiles = 0.5) +
#   #ylim(-50, 160) +
#   geom_hline(yintercept=0, col="blue", size=1.25, linetype = "dashed") +
#   #annotate("rect", xmin=0, xmax=Inf, ymin=-20, ymax=20, alpha=.5, col="red") +
...
#   ggtitle("Age miss: lambda prior = none") +
# #  ylim(-100, 100) +
#   facet_grid(rows = vars(sampling.scheme)) +
#   #facet_wrap(~prop.sampled.lab) +
#   guides(fill=guide_legend(title="Age assignment CV"))
# 
# #Lambda and phi
# obj4_results.all %>% dplyr::filter(!parameter %in% c("Nf", "Nm", "psi"),
...
#   labs(x="Parameter", y="Relative bias") +
#   scale_fill_manual(values = plot.colors.fig3) +
#   ggtitle("Age miss: lambda prior = diffuse") +
# #  ylim(-100, 100) +
#   facet_grid(rows = vars(sampling.scheme), scales = "free") +
#   #facet_wrap(~prop.sampled.lab) +
#   guides(fill=guide_legend(title="Age assignment CV"))
...
```{r Obj3 compare Ben and Liz viz}
plot.colors.fig3 <- c("lightslateblue", "lightblue1", "darkslateblue", "aquamarine")
scales::show_col(plot.colors.fig3)
