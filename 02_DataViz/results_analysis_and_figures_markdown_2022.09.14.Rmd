...
ggplot(aes(x=factor(sampling.scheme), fill = factor(sim.psi))) +
  ...
  ylim(-100, 100) +
  facet_grid(rows = vars(parameter), cols = vars(psi.prior)) +
...
ggplot(aes(x=factor(sampling.scheme), fill = factor(sim.psi))) +
  ...
  #ylim(-100, 100) +
  facet_grid(rows = vars(parameter), cols = vars(psi.prior), scales = "free") +
...
obj4_samps %>% dplyr::filter(lambda.prior == "none") %>% #There aren't many individuals older than 25
  ggplot(aes(x = yrs.off,
             y = age.x,
             fill = sampling.scheme)) + 
