...
plot.colors.fig2a <- c("lightslateblue", "lightblue1", "indianred3")
...
fig2.b <- obj2_results %>% dplyr::mutate(lambda.prior2 = ifelse(lambda.prior == "tight", "misspecified", as.character(lambda.prior))) %>%
  dplyr::filter(parameter %in% c("Nf", "Nm"),
                population.growth == "extreme negative",
                lambda.prior2 != "none") %>% 
  ggplot(aes(x = relative_bias,
             y = est.yr.lab, 
...
#  theme_ridges() + 
  ...
  xlim(-100, 200) +
  scale_fill_manual(values = plot.colors.fig2b,
                    name = "Lambda prior") +
  scale_y_discrete(expand = expansion(mult = c(0.01, .05)))
...
fig2.c <- lambda.extreme_comps4viz %>% ggplot(aes(x = ref.yrs, fill = sampling.scheme)) +
  geom_histogram(binwidth = 1,
                 ...
                 linetype = 1,
                 colour = "black") +
  labs(x="Reference year gap", title="C) Reference year gap") +
...
(Fig2cd <- ggarrange(fig2.c, fig2.d, common.legend = FALSE, legend = "bottom"))
```
