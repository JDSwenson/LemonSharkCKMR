...
NEXT TO DO: 
Fix purposes and make sure they mesh (they don't for obj2.2) - Think I just took care of this 07/16/2022
Transfer new objective 2 trials from cluster to local computer (started running 7/15) - DONE
...
obj2.1_pop_size <- readRDS(file = paste0(PopSim.location, "pop.size_", date.of.PopSim, "_", inSeeds, "_", PopSim.lambda.1, "_", PopSim.annual.breeding, "_", Sampling.scheme.YOY)) %>% 
...
```{r}
obj1.2_results %>% dplyr::filter(HSPs_detected == HSPs_expected & sampling.scheme != "sample all age classes") %>% 
  dplyr::arrange(iteration) %>% 
  dplyr::select(parameter, prop.sampled, iteration, sampling.scheme, relative_bias) %>% 
  View()
