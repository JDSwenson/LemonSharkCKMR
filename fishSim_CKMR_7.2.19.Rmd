...
for (y in c(t_start:t_end)) {
  ...
  indiv <- mort(indiv, type = "simple", maxPop = 9000, year=y, maxAge = 40) #make maxPop ~ 9000 if setting mort to flat, as this is the approx size of the population in the Bravington script.
  indiv <- birthdays(indiv)
  sample_vec <- append(sample_vec, sample(indiv[indiv[,5] > (y-n_ages) & is.na(indiv[,6]) == TRUE,][,1], size =
  n_samples_per_yr))
}
...
length(sample_vec)
sample_vec2 <- unique(sample_vec) #recapturing individuals -- eventually need to adjust simulation settings. For now, just get rid of recaptures.
length(sample_vec2)

tail(indiv)
#sampled[,1] = ID
#sampled[,2] = birth year
#sampled[,3] = death yeear
#sampled[,4] = sex
sampled <- data.frame(sample_vec2) #Make dataframe to store unique captures
death_yr <- c(rep(100:105, each=length(sample_vec2)/6)) #assign death year, evenly split among samples. Number is odd because of need to eliminate recaptures from dataset.

for(i in 1:length(sampled[,1])) {
  sampled[i,2] = as.numeric(indiv[which(indiv[,1] == sampled[i,1]),5]) #birth year
  sampled[i,3] =  death_yr[i] #death year
  sampled[i,4] = indiv[which(indiv[,1] == sampled[i,1]),2] #sex
}

sampled_col <- c("ID", "Birth Year", "Death Year", "Sex")
colnames(sampled) <- sampled_col
length(sampled[,1])
head(sampled)
tail(sampled)
#sampled[577:579,3] = 105 #Fill in any NAs at the tail of the data frame with the most recent sampling year
## Make this calculate automatically
