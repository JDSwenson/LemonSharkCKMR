...
#First two years of simulations, have every female breed on cycle
if(v <= 2){
...
} else if(v >2){ #After first two years, include a proportion that don't breed on-cycle due to condition
  
  mothers.new <- which(data1$sex == 'F' & data1$age.x == repro.age & data1$repro.cycle == repro.cycle.vec[v+1])

    mothers.conform <- which(data1$sex == 'F' & data1$age.x >= repro.age & data1$repro.cycle == repro.cycle.vec[v+1]) #determine which females are available to breed in this year; this is an index
  
  mothers.nonconform <- which(data1$sex=='F' & data1$age.x>=repro.age & data1$repro.cycle == repro.cycle.vec[v+1]) #determine which females are available to breed in this year; this is an index
