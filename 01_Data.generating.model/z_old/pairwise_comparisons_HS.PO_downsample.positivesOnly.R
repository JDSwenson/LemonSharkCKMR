...
#Get number of positive HS comparisons
downsample <- function(mom_comps.all, dad_comps.all, positives.HS, HS.samps.df){

  #Initialize vectors  
  mom.HS.samples.to.remove = dad.HS.samples.to.remove <- NULL
  #-------------------------Moms------------------------------#
mom_comps.HS.pos <- positives.HS %>% dplyr::filter(shared.parent == "mother")
mom.HS.samples <- c(mom_comps.HS.pos$older.sib, mom_comps.HS.pos$younger.sib)

mom.num.HS.pos <- nrow(mom_comps.HS.pos)

if(mom.num.HS.pos > max.HSPs){
  #Downsample positive comparisons
  mom_comps.HS.down <- mom_comps.HS.pos %>% 
    slice_sample(n = max.HSPs) #Can try adding a vector of weights here so the same year gaps are sampled proportionally
  
  #save names of downsampled individuals and only keep unique instances
  mom.HS.samples.to.keep <- c(mom_comps.HS.down$older.sib, mom_comps.HS.down$younger.sib) %>% 
    unique()
  
  #Create vector of sampled individuals to remove
  #This only contains individuals that were involved in positive comparisons
  mom.HS.samples.to.remove <- mom.HS.samples[which(!mom.HS.samples %in% mom.HS.samples.to.keep)]
  }

#------------------------------Dads-------------------------------------#
dad_comps.HS.pos <- positives.HS %>% dplyr::filter(shared.parent == "father")
dad.HS.samples <- c(dad_comps.HS.pos$older.sib, dad_comps.HS.pos$younger.sib)

dad.num.HS.pos <- nrow(dad_comps.HS.pos)

if(dad.num.HS.pos > max.HSPs){
  #Downsample positive comparisons
  dad_comps.HS.down <- dad_comps.HS.pos %>% 
    dplyr::filter(!older.sib %in% mom.HS.samples.to.remove & !younger.sib %in% mom.HS.samples.to.remove) %>% 
    slice_sample(n = max.HSPs) #Can try adding a vector of weights here so the same year gaps are sampled proportionally
  
  #save names of downsampled individuals and only keep unique instances
  dad.HS.samples.to.keep <- c(dad_comps.HS.down$older.sib, dad_comps.HS.down$younger.sib) %>% 
    unique()
  
  #double-check that we're not keeping individuals we tossed from mom's side
  sum(dad.HS.samples.to.keep %in% mom.HS.samples.to.remove)
  
  #Create vector of sampled individuals to remove
  #This only contains individuals that were involved in positive comparisons
  dad.HS.samples.to.remove <- dad.HS.samples[which(!dad.HS.samples %in% dad.HS.samples.to.keep)]
}

all.samples.to.remove <- unique(c(mom.HS.samples.to.remove, dad.HS.samples.to.remove))
