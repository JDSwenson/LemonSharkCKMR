...
colnames(results) <- c("N_est", "SE", "Truth", "Sex", "Parents_detected", "Samples")
 ...
 results %>% group_by(Sex) %>% 
  ...
  write.table(results, file = paste0("results/sex.specific_single.est_no.popgrowth_", n_samples, "_samples.12.23.2020.csv"), sep=",", dec=".", qmethod="double", row.names=FALSE)
}
