...
#Population growth fixed to observed population growth of adult males and females only, calculated separately.
    ...
    get_P_lemon <- function(Pars,P_Mother,P_Father,n_yrs,t_start,t_end){
      ...
      for(os_birth in 1:(n_yrs-1)){  #> = after, < = before
        for(ys_birth in (os_birth+1):n_yrs){
          if((ys_birth - os_birth) <= (max_age - min(f_adult_age) + 1)){ #Add 1 bc script sometimes goes over by one
            P_Mother[os_birth, ys_birth] <- (Surv^(ys_birth - os_birth))/(N_F*pop_growth_F_mean^(ys_birth-min_est_cohort_F))
      ...
      for(os_birth in 1:(n_yrs-1)){  #> = after, < = before
        for(ys_birth in (os_birth+1):n_yrs){
          if((ys_birth - os_birth) <= (max_age - min(m_adult_age) + 1)){ #Add 1 bc script sometimes goes over by one
            P_Father[os_birth,ys_birth] <- (Surv^(ys_birth - os_birth))/(N_M*pop_growth_M_mean^(ys_birth-min_est_cohort_M))
...
#Fit model
    ...
    #-----------------Loop end-----------------------------
results[index,] <- c(round(exp(CK_fit[1]),0), round(SE[1],0), round(Mom_truth[min_est_cohort_F],0), "F", sum(mom_positives[,3]), pop_growth_F_mean, n_samples)
results[index+1,] <- c(round(exp(CK_fit[2]),0), round(SE[2],0), round(Dad_truth[min_est_cohort_M],0), "M", sum(dad_positives[,3]), pop_growth_M_mean, n_samples)
colnames(results) <- c("N_est", "SE", "Truth", "Sex", "Parents_detected", "Pop_growth", "Samples")
 ...
 results %>% group_by(Sex) %>% 
  ...
  write.table(results, file = paste0("results/sex.specific.popgrowth_sex.specific.N", n_samples, "_samples.12.23.2020.csv"), sep=",", dec=".", qmethod="double", row.names=FALSE)
}