      ...
      for(os_birth in 1:(n_yrs-1)){  #> = after, < = before
        for(ys_birth in (os_birth+1):n_yrs){
          if((ys_birth - os_birth) <= (max_age - min(f_adult_age) + 1)){ #Add 1 bc script sometimes goes over by one
            P_Mother[os_birth, ys_birth] <- (Surv^(ys_birth - os_birth))/(N_F*pop_growth_all_mean^(ys_birth-min_est_cohort_F))
      ...
      for(os_birth in 1:(n_yrs-1)){  #> = after, < = before
        for(ys_birth in (os_birth+1):n_yrs){
          if((ys_birth - os_birth) <= (max_age - min(m_adult_age) + 1)){ #Add 1 bc script sometimes goes over by one
            P_Father[os_birth,ys_birth] <- (Surv^(ys_birth - os_birth))/(N_M*pop_growth_all_mean^(ys_birth-min_est_cohort_M))
...
colnames(results) <- c("N_est", "SE", "Truth", "Sex", "Parents_detected", "Pop_growth", "Samples")
 ...
 results %>% group_by(Sex) %>% 
  ...
  write.table(results, file = paste0("results/sex.combined.popgrowth_sex.specific.N", n_samples, "_samples.12.23.2020.csv"), sep=",", dec=".", qmethod="double", row.names=FALSE)
}