...
# (currently 2:0, which means sample the last 2 years of the simulation)
sample.years <- c(n_yrs - c(2:0))
...
for(iter in 1:iterations) {
  ...
  #Survival
  length(which(loopy.list[[n_yrs]]$Survival=='S' & loopy.list[[n_yrs]]$age.x>=repro.age))/length(which(loopy.list[[n_yrs]]$age.x>=repro.age)) #survival of adults
  ...
  #try({  
    ...
    sample.size <- c(100, 200, 300)[samps] #To loop over different sample sizes, draw a different number of samples each time
    ...
    pairwise.out.filt <- build.pairwise(sample.df_all.info)
