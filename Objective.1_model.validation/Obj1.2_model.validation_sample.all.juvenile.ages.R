...
script_name <- "Obj1.2_model.validation_sample.all.juvenile.ages.R" #Copy name of script here
primary_goal <- "Validate iniitial model" #Why am I running this simulation? Provide details

question1 <- "Does the model perform as expected in a simplified base-case scenario with a diffuse prior on survival?"
question2 <- ""
question3 <- ""
purpose <- "Obj1.2_model.validation_sample.all.juvenile.ages" #For naming output files
...
#If we only want to use one type of relationship, we'll filter here
    ...
    #Define JAGS data and model, and run the MCMC engine
      set.seed(rseed)
    source("Objective.1_model.validation/functions/Obj1.2_run.JAGS_HS.only.R")

      #Calculate mean truth over estimation period
