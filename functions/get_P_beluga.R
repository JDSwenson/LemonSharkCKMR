#Fills the arrays with probabilities of POP kinship based on ages. ideath1 starts at 41 because that is when the study starts. This means every time the loop completes, the first 40 columns are not given any values. Seems to produce the same values when line 8 is adjusted to for(ideath1 in t_start:t_end)
...
get_P_beluga <- function(Pars,P_Mother,P_Father,Maturity_age,n_yrs,t_start,t_end){
  ...
  for(ibirth1 in 1:n_yrs){  #n_yrs number of iterations, ibirth1 = mother's birth?
    for(ideath1 in max(ibirth1, t_start):t_end){ #ideath1 is mother's death year. Loops 60 times, but starts at year 41.
