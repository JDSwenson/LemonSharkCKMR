...
s_juv = c(round(seq(s_YOY, s_adult, length.out = 2), 2), rep(s_adult, times = 9))    # Juvenile survival - ramp up from YOY survival to adult survival; with 13 columns corresponding to years 0 (YOY) - 12 (mature adults), we set 11 different survival rates, starting with the YOY survival and moving to adult. The for loop in the function below starts at index 2, so doesn't repeat YOY survival
...
#  results$Relative_bias <- round(((results$N_est - results$Truth)/results$Truth)*100,1)
  ...
  write.table(results, file = paste0("../Results/Leslie_null_", n_samples, "_samples_01.18.2021_pos.csv"), sep=",", dec=".", qmethod="double", row.names=FALSE)
}