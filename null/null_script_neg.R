...
s_juv = c(round(seq(s_YOY, s_adult, length.out = 5), 2), rep(s_adult, times = 6))    # Juvenile survival - ramp up from YOY survival to adult survival; with 13 columns corresponding to years 0 (YOY) - 12 (mature adults), we set 11 different survival rates, starting with the YOY survival and moving to adult. The for loop in the function below starts at index 2, so doesn't repeat YOY survival
...
#want to sample entire population over last five years, with prob equal to stable_age
  ...
  N_f <- adult_truth[,min_est_cohort]/2
  N_m <- adult_truth[,min_est_cohort]/2
...
source("null/functions/get_P_lemon_HS.R")
source("null/functions/lemon_neg_log_lik_HS_Sex_specific.R")
...
mom_positives <- Data2[which(Data2$Mom_Matches >0), c(1:2,6)]
dad_positives <- Data2[which(Data2$Dad_Matches >0), c(1:2,7)]
dad_negatives = mom_negatives <- Data2
dad_negatives[,3] <- dad_negatives[,3] - dad_negatives[,7]
dad_negatives <- dad_negatives[,c(1:3)]
mom_negatives[,3] <- mom_negatives[,3] - mom_negatives[,6]
mom_negatives <- mom_negatives[,c(1:3)]
    ...
    CK_fit <- optimx(par=Pars,fn=lemon_neg_log_lik,hessian=TRUE, method="BFGS", Negatives_Mother=mom_negatives, Negatives_Father=dad_negatives, Pairs_Mother=mom_positives, Pairs_Father=dad_positives, P_Mother=P_Mother, P_Father=P_Father, t_start=t_start, t_end=t_end)
    ...
    estimates <- data.frame(cbind(round(exp(c(CK_fit$p1[1], CK_fit$p2[1])),0)), SE, c("F", "M"))
    estimates <- cbind(estimates, truth = round(adult_truth[min_est_cohort]/2,0))
    colnames(estimates) <- c("CKMR_estimate", "SE", "Sex", "Truth")
    estimates
    
    #-----------------Loop end-----------------------------
    results[index,] <- c(round(exp(CK_fit[1]),0), round(SE[1],0), round(adult_truth[min_est_cohort]/2,0), "F", sum(mom_positives[,3]), lam, n_samples)
    results[index+1,] <- c(round(exp(CK_fit[2]),0), round(SE[2],0), round(adult_truth[min_est_cohort]/2,0), "M", sum(dad_positives[,3]), lam, n_samples)
...
#  results$Relative_bias <- round(((results$N_est - results$Truth)/results$Truth)*100,1)
  ...
  write.table(results, file = paste0("../Results/Leslie_null_", n_samples, "_samples_01.18.2021_neg.csv"), sep=",", dec=".", qmethod="double", row.names=FALSE)
}