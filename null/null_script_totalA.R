...
mat_a[12:maxAge]=1  #Knife-edge maturity for all animals, beginning at age 12
...
#want to sample entire population over last five years, with prob equal to stable_age
  ...
  Samples2 <- t_end-Samples #Change age to birth year
  min_est_cohort <- min(Samples2) #Specify first year of estimate
...
source("null/functions/get_P_lemon_HS_TotalA.R")
source("null/functions/lemon_neg_log_lik_HS_TotalA.R")
...
for(i in 1:nrow(Data2)){
  Data2$Matches[i] <- sum(rbinom(n=Data2$freq[i], size=1, p=Data2$Parent_prob[i]))
}
...
parent_positives <- Data2[which(Data2$Matches >0), c(1:2,5)]
parent_negatives <- Data2
parent_negatives[,3] <- parent_negatives[,3] - parent_negatives[,5]
parent_negatives <- parent_negatives[,c(1:3)]
    ...
    CK_fit <- optimx(par=Pars,fn=lemon_neg_log_lik_TotalA,hessian=TRUE, method="BFGS", Negatives_Parent=parent_negatives, Pairs_Parent=parent_positives, P_Parent=P_Parent, t_start=t_start, t_end=t_end)
    ...
    yrs <- c(min(parent_positives$Young_sib_birth, parent_positives$Young_sib_birth):t_end)

    
    ###Need to finish adapting script below 01/19/2021    
...
#  results$Relative_bias <- round(((results$N_est - results$Truth)/results$Truth)*100,1)
  ...
  write.table(results, file = paste0("../Results/Leslie_null_", n_samples, "_samples_01.18.2021.csv"), sep=",", dec=".", qmethod="double", row.names=FALSE)
}