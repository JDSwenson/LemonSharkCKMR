...
source("/Users/Zeus/Documents/R/R_working_dir/CKMR/LemonSharkCKMR_GitHub/time_series/models/get_P_lemon_HS_time_series.R")
source("/Users/Zeus/Documents/R/R_working_dir/CKMR/LemonSharkCKMR_GitHub/likelihood_functions/lemon_neg_log_like_HS.R")
...
Samples[,1]=rep(c(t_start:t_end),times=c(rep(n_samples_per_yr, 3),n_samples_per_yr+1)) #what year did I capture you? repeat each year for the number of samples that were taken.Add one at the end so the total number of samples is met.
...
Data
tapply(Data$freq, Data$Young_sib_birth, sum)
...
Data2
t <- tapply(Data2$freq, Data2$Young_sib_birth, sum)
round(t/sum(t)*100,0)
...
#    cat(paste("Estimated mature female abundance: ",c(exp(CK_fit[1,1:7])),", SE = ",c(SE[1:7]),"\n"))
 #   cat(paste("Estimated mature male abundance: ",c(exp(CK_fit[1,8:14])),", SE = ",c(SE[8:14]),"\n"))
    ...
    sim_results[iter, 4] <- sum(Data_mom_yes[which(Data_mom_yes$Young_sib_birth==3),3]) #Number of HS mothers
    sim_results[iter, 5] <- round(exp(CK_fit$p2[1]),0) #Nf
    sim_results[iter, 6] <- round(SE[2],0) #NfSE
    sim_results[iter, 7] <- exp(Pars[2]) #Truth
    sim_results[iter, 8] <- sum(Data_mom_yes[which(Data_mom_yes$Young_sib_birth==4),3]) #Number of HS mothers
    sim_results[iter, 9] <- round(exp(CK_fit$p3[1]),0) #Nf
    sim_results[iter, 10] <- round(SE[3],0) #NfSE
    sim_results[iter, 11] <- exp(Pars[3]) #Truth
    sim_results[iter, 12] <- sum(Data_mom_yes[which(Data_mom_yes$Young_sib_birth==5),3]) #Number of HS mothers
    sim_results[iter, 13] <- round(exp(CK_fit$p4[1]),0) #Nf
    sim_results[iter, 14] <- round(SE[4],0) #NfSE
    sim_results[iter, 15] <- exp(Pars[4]) #Truth
    sim_results[iter, 16] <- sum(Data_mom_yes[which(Data_mom_yes$Young_sib_birth==6),3]) #Number of HS mothers
    sim_results[iter, 17] <- round(exp(CK_fit$p5[1]),0) #Nf
    sim_results[iter, 18] <- round(SE[5],0) #NfSE
    sim_results[iter, 19] <- exp(Pars[5]) #Truth
    sim_results[iter, 20] <- sum(Data_mom_yes[which(Data_mom_yes$Young_sib_birth==7),3]) #Number of HS mothers
    sim_results[iter, 21] <- round(exp(CK_fit$p6[1]),0) #Nf
    sim_results[iter, 22] <- round(SE[6],0) #NfSE
    sim_results[iter, 23] <- exp(Pars[6]) #Truth
    sim_results[iter, 24] <- sum(Data_dad_yes[which(Data_dad_yes$Young_sib_birth==3),3]) #Number of HS mothers
    sim_results[iter, 25] <- round(exp(CK_fit$p7[1]),0) #Nf
    sim_results[iter, 26] <- round(SE[7],0) #NfSE
    sim_results[iter, 27] <- exp(Pars[7]) #Truth
    sim_results[iter, 28] <- sum(Data_dad_yes[which(Data_dad_yes$Young_sib_birth==4),3]) #Number of HS mothers
    sim_results[iter, 29] <- round(exp(CK_fit$p8[1]),0) #Nf
    sim_results[iter, 30] <- round(SE[8],0) #NfSE
    sim_results[iter, 31] <- exp(Pars[8]) #Truth
    sim_results[iter, 32] <- sum(Data_dad_yes[which(Data_dad_yes$Young_sib_birth==5),3]) #Number of HS mothers
    sim_results[iter, 33] <- round(exp(CK_fit$p9[1]),0) #Nf
    sim_results[iter, 34] <- round(SE[9],0) #NfSE
    sim_results[iter, 35] <- exp(Pars[9]) #Truth
    sim_results[iter, 36] <- sum(Data_dad_yes[which(Data_dad_yes$Young_sib_birth==6),3]) #Number of HS mothers
    sim_results[iter, 37] <- round(exp(CK_fit$p10[1]),0) #Nf
    sim_results[iter, 38] <- round(SE[10],0) #NfSE
    ...
    sim_results[iter, 41] <- sum(Samples[,2]==1)
    sim_results[iter, 42] <- sum(Samples[,2]==2)
    sim_results[iter, 43] <- sum(Samples[,2]==3)
    sim_results[iter, 44] <- sum(Samples[,2]==4)
    sim_results[iter, 45] <- sum(Samples[,2]==5)
    sim_results[iter, 46] <- sum(Samples[,2]==6)
    sim_results[iter, 47] <- sum(Samples[,2]==7)
...
colnames(sim_results) <- c("Nf_yr3", "NfSE_yr3", "NfTruth_yr3", "Moms_detected_yr3",
                           ...
                           "Nm_yr3", "NmSE_yr3", "NmTruth_yr3", "Dads_detected_yr3",
...
head(sim_results)
apply(sim_results[, grep("^Dads", colnames(sim_results))], 2, mean) #Calculate mean detected dads
apply(sim_results[, grep("^Moms", colnames(sim_results))], 2, mean) #Calculate mean detected moms

#write.table(sim_results, file = paste0("Halfsib_sim_", n_samples, "samps_11.30.2019.csv"), sep=",", dec=".", qmethod="double", row.names=FALSE)

groupdf <- group_by(Data2, Young_sib_birth) %>%
  summarise(sum_by_yr = sum(freq))
