...
cat(paste("Estimated adult abundance: ",c(exp(CK_fit[1,1:4])),", SE = ",c(SE[1:4]),"\n"))
 #cat(paste("Estimated mature male abundance: ",c(exp(CK_fit[1,5:8])),", SE = ",c(SE[5:8]),"\n"))
    ...
    save(CK_fit, file=paste0("../Results/model_objects/Lemon_CKModel_HS_time_series_TotalA_null_iteration_", iter))
    #  } else {
...
write.table(sim_results, file = paste0("../Results/HS_time_series_TotalA_null_5.17.2020.csv"), sep=",", dec=".", qmethod="double", row.names=FALSE)
