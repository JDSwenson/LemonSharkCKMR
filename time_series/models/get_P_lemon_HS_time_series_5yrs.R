...
get_P_lemon <- function(Pars,P_Mother,P_Father,n_yrs,t_start,t_end){
  ...
  #N_F=exp(Pars[1:n_yrs]) #number of mature females
    ...
    for(ys_birth in max(3,(os_birth+1)):n_yrs){
      if((ys_birth - os_birth) <= (max_age - min(f_adult_age))){
        
      P_Mother[os_birth, ys_birth] <- (Surv^(ys_birth - os_birth))/N_F[ys_birth-2] #N_F is the number of females alive when the younger sibling was born
  ...
  for(os_birth in 1:(n_yrs-1)){  #> = after, < = before
    for(ys_birth in max(3,(os_birth+1)):n_yrs){
      ...
      P_Father[os_birth,ys_birth] <- (Surv^(ys_birth - os_birth))/N_M[ys_birth-2]
