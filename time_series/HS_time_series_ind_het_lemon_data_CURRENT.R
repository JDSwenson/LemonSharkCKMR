...
##NEXT: 
#1) Test model with fishSim
#2) Check naive model for the same years
...
source("./likelihood_functions/lemon_neg_log_like_HS_TS_IndHet.R")
...
sum_table_mom <- grouped_by_DOB_mom %>% 
  summarize(mean_pups_yr = round(mean(pups), 0)) %>%
  left_join(grouped_by_DOB_mom) %>% 
  select(Juv_study_DOB, Mother, pups, mean_pups_yr)

#Save mean pups/female/yr to a smaller table
mom_mean_pups_yr <- grouped_by_DOB_mom %>% 
...
sum_table_dad <- grouped_by_DOB_dad %>% 
  ...
  select(Juv_study_DOB, Father, pups, mean_pups_yr)

#Save mean pups/male/yr to a smaller table
dad_mean_pups_yr <- grouped_by_DOB_dad %>% 
  summarize(mean_pups_yr = round(mean(pups), 0))
...
group_mom_df[,1] <- as.numeric(as.character(group_mom_df[,1]))
group_mom_df[,2] <- as.numeric(as.character(group_mom_df[,2]))
group_mom_df$freq <- as.numeric(as.character(group_mom_df$freq))

#Test to see if working -- years 1,4,5,6 should be represented (not 3)
group_mom_df[group_mom_df$V3=="FEMALE#005",]
...
#Remove same cohort comparisons and those we're not interested in (unknowns)
...
head(mom_positives)
tail(mom_positives)
mom_positives[mom_positives$Mother=="FEMALE#005",]
...
group_dad_df[,1] <- as.numeric(as.character(group_dad_df[,1]))
group_dad_df[,2] <- as.numeric(as.character(group_dad_df[,2]))
group_dad_df$freq <- as.numeric(as.character(group_dad_df$freq))
...
#Create matrix of all possible pairwise comparisons - separately for mothers and fathers (to remove unknowns)
known_mothers <- Juv_lems %>%
  filter(Mother!="Unknown") #Filter out unknown mothers

mother_comparisons <- t(combn(as.numeric(known_mothers$Juv_study_DOB), m=2))
mother_comparisons <- plyr::count(t(apply(mother_comparisons, 1, sort)))
colnames(mother_comparisons)[1:2] <- c("Old_sib_birth", "Young_sib_birth")

#Remove same cohort comparisons
mother_comparisons = mother_comparisons2 <- mother_comparisons[which(mother_comparisons$Old_sib_birth != mother_comparisons$Young_sib_birth & mother_comparisons$Young_sib_birth >= min_est_cohort),]

#For fathers
known_fathers <- Juv_lems %>%
  filter(Father!="Unknown") #Filter out unknown fathers

father_comparisons <- t(combn(as.numeric(known_fathers$Juv_study_DOB), m=2))
father_comparisons <- plyr::count(t(apply(father_comparisons, 1, sort)))
colnames(father_comparisons)[1:2] <- c("Old_sib_birth", "Young_sib_birth")

#Remove same cohort comparisons
father_comparisons = father_comparisons2 <- father_comparisons[which(father_comparisons$Old_sib_birth != father_comparisons$Young_sib_birth & father_comparisons$Young_sib_birth >= min_est_cohort),]
...
nrow(mother_comparisons)
nrow(mom_total_matches)

nrow(father_comparisons)
nrow(dad_total_matches)

#Subtract positive comparisons from all comparisons to make negatives dataframes
mom_negatives <- merge(mom_total_matches, mother_comparisons2, by=c("Old_sib_birth", "Young_sib_birth")) %>%
  mutate(negs=freq-matches) %>% 
  select(c(Old_sib_birth, Young_sib_birth, negs)) #could also select matches if want total matches
mom_negatives  

dad_negatives <- merge(mom_total_matches, father_comparisons2, by=c("Old_sib_birth", "Young_sib_birth")) %>%
  mutate(negs=freq-matches) %>% 
  select(c(Old_sib_birth, Young_sib_birth, negs)) #could also select matches if want total matches
dad_negatives 


#So now we have six dataframes to use for probabilities of kinship:
head(mom_positives); nrow(mom_positives) #1 Positive comparisons specific to mother and year
head(dad_positives);nrow(dad_positives) #2 Positive comparisons specific to father and year
head(mom_negatives); nrow(mom_negatives) #3 Negative mother comparisons specific to year
head(dad_negatives); nrow(dad_negatives) #4 Negative father comparisons specific to year
sum_table_mom #5 5ummary of pups per female for each year 
sum_table_dad #6 Summary of pups per male for each year
#mom_mean_pups_yr #7 Mean pups per female per year - but don't need
#dad_mean_pups_yr #8 Mean pups per male per year - but don't need
...
######################## QC ##################################
#Check that the estimates reflect >= the number of parents detected
#Mothers
test97 <- subset(lems, subset=DOB == 1997, select=c(Indiv_ID, Capture_Year, Father, Mother, DOB, Sex)) #Subset for sharks borm in 1995
length(unique(test97$Mother))
round(exp(CK_fit$p1),0)

test98 <- subset(lems, subset=DOB == 1998, select=c(Indiv_ID, Capture_Year, Father, Mother, DOB, Sex)) #Subset for sharks borm in 1995
length(unique(test98$Mother))
round(exp(CK_fit$p2),0)

test99 <- subset(lems, subset=DOB == 1999, select=c(Indiv_ID, Capture_Year, Father, Mother, DOB, Sex)) #Subset for sharks borm in 1995
length(unique(test99$Mother))
round(exp(CK_fit$p3),0)

test00 <- subset(lems, subset=DOB == 2000, select=c(Indiv_ID, Capture_Year, Father, Mother, DOB, Sex)) #Subset for sharks borm in 1995
length(unique(test00$Mother))
round(exp(CK_fit$p4),0)

#Fathers
test97 <- subset(lems, subset=DOB == 1997, select=c(Indiv_ID, Capture_Year, Father, Father, DOB, Sex)) #Subset for sharks borm in 1995
length(unique(test97$Father))
round(exp(CK_fit$p5),0)

test98 <- subset(lems, subset=DOB == 1998, select=c(Indiv_ID, Capture_Year, Father, Father, DOB, Sex)) #Subset for sharks borm in 1995
length(unique(test98$Father))
round(exp(CK_fit$p6),0)

test99 <- subset(lems, subset=DOB == 1999, select=c(Indiv_ID, Capture_Year, Father, Father, DOB, Sex)) #Subset for sharks borm in 1995
length(unique(test99$Father))
round(exp(CK_fit$p7),0)

test00 <- subset(lems, subset=DOB == 2000, select=c(Indiv_ID, Capture_Year, Father, Father, DOB, Sex)) #Subset for sharks borm in 1995
length(unique(test00$Father))
round(exp(CK_fit$p8),0)
