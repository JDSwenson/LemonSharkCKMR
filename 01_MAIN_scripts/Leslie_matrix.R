...
corr.vec <- c(0, 0.25, 0.5, 0.75)
surv.vec <- c(.75, 0.8, .85, 0.9)
fec.vec <- c(2.5, 3, 3.5, 4) #Fecundity per breeding cycle i.e. ff
...
#Loop through different survival values
  for(s in 1:length(surv.vec)){
    mean.survival <- surv.vec[s]
    sd.survival <- .04

  #Loop through different fecundity values
    for(fec in 1:length(fec.vec)){
      ...
      fecSurv.draw <- mvrnorm(n=1, mu=vec.mean, Sigma=covar.mat)
...
Adult.survival <- fecSurv.draw[2] # CHANGED FROM 0.9; Adult survival
...
lambda.temp.df <- lambda1 %>% 
  mutate(mean.survival = mean.survival, mean.fecundity = mean.fecundity, correlation = correlation)
