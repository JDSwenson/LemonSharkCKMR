...
_Model validation_  
To initially validate our model, we assumed a scenario where adult survival and population growth are known with a 10% and 1% CV respectively (Fig 1). After confirming that the model performed as expected under this ideal scenario, we assumed a data-limited scenario where survival and population growth rate are unknown and compared two distinct methods of setting a diffuse prior on lambda in situations with stable population growth (Fig 1) and also in scenarios where the population may be increasing or declining over time (Fig S1).

Regardless of prior information, in most instances, the model captured the true value of abundance within the highest posterior density interval (HPDI) at the expected rate (Fig 1A), while the median of the posterior distributions was unbiased (Fig 1B). As expected, the CVs on abundance estimates were smallest when we assumed ample prior data to inform survival and population growth (Fig 1C); in the data-limited scenario, the CV was slightly improved by using a uniform prior for population growth relative to fixing population growth to three values in turn, though the model performed well under both scenarios.  Combined, these results demonstrate that our CKMR model reliably estimates abundance in both data-rich and data-limited scenarios.
...
results.obj1.4 <- read_csv(paste0(results_location, results_prefix, "_", date.of.simulation.obj1.4, "_", seeds, "_", purpose.obj1.4, ".csv")) %>%
  mutate(model_type = model.type.obj1.4,
         purpose.lab = purpose.obj1.4.lab)

results.obj1.5 <- read_csv(paste0(results_location, results_prefix, "_", date.of.simulation.obj1.5, "_", seeds, "_", purpose.obj1.5, "allCombined.csv")) %>% 
  mutate(model_type = model.type.obj1.5, 
         purpose.lab = purpose.obj1.5.lab)

#-------------Objective 2 --------------------#
date.of.simulation.obj2.1 <- "29Jun2022"
purpose.obj2.1 <- "obj2.1_model.validation"
purpose.obj2.1.lab <- "Target YOY | PopSim Lambda 1 | Informed priors"
model.type.obj2.1 <- "HS.only"

date.of.simulation.obj2.2 <- "29Jun2022"
purpose.obj2.2 <- "obj2.2_PopSimLambda.1_uniform.lambda.prior"
purpose.obj2.2.lab <- "Target YOY | PopSim Lambda 1 | Uniform lambda"
model.type.obj2.2 <- "HS.only"

date.of.simulation.obj2.3 <- "29Jun2022"
purpose.obj2.3 <- "obj2.2_PopSimLambda.1_fixed.lambda.prior"
purpose.obj2.3.lab <- "Target YOY | PopSim Lambda 1 | Fixed lambda"
model.type.obj2.3 <- "HS.only"

date.of.simulation.obj2.4 <- "29Jun2022"
purpose.obj2.4 <- "obj2.2_PopSimLambda.variable_uniform.lambda.prior"
purpose.obj2.4.lab <- "Target YOY | PopPSim Lambda variable | Uniform lambda"
model.type.obj2.4 <- "HS.only"

date.of.simulation.obj2.5 <- "29Jun2022"
purpose.obj2.5 <- "obj2.2_PopSimLambda.variable_fixed.lambda.prior"
purpose.obj2.5.lab <- "Target YOY | PopSim Lambda variable | Fixed lambda prior"
model.type.obj2.5 <- "HS.only"


seeds <- "Seeds2022.04.15"
sim.samples.obj2 <- "1prop.sampled" #Label on file output
samples.obj2.lab <- "1 percent sampled" #Label for dataframe and figures


#Set results to NULL and the script will run regardless of how many actual results are being compared
results.obj2.1 = results.obj2.2 = results.obj2.3 = results.obj2.4 <- results.obj2.5 <- NULL

results.obj2.1 <- read_csv(paste0(results_location, results_prefix, "_", date.of.simulation.obj2.1, "_", seeds, "_", purpose.obj2.1, ".csv")) %>% 
  mutate(model_type = model.type.obj2.1, 
         purpose.lab = purpose.obj2.1.lab)

results.obj2.2 <- read_csv(paste0(results_location, results_prefix, "_", date.of.simulation.obj2.2, "_", seeds, "_", purpose.obj2.2, ".csv")) %>% 
  mutate(model_type = model.type.obj2.2,
         purpose.lab = purpose.obj2.2.lab)

results.obj2.3 <- read_csv(paste0(results_location, results_prefix, "_", date.of.simulation.obj2.3, "_", seeds, "_", purpose.obj2.3, "allCombined.csv")) %>%
  mutate(model_type = model.type.obj2.3,
         purpose.lab = purpose.obj2.3.lab)

results.obj2.4 <- read_csv(paste0(results_location, results_prefix, "_", date.of.simulation.obj2.4, "_", seeds, "_", purpose.obj2.4, ".csv")) %>%
  mutate(model_type = model.type.obj2.4,
         purpose.lab = purpose.obj2.4.lab)

results.obj2.5 <- read_csv(paste0(results_location, results_prefix, "_", date.of.simulation.obj2.5, "_", seeds, "_", purpose.obj2.5, "allCombined.csv")) %>% 
  mutate(model_type = model.type.obj2.5, 
         purpose.lab = purpose.obj2.5.lab)
```
...
results.obj2.all <- results.obj2.1 %>% 
  bind_rows(results.obj2.2, results.obj2.3, results.obj2.4, results.obj2.5) %>% 
  #mutate(total_samples = total_juvenile_samples + total_adult_samples) %>%
  mutate(relative_bias = round(((Q50 - all.truth)/all.truth)*100, 1)) %>% #Can change truth to breed.truth if looking for number of active breeders
  mutate(in_interval = ifelse(HPD2.5 < all.truth & all.truth < HPD97.5, "Y", "N")) %>% 
  mutate(cv = (sd/mean)*100) %>% 
...
results.obj2.all$samples.lab <- factor(results.obj2.all$samples.lab, levels = c(samples.obj2.lab))

results.obj2.all$purpose.lab <- factor(results.obj2.all$purpose.lab, levels = c(purpose.obj2.1.lab, purpose.obj2.2.lab, purpose.obj2.3.lab, purpose.obj2.4.lab, purpose.obj2.5.lab))
...
no.convergence.obj2 <- results.obj2.all %>% dplyr::filter(Rhat > 1.01) %>% 
  mutate(purp = ifelse(purpose.lab == purpose.obj2.1.lab, 1,
                       ifelse(purpose.lab == purpose.obj2.2.lab, 2,
                              ifelse(purpose.lab == purpose.obj2.3.lab, 3, 4)))) %>% 
  distinct(purpose.lab, purp, iteration)
...
(fig1.p1.violin <- results.fig1.4viz %>% dplyr::filter(parameter == "Nf" | parameter == "Nm") %>% 
...
(figS2.Nf_HSPs <- results.fig1.4viz %>% dplyr::filter(parameter == "Nf") %>% 
...
(figS2.Nm_HSPs <- results.fig1.4viz %>% dplyr::filter(parameter == "Nm") %>% 
  ggplot(aes(x = HSPs_detected, color = sampling.scheme, fill = sampling.scheme)) +
    geom_density(alpha = 0.6) + 
...
```{r Fig1-print-lambda-plots2, echo=FALSE, out.width = "120%", fig.height = 7}
...
(figS1.p.violin <- results.figS1.4viz %>% dplyr::filter(parameter == "Nf" | parameter == "Nm") %>% 
  ggplot(aes(x=factor(parameter), fill = factor(purpose.lab))) +
  geom_violin(aes(y=cv), draw_quantiles = 0.5) +
  #ylim(-50, 160) +
  geom_hline(yintercept=0, col="black", size=1.25) +
...
```{r FigS1-print-plots, echo=FALSE, out.width = "120%", fig.height = 7}
#Survival HPDI and relative bias for all four sample sizes
ggarrange(figS1.p.Nf, figS1.p.Nm, figS1.p2.Nf, figS1.p2.Nm, common.legend = TRUE, nrow = 2, ncol = 2, legend = "none")
```

```{r FigS1-print-plots, echo=FALSE, out.width = "120%", fig.cap = "**Figure S1:** Model performance with diffuse priors with an increasing or declining population."}
figS1.p.violin
```
\
\
...
```{r FigS2-print-plots, echo=FALSE, out.width = "120%", fig.cap = "**Figure S1:** Elasticity analysis of lemon shark vital rates, showing that population growth is most sensitive to juvenile survival and fecundity at younger mature age classes."}
