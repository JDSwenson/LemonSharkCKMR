...
purpose.combo <- "Examine_mode_performance_when_psi_is_set_at_birth" #What am I comparing right now? This will be used to name output files
...
```{r HPDI line graphs}
(p.Nfb <- all.4viz %>% dplyr::filter(parameter == "Nfb", proportion_sampled == "1_percent_sampled") %>% 
...
p.Nm <- all.4viz %>% dplyr::filter(parameter == "Nm", proportion_sampled == "1_percent_sampled") %>% 
  ggplot(aes(x = interval.scale)) +
  geom_point(aes(y = percent_in_interval, color = purpose.lab, fill = purpose.lab, shape = purpose.lab), size = 2.5, alpha = .7) + 
  geom_smooth(aes(y = percent_in_interval, color = purpose.lab, linetype = ), se = FALSE, size = 1.5, show.legend = FALSE) + 
  geom_line(aes(y = interval.scale), size = 1.5) + 
...
```{r Density plots}
#Density plots
p2.Nfb <- results.all %>% dplyr::filter(parameter == "Nfb", samples.lab == samples2.lab) %>% 
...
p2.Nm <- results.all %>% dplyr::filter(parameter == "Nm", samples.lab == samples2.lab) %>% 
  ggplot(aes(x = relative_bias, color = purpose.lab, fill = purpose.lab)) +
  geom_density(alpha = 0.6) + 
  ggtitle("D) Nm") + 
  labs(x = "Relative bias of posterior median") +
