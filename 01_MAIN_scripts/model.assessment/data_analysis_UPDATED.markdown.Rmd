...
purpose.combo <- "sample.all.ages_vs_targeted.sampling_SB" #What am I comparing right now? This will be used to name output files
...
purpose1.lab <- "Targeted sampling"
model.type1 <- "HS.PO"

date.of.simulation2 <- "28Apr2022"
purpose2 <- "skipped.breeding_sample.all.ages"
...
jags_params <- c("Nfa", "Nfb", "Nm", "surv", "lam", "psi", "pb") #Specify parameters
...
results.all %>% dplyr::filter(parameter == "Nfa" | parameter == "Nm") %>% 
  group_by(purpose.lab, parameter, total_samples) %>% 
...
HPD.480.1.4viz <- results.1 %>% filter(total_samples == 480) %>% 
  ...
  anti_join(no.convergence, by = c("purpose.lab", "iteration", "total_samples"))
...
HPDI.plot.list <- list(p.Nfb, p.Nfa, p.pb, p.Nm, p.surv, p.lam)
...
HPDI.plots <- ggarrange(p.Nfb, p.Nfa, p.pb, p.Nm, p.surv, p.lam, common.legend = TRUE, ncol = 2, nrow = 3, legend = "bottom")
...
#Save plots so I can load them quickly later
Density.plot.list <- list(p2.Nfb, p2.Nfa, p2.pb, p2.Nm, p2.surv, p2.lam)
#Save the plots as a list, so I can load them later and plot with ggarrange/annotate_figure
saveRDS(Density.plot.list, file = paste0(results_plots_location, "plot_lists/", purpose.combo, "_Relative_bias_density_plots"))



RelBias.Density.Plots <- ggarrange(p2.Nfb, p2.Nfa, p2.pb, p2.Nm, p2.surv, p2.lam, common.legend = TRUE, ncol = 2, nrow = 3, legend = "bottom")
