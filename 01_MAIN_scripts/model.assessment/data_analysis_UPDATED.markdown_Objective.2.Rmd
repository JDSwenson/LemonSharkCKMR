...
purpose.combo <- "Determine best uninformative prior for lambda" #What am I comparing right now? This will be used to name output files
...
#Trial 1
s1.1 <- readRDS(paste0(MCMC_location, MCMC_prefix, "_", date.of.simulation1, "_", seeds, "_", sim.samples, "_", MCMC.settings, "_", purpose1))
s1.1 <- s1.1[1:100]

#Trial 2
s2.1 <- readRDS(paste0(MCMC_location, MCMC_prefix, "_", date.of.simulation2, "_", seeds, "_", sim.samples, "_", MCMC.settings, "_", purpose2))
...
#HPD.1.3.temp <- HPD.1.4.temp <- HPD.samples1.4 <- HPD.samples1.3 <- model.sum.4 <- model.sum.4.2 <- model.sum.3 <- model.sum.3.2 <- model.sum.3.all <- model.sum.4.all <-   NULL
```

```{r Create results file for combined mcmc chains for lambda}
#Extract truth values from runs with same sample scheme
results.1.4comb <- results.1 %>% dplyr::select(!c(Q2.5, Q97.5, Q50, mean, sd, HPD2.5, HPD97.5, Rhat, neff, purpose, purpose.lab))

results.3 <- model.sum.3.all %>% left_join(results.1.4comb, by = c("parameter", "iteration")) %>% 
  mutate(purpose = purpose3, purpose.lab = purpose3.lab)

results3.2 <- results.3 %>%
...
results.2.4comb <- results.2 %>% dplyr::select(!c(Q2.5, Q97.5, Q50, mean, sd, HPD2.5, HPD97.5, Rhat, neff, purpose, purpose.lab, relative_bias, in_interval))
results.4 <- model.sum.4.all %>% left_join(results.2.4comb, by = c("parameter", "iteration")) %>% 
    mutate(purpose = purpose4, purpose.lab = purpose4.lab)

results4.2 <- results.4 %>%
     mutate(relative_bias = round(((Q50 - all.truth)/all.truth)*100, 1)) %>% #Can change truth to breed.truth if looking for number of active breeders
     mutate(in_interval = ifelse(HPD2.5 < all.truth & all.truth < HPD97.5, "Y", "N")) %>%
     mutate(total_samples = total_juvenile_samples + total_adult_samples) %>%
     as_tibble()
...
results.all <- results.1 %>% 
  bind_rows(results.2, results3.2, results4.2) %>% 
     mutate(relative_bias = round(((Q50 - all.truth)/all.truth)*100, 1)) %>% #Can change truth to breed.truth if looking for number of active breeders
     mutate(in_interval = ifelse(HPD2.5 < all.truth & all.truth < HPD97.5, "Y", "N")) %>% 
...
#------------------------------- CV -----------------------------------#
results.all %>% dplyr::group_by(model_type, purpose.lab, parameter) %>% 
...
HPD.1.summary_all <- HPD.samples1.1.summary %>% 
...
#Combine above dataframes
HPD.2.summary_all <- HPD.samples1.2.summary %>% 
  mutate(interval.scale = interval*100)
