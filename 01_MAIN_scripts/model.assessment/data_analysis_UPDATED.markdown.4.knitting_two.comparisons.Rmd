...
HPD.samples1.1.4viz <- HPD.1.summary.tidy %>% dplyr::filter(proportion_sampled == "0.5_percent_sampled")
HPD.samples2.1.4viz <- HPD.1.summary.tidy %>% dplyr::filter(proportion_sampled == "1_percent_sampled")
HPD.samples3.1.4viz <- HPD.1.summary.tidy %>% dplyr::filter(proportion_sampled == "1.5_percent_sampled")
HPD.samples4.1.4viz <- HPD.1.summary.tidy %>% dplyr::filter(proportion_sampled == "2_percent_sampled")

HPD.samples1.2.4viz <- HPD.2.summary.tidy %>% dplyr::filter(proportion_sampled == "0.5_percent_sampled")
HPD.samples2.2.4viz <- HPD.2.summary.tidy %>% dplyr::filter(proportion_sampled == "1_percent_sampled")
HPD.samples3.2.4viz <- HPD.2.summary.tidy %>% dplyr::filter(proportion_sampled == "1.5_percent_sampled")
HPD.samples4.2.4viz <- HPD.2.summary.tidy %>% dplyr::filter(proportion_sampled == "2_percent_sampled")

# HPD.samples1.3.4viz <- HPD.3.summary.tidy %>% dplyr::filter(proportion_sampled == "0.5_percent_sampled")
# HPD.samples2.3.4viz <- HPD.3.summary.tidy %>% dplyr::filter(proportion_sampled == "1_percent_sampled")
# HPD.samples3.3.4viz <- HPD.3.summary.tidy %>% dplyr::filter(proportion_sampled == "1.5_percent_sampled")
# HPD.samples4.3.4viz <- HPD.3.summary.tidy %>% dplyr::filter(proportion_sampled == "2_percent_sampled")
# 
# HPD.samples1.4.4viz <- HPD.4.summary.tidy %>% dplyr::filter(proportion_sampled == "0.5_percent_sampled")
# HPD.samples2.4.4viz <- HPD.4.summary.tidy %>% dplyr::filter(proportion_sampled == "1_percent_sampled")
# HPD.samples3.4.4viz <- HPD.4.summary.tidy %>% dplyr::filter(proportion_sampled == "1.5_percent_sampled")
# HPD.samples4.4.4viz <- HPD.4.summary.tidy %>% dplyr::filter(proportion_sampled == "2_percent_sampled")
...
## Background info
One hundred different populations were simulated where 100% of females in the population bred biennially. Each population was sampled at four different sampling intensities corresponding to 0.5, 1, 1.5, and 2 percent of the juvenile population each year over four consecutive years. Samples were either targeted to the young-of-year (YOY) portion of the population only, or included juveniles of all age classes. A pairwise comparisons matrix that included all possible positive and negative comparisons was used as input to a CKMR model that either a) assumed no prior information for breeding schedule (psi) and survival, or b) assumed the breeding schedule was known without error (i.e. fixed to the truth) and included a tight prior on survival dnorm(mean = truth, sd = 0.02).
...
**Main takeaways:** This figure shows results for the lowest and highest sample schemes employed only (i.e. 0.5% and 2% of the juvenile population). When all age classes are sampled, the HPDI is slightly less likely to capture the truth, suggesting that the model may converge on inaccurate values in some instances. However, the density plots show that the median of the posteriors for the same scenarios do tend to be centered on 0.

With diffuse priors on psi and survival (red and green), there is a difference in the potential for the HPDI to capture the truth, with targeted sampling (red) performing better than sampling across age classes. Regardless of sampling scheme, when using a diffuse prior on survival and psi, the posterior medians for abundance estimates have a tendency to get pulled upward, similar to survival (Fig 2), though this effect is reduced when all age classes are sampled relative to targeted sampling of YOY.
...
**Main takeaways:** When priors are diffuse, survival estimates have a tendency to get pulled upward. This tendency is amplified when sampling is targeted to YOY, likely because there are fewer age gaps represented in the data as well as fewer half-siblings available to inform survival estimates.
