...
purpose.combo <- "Model validation with informed priors" #What am I comparing right now? This will be used to name output files
...
purpose1 <- "Obj1.1_model.validation"
purpose1.lab <- "Target YOY | Informed priors"
model.type1 <- "HS.only"
...
#date.of.simulation <- "07Apr2022"
# pop.size.tibble <- readRDS(paste0(results_location, pop.size.prefix, "_", date.of.simulation1, "_", seeds, "_", purpose1)) %>% mutate(Total.adult.pop = Male.adult.pop + Female.adult.pop)
...
results.all$purpose.lab <- factor(results.all$purpose.lab, levels = c(purpose1.lab))
...
HPD.samples1.1.4viz <- results.1 %>% filter(sample.prop.juvs == sim.samples) %>% 
...
------------------------------------------------------------------------------------------
##Don't usually worry about the below but can use it if wanted
```{r other analysis and viz}
