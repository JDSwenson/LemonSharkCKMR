...
#Outputs a list, where the first element is a list of various population parameters for each year, and the second is the population size for each year
  ...
  for(samps in 1:length(sample.vec)){
    ...
    mom_comps_gap <- pairwise.out.filt[[9]] %>% as_tibble()
    dad_comps_gap <- pairwise.out.filt[[10]] %>% as_tibble()
    ...
    jags_data = list(
      ...
      MHSP_gap = mom_comps_gap$yes, #positive comparisons for global abundance estimate (Y)
      M_c = mom_comps_gap$yr_gap, #Gap in years (c)
      mom_gap_n_comps = mom_comps_gap$all, #all comparisons (R)
      mom_gap_yrs = nrow(mom_comps_gap),
      ...
      FHSP_gap = dad_comps_gap$yes,  #positive comparisons for global abundance estimate (Y)
      F_c = dad_comps_gap$yr_gap, #Gap in years (c)
      dad_gap_n_comps = dad_comps_gap$all, #all comparisons (R)
      dad_gap_yrs = nrow(dad_comps_gap),
    ...
    HS_model = function(){
      ...
      #PRIORS
#      muMom ~ dunif(1, 10000) # Uninformative prior for global female abundance
 #     muDad ~ dunif(1, 10000)  # Uninformative prior for global male abundance
      Nf ~ dnorm(muMom, N.tau) # Uninformative prior for female abundance
      Nm ~ dnorm(muDad, N.tau) # Uninformative prior for male abundance
      surv ~ dbeta(1 ,1) # Uninformative prior for adult survival
      lam ~ dnorm(1, lam.tau) #Lambda
      muDad ~ dunif(0, 10000) # Uninformative prior for female abundance
      muMom ~ dunif(0, 10000) # Uninformative prior for male abundance
      
      
      for(mg in 1:mom_gap_yrs){
        MHSP_gap[mg] ~ dbin(surv^(M_c[mg])/muMom, mom_gap_n_comps[mg])
      }
      
      for(fg in 1:dad_gap_yrs){
        FHSP_gap[fg] ~ dbin(surv^(F_c[fg])/muDad, dad_gap_n_comps[fg])
      }
