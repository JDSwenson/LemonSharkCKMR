...
#cv <- c(0.05, 0.1) #What CVs do we want to loop over
...
mean.survival <- leslie.survival #adult survival
...
mean.fecundity <- leslie.fecundity
...
vec.mean <- c(mean.fecundity, YOY.survival, juvenile.survival, mean.survival)   #vector of the means
...
surv.cv = surv.cv
survival.sd <- surv.cv * mean.survival #survival standard deviation


#Set CV and SD for fecundity
fec.cv <- fec.cv
fecundity.sd <- fec.cv * mean.fecundity #Fecundity per breeding cycle i.e. ff

#create matrix of SD values
vec.sd <- c(fecundity.sd, survival.sd, survival.sd, survival.sd)
mat.sd <- diag(vec.sd)  
