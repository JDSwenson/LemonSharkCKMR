...
estimates <- model.summary2 %>%
  mutate(min = c(Mom_min, Dad_min, surv_min, lam_min, NA), 
         max = c(Mom_max, Dad_max, surv_max, lam_max, NA), 
         truth = c(Mom.all_truth, Dad.all_truth, surv_mean, lam_truth, psi_truth),
         breed.truth = c(Mom.breed_truth, Dad.breed_truth, surv_mean, lam_truth, psi_truth)) %>%
  as_tibble()
