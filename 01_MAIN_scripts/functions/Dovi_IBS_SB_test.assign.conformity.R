...
#------------------------------------Conformist mothers------------------------------------#
mothers <- which(init.pop$sex=='F' & init.pop$age.x>=repro.age & init.pop$repro.cycle == repro.cycle.vec[1] & repro.strategy == "conformist") # determine which females are available to breed in this year

#-----------------------------------Non-conformist mothers------------------------------------#
mothers.2 <- which(init.pop$sex=='F' & init.pop$age.x>=repro.age & repro.strategy == "non-conformist") #Assume for now that non-conformist means annual breeding
