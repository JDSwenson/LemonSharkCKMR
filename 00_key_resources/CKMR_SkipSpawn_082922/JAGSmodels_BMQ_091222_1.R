...
HSPOP_noLambda_Skip_model= function(){
  ...
  for(k in 1:POP.yrs)
  {
    positives_POPs[k] ~ dbin((1 / Nm * POP.isPaternal[k]) +
                               (1 / Nf * (1 - POP.isPaternal[k])),n.comps_POPs[k])
  }
}
...
HS.only_wideLambda_Skip_model = function(){
  ...
  for(j in 1:dad.yrs_HS){ # Loop over paternal cohort comparisons
    dad.positives_HS[j] ~ dbin(((survival^dad.mort.yrs_HS[j])/(Nm * (lambda^dad.popGrowth.yrs_HS[i]))), dad.n.comps_HS[j])
  }
...
HSPOP_wideLambda_Skip_model= function(){
  ...
  for(k in 1:POP.yrs)
  {
    positives_POPs[k] ~ dbin((1 / (Nm * (lambda^POP.popGrowth.yrs_HS[k]))  * POP.isPaternal[k]) +
                               (1 / (Nf * (lambda^POP.popGrowth.yrs_HS[k])) * (1 - POP.isPaternal[k])),n.comps_POPs[k])
  }
}
